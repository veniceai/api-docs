...
| Modo | Prefixo do ID do modelo | Proteção |
| --- | --- | --- |
| **TEE** | `e2ee-*` | Inferência isolada por hardware com atestação remota. |
| **E2EE** | `e2ee-*` | Criptografia no lado do cliente para um TEE verificado. |

Ambos os modos usam o mesmo ID de modelo. O fluxo da requisição determina se o modelo é executado apenas no modo TEE ou usa E2EE no lado do cliente.
