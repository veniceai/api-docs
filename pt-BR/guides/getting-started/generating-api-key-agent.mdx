...
'og:title': "Autonomous Agent API Key | Documentação da API Venice"
...
Este guia walks through the full flow end to end and covers the funding options for actually paying for inference once the key is minted.
...
* Any non-zero amount of VVV to stake. The minting endpoint requires only that the wallet has a non-zero sVVV balance, so 1 VVV is enough to mint a key. Veja [Paying for inference](#paying-for-inference) for what you need to actually call paid endpoints.
