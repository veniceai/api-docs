...
Para resolver o erro, aumente ou remova os Epoch Consumption Limits da chave em [venice.ai/settings/api](https://venice.ai/settings/api), ou chame [Update API Key](/api-reference/endpoint/api_keys/update) com novos `consumptionLimits`. Os saldos da conta (USD, DIEM e créditos incluídos) são fontes de pagamento independentes e são consumidos automaticamente conforme a disponibilidade; você não precisa escolher uma delas no momento da requisição.
