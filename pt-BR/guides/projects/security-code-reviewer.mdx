...
| Venice client | Wraps the OpenAI Python SDK pointed at Venice's compatível com OpenAI endpoint |
...
Venice is compatível com OpenAI, so we can use the official OpenAI Python SDK and just point its `base_url` at Venice. Centralising the client construction in one file means the rest of the code never has to know which provider it's talking to: swapping backends would only touch this one module.
