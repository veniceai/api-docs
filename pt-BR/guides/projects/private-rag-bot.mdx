...
We'll use the OpenAI Python SDK because Venice exposes an compatível com OpenAI API. We'll also use Qdrant's Python client with FastEmbed support:
...
The embedding model name is intentionally compatível com OpenAI. Venice maps compatible embedding model names to Venice-hosted embedding models, so existing OpenAI SDK code can usually move over by changing the `base_url` and API key.
...
Create one compatível com OpenAI Venice client for both embeddings and chat completions:
