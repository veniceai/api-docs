...
description: "Clone uma voz a partir de uma pequena amostra de áudio de referência com o Chatterbox HD, salve o handle de voz retornado e gere fala pela API de áudio da Venice."
...
<Note>
Os handles de voz são específicos do modelo. Um handle criado com `tts-chatterbox-hd` deve ser usado com `tts-chatterbox-hd`. A clonagem de voz é text-to-speech a partir de uma amostra de referência. Para regravar uma gravação existente em outra voz, use a API do [Trocador de Voz](/pt-BR/guides/media/voice-changer) em vez disso.
</Note>
...
Ao usar `curl -F`, não defina `Content-Type` manualmente. O `curl` adiciona o cabeçalho `multipart/form-data` e o boundary necessário.
...
O corpo da resposta é áudio binário no formato padrão do modelo, e não JSON. Atualmente, o `tts-chatterbox-hd` usa WAV como padrão.

---

## Exemplo completo

Este exemplo envia uma amostra de referência, extrai o handle de voz com `jq` e grava o áudio gerado em `chatterbox-clone.wav`:
...
| `file` | file | Sim | Amostra de áudio de referência. Os formatos suportados são MP3, WAV, FLAC e MP4. |
...
| `response_format` | string | Não | Específico do modelo | Substituição opcional do formato de saída. Verifique os formatos suportados e o padrão do modelo antes de defini-lo. |
...
A resposta de fala bem-sucedida é áudio binário, e seu `Content-Type` identifica o formato retornado. Você pode omitir `response_format` para usar o padrão do modelo. Consulte `model_spec.supported_formats` e `model_spec.default_format` em `GET /models?type=tts` antes de substituí-lo; solicitar um formato não suportado retorna HTTP `400`.

## Erros comuns

| Status | Causa | Correção |
|---|---|---|
| `400` | Container de áudio não suportado ou handle de voz incompatível | Use MP3, WAV, FLAC ou MP4 e combine o handle com o mesmo modelo usado para criá-lo. |
