...
description: "Aprimore resolução e qualidade de vídeos com o modelo topaz-video-upscale na fila assíncrona de vídeo da Venice, com fatores de upscale de 1x, 2x ou 4x."
...
`video_url` é **obrigatório** em cotações para modelos de upscale — o endpoint de cotação busca o arquivo e detecta duração, FPS e altura a partir dos bytes, então não há necessidade de fornecê-los. Omiti-lo retorna `400` com *"video_url is required for quotes on this model"*.

<Warning>
Os endpoints de cotação e de fila atualmente nomeiam o fator de upscale de forma diferente: `/video/queue` recebe `upscale_factor` (`1`, `2` ou `4`), enquanto `/video/quote` lê o fator de `resolution` (`"2x"` ou `"4x"`). Enviar `upscale_factor` ao endpoint de cotação não tem efeito — ele recai no padrão `"2x"` e retorna um preço de 2x para o que pode ser um job de 4x. Ao cotar um upscale de 4x, passe `resolution: "4x"`.
</Warning>
...
| `"Insufficient balance"` | Créditos da conta muito baixos | Adicione créditos em [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) |
