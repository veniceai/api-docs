...
description: "Gere vídeos a partir de texto ou imagens na API de fila assíncrona da Venice: cote o preço, envie para /video/queue e faça polling em /video/retrieve pelo MP4."
...
Para modelos que retornaram um `download_url` na fila, o retrieve sempre retorna JSON. Busque o vídeo com `GET download_url` (sem cabeçalho de autenticação). Veja [Links de download privados](#links-de-download-privados) para como essas URLs funcionam, retries e `DELETE` opcional.
...
| `prompt` | string | Sim | - | O que gerar. O comprimento máximo é específico de cada modelo e varia bastante — de algumas centenas de caracteres até 20.000. Modelos que não declaram seu próprio limite assumem 2.500 como padrão. Leia `model_spec.constraints.prompt_character_limit` de `GET /models?type=video` em vez de assumir um limite único |
...
| `bitrate_mode` | string | Não | `"standard"` | Apenas Seedance 2.0 / 2.5. `"standard"` ou `"high"`. `high` solicita uma codificação de maior qualidade e arquivo maior. Não altera o preço. Omita para outras famílias |
...
4. Se `Content-Type` for `application/json` e `status` for `"COMPLETED"`, faça `GET` no `download_url` da resposta de fila para buscar o vídeo (veja [Links de download privados](#links-de-download-privados))
...
Ao contrário da geração de imagem, o endpoint de fila de vídeo não aceita um parâmetro `safe_mode`. O suporte a conteúdo adulto é específico do modelo: escolha uma variante de modelo projetada para saída sem censura. A família Wan 2.7, por exemplo, expõe variantes Enhanced ao lado das variantes padrão:

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

Requisições enviadas às variantes padrão podem ser rejeitadas como violações de conteúdo (`422`). Veja [Modelos de vídeo](/models/video) e [Preços](/overview/pricing) para a lista atual de variantes e seus preços por segundo.
