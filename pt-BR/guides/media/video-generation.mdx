...
Ao contrário da geração de imagem, o endpoint de fila de vídeo não aceita um parâmetro `safe_mode`. O suporte a conteúdo adulto é específico do modelo: escolha uma variante de modelo projetada para saída sem censura. A família Wan 2.7, por exemplo, expõe variantes sem censura ao lado das variantes padrão:

- `wan-2-7-uncensored-text-to-video`
- `wan-2-7-uncensored-image-to-video`

Requisições enviadas a variantes não-uncensored podem ser rejeitadas como violações de conteúdo (`422`). Veja [Modelos de vídeo](/models/video) e [Preços](/overview/pricing) para a lista atual de variantes e seus preços por segundo.
