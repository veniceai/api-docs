...
"og:title": "Video Generation | Documentação da API Venice"
...
For models that returned a `download_url` at queue time, retrieve always returns JSON. Fetch the video with `GET download_url` (no auth header). Veja [Private download links](#private-download-links) for how these URLs work, retries, and optional `DELETE`.
...
1. Poll `/video/retrieve` on an interval (por exemplo, every 5 seconds)
...
4. If `Content-Type` is `application/json` and `status` is `"COMPLETED"`, `GET` the `download_url` from the queue response to fetch the video (veja [Private download links](#private-download-links))
...
Veja [Video Models](/pt-BR/models/video) for current model list and pricing.
