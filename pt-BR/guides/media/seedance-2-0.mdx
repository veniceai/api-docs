...
| `seedance-2-5-text-to-video-basic` | T2V | 480p / 720p / 1080p | Até 30s de saída; áudio nativo |
| `seedance-2-5-image-to-video-basic` | I2V | 480p / 720p / 1080p | Até 30s de saída; grounding por primeiro frame (e opcionalmente último frame) |
| `seedance-2-5-reference-to-video-basic` | R2V | 480p / 720p / 1080p | Até 30 imagens + 10 vídeos + 10 doadores de áudio; mesmos workflows Reference / Edit / Extend / Stitch |
...
Passe `resolution` como um dos seguintes: `480p`, `720p`, `1080p` ou `4k` (em minúsculas). Seedance **2.0** (não-Fast) aceita todos os quatro; **2.5** aceita `480p` / `720p` / `1080p`; **2.0 Fast** aceita apenas `480p` / `720p`. Descubra os IDs de modelo e capacidades ao vivo com `GET /models?type=video` — não hardcode a disponibilidade.
...
Conecta clipes de entrada com transições geradas por IA. Respeite os limites **específicos por família** de duração combinada e contagem de clipes em [Limites de entrada multimodal](#limites-de-entrada-multimodal) (Seedance 2.0: ≤3 clipes / ≤15 s combinados; Seedance 2.5: limites de vídeo maiores).
...
| Resoluções de saída | 480p / 720p / 1080p / **4k** (Fast: apenas 480p / 720p) | 480p / 720p / 1080p |
...
A resposta é JSON (`{ "status": "PROCESSING", ... }`) enquanto o job roda, e o corpo muda para bytes `video/mp4` quando ele conclui. `status` é `PROCESSING` ou `COMPLETED` — não existe status `failed`. Um job que falhou retorna, em vez disso, uma resposta não-200 carregando o erro registrado. Veja [Geração de Vídeo](/guides/media/video-generation) para o padrão completo de polling.
...
Submissões reference-to-video devem incluir pelo menos um de `reference_image_urls`, `reference_video_urls` ou `image_references`. Geração puramente text-only não é um workflow R2V válido — use um ID de modelo text-to-video em vez disso. `reference_audio_urls` sozinho não é suficiente (veja a seção sobre áudio acima). Observe que `video_references` **não** é aceito no Seedance — ele é específico de modelos que suportam áudio de voz por referência, e passá-lo retorna *"This model does not support per-reference video entities"*.
