...
description: "Generate images from text prompts using Venice's native image API or the compatível com OpenAI images endpoint"
'og:title': "Image Generation | Documentação da API Venice"
...
| `POST /images/generations` | compatível com OpenAI image generation API | Use this when migrating existing OpenAI image clients |
...
Use [Image Models](/pt-BR/models/image) or the [Models API](/pt-BR/api-reference/endpoint/models/list) to confirm which sizing fields each model accepts.
...
| `style_preset` | string | No | - | Apply a preset style from [Image Styles](/pt-BR/api-reference/endpoint/image/styles) |
...
Validation is model-specific. Check [Image Models](/pt-BR/models/image) and the [Models API](/pt-BR/api-reference/endpoint/models/list) before relying on a parameter across multiple models.
...
Some image models support `aspect_ratio` without a selectable `resolution` tier. Por exemplo, `qwen-image-2` accepts aspect ratio and maps it to model-specific output dimensions:
...
Other image models support `aspect_ratio` plus a `resolution` tier. Por exemplo, `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro` support `1K`, `2K`, and `4K`:
...
Use [Image Models](/pt-BR/models/image) to see which models support higher resolutions and how they are priced.

### Style presets

If the selected model supports it, `style_preset` lets you steer the output without rewriting your whole prompt. You can fetch valid preset names from [Image Styles](/pt-BR/api-reference/endpoint/image/styles):
...
Veja [Image Styles](/pt-BR/api-reference/endpoint/image/styles) for the current style list.

---

## compatível com OpenAI endpoint
...
Use the compatível com OpenAI route for faster migrations. Use `/image/generate` when you need Venice-specific options such as `cfg_scale`, `style_preset`, `variants`, or binary responses.
...
Veja [Image Models](/pt-BR/models/image) for the current model list, pricing, and feature support.
