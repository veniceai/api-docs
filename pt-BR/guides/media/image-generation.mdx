...
description: "Gere imagens a partir de prompts de texto com a API Venice, cobrindo opções de dimensionamento por modelo, presets de estilo, força de LoRA, safe mode e saída binária."
...
<Warning>
`qwen-image`, `qwen-image-3` e `qwen-image-3-pro` não aceitam mais `width` ou `height`. Enviar qualquer um deles retorna `400` com *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."* Verifique `model_spec.constraints` em `GET /models?type=image` antes de escolher os campos de dimensionamento.
</Warning>
...
Os nomes dos estilos ficam dentro de `data`, não no nível superior — leia `response.data` em vez de iterar diretamente sobre o objeto da resposta.
...
| `width` | integer | Não | `1024` | Largura de saída em pixels para modelos baseados em pixels, como `venice-sd35`. Rejeitado pela família Qwen Image — veja a nota abaixo |
| `height` | integer | Não | `1024` | Altura de saída em pixels para modelos baseados em pixels, como `venice-sd35`. Rejeitado pela família Qwen Image — veja a nota abaixo |
...
| `lora_strength` | integer | Não | - | Força (`0`-`100`) dos LoRAs integrados ao modelo; ignorado por modelos sem LoRAs adicionais |
...
Alguns modelos de imagem vêm com LoRAs adicionais integrados. O parâmetro `lora_strength` (integer, `0` a `100`) controla a intensidade com que esses LoRAs integrados são aplicados. Ele só afeta modelos que usam LoRAs adicionais. Outros modelos o ignoram.

A API não suporta upload nem carregamento de arquivos LoRA personalizados. Também não há um endpoint que liste os LoRAs incluídos em um modelo. O uso de LoRA se limita a ajustar `lora_strength` nos modelos que os incluem.

### Conteúdo adulto e safe mode

`safe_mode` controla se o conteúdo adulto na saída da geração é borrado. O padrão é `true`. Defina como `false` se o seu caso de uso permitir conteúdo adulto e você quiser a saída sem alteração:
...
Alguns modelos de imagem são projetados especificamente para geração adulta / sem censura e combinam bem com `safe_mode: false`. Exemplos na página de preços incluem `lustify-sdxl`, `lustify-v7` e `lustify-v8`. Veja [Modelos de imagem](/models/image) e [Preços](/overview/pricing) para a lista atual e os custos por modelo.

Para edição de imagens existentes, o modelo de edição padrão `qwen-edit` bloqueia imagens sexuais explícitas. Se você precisa de edição sem censura, use `qwen-edit-uncensored` via [Edição de imagem](/guides/media/image-editing).

Quando `safe_mode` está habilitado e o modelo retorna saída borrada ou moderada, inspecione os cabeçalhos de resposta `x-venice-is-blurred` e `x-venice-is-content-violation` se precisar detectar isso programaticamente.
...
### Controlando o embaçamento de conteúdo adulto em `/images/generations`

O endpoint compatível com OpenAI não aceita `safe_mode`. Enviá-lo retorna `400` com `Unrecognized key(s) in object: 'safe_mode'`. Use o parâmetro `moderation` no estilo OpenAI:

| Valor | Comportamento |
|---|---|
| `auto` (padrão) | Habilita o modo Safe Venice e embaça conteúdo adulto |
| `low` | Desabilita o modo Safe Venice; retorna a saída sem embaçamento |
...
`safe_mode` funciona apenas no endpoint nativo `POST /image/generate`. `moderation` funciona apenas no endpoint compatível com OpenAI `POST /images/generations`.
