...
Esse recurso está disponível nos modelos **Kling O3** e **Grok Imagine R2V** no [Venice Video Studio](https://venice.ai/video?utm_source=venice-api-documentation). Cada família de modelo usa uma abordagem diferente para imagens de referência — veja as seções específicas de cada modelo abaixo.
...
Você pode adicionar até **4 elements** por geração. Referencie-os no seu prompt usando `@Element1`, `@Element2` etc.
...
Cada tipo de entrada tem um teto independente imposto pela API:

| Limite | Valor |
|---|---|
| **Mínimo obrigatório** | Pelo menos 1 entrada visual (start frame, element ou scene image) |
| `elements` | **4 no máximo** |
| `scene_image_urls` | 4 no máximo |
| `reference_image_urls` por element | 1–3 |

<Note>
A API aplica esses tetos de forma independente — não há um orçamento combinado de imagens entre `elements`, `scene_image_urls` e os start/end frames, e adicionar um start ou end frame não reduz quantos elements você pode enviar. Exceder um único teto retorna um `400` nomeando esse campo, por exemplo *"elements must have at most 4 items"*.
</Note>
...
Vá em [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). No Model Browser à esquerda, selecione um dos modelos **Kling O3 Reference to Video**:
...
Repita para personagens ou objetos adicionais (até 4 elements no total).

<Warning>
`elements` e `scene_image_urls` são limitados a **4** cada, e os tetos se aplicam de forma independente. Veja [Limitações](#limitações) para detalhes.
</Warning>
...
| Erro `"elements must have at most 4 items"` | Mais de 4 elements enviados | Envie no máximo 4 elements; o teto não muda quando um start ou end frame está presente |
| Erro `"scene_image_urls must have at most 4 images"` | Mais de 4 scene images enviadas | Envie no máximo 4 scene images. Esse teto é independente do teto de elements |
...
| Duration | Qualquer segundo inteiro de 1s a 10s | 8s |
...
Vá em [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). No Model Browser, selecione **Grok Imagine R2V**.
...
| `reference_image_urls` | string[] | **Sim** | 1–7 URLs de imagem ou data URLs |
| `duration` | string | **Sim** | Qualquer segundo inteiro de `"1s"` a `"10s"`. O sufixo `s` é obrigatório |
| `aspect_ratio` | string | **Sim** | por exemplo, `"16:9"`, `"9:16"`, `"1:1"` |
| `resolution` | string | Não | `"480p"` (padrão) ou `"720p"` |

<Note>
O Grok Imagine R2V não usa os campos `elements`, `scene_image_urls` ou `image_url`. Todas as imagens de referência são passadas via `reference_image_urls`.
</Note>
...
| Erro "At least one reference image is required" | `reference_image_urls` está vazio ou ausente | Forneça pelo menos uma URL de imagem em `reference_image_urls` |
...
| Vídeo muito curto | Duração padrão é 8s | Defina duração para `"10s"` para vídeos mais longos |
| Erro `"Invalid enum value ... received '8'"` | `duration` foi enviado sem o sufixo `s` | Envie `"8s"`, não `"8"`. A API compara exatamente com a forma exibida |
