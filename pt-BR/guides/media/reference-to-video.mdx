...
'og:title': "Reference to Video Guide | Documentação da API Venice"
...
<Warning>
The combined total of first frame, last frame, elements, and scene images cannot exceed **7**. Veja [Limitations](#limitations) for details.
</Warning>

<Tip>
**Best reference images:** Use well-lit photos with a clean background. Provide front, side, and 45-degree angle views for the strongest identity lock. Certifique-se de all reference images share the same visual style (don't mix photorealistic and anime).
</Tip>
...
If you want to control the exact first frame of your video, switch to the **Image** input type and upload a start frame. Você também pode optionally set an end frame.
...
<Warning>
Multi-shot total duration cannot exceed 15 seconds. Por exemplo, three 5-second shots = 15s maximum.
</Warning>
...
**Exemplo:** A 10-second video with audio = 10 × $0.14 = **$1.40**
...
Kling O3 Reference to Video is also available via the Venice API. Veja o [Video Queue API](https://docs.venice.ai/api-reference/endpoint/video/queue) for full details.
...
**Exemplo:** An 8-second video at 480p = 8 × $0.063 = **~$0.50**
