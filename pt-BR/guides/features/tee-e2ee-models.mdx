---
title: "Modelos TEE e E2EE"
description: "Execute inferência privada na Venice com modelos Trusted Execution Environment (TEE) e criptografia ponta a ponta que mantêm os prompts ilegíveis para o host."
'og:title': "Modelos TEE e E2EE | Documentação da API Venice"
'og:description': "Aprenda como usar modelos TEE e E2EE para máxima privacidade em suas aplicações de IA"
---

A Venice oferece modelos com privacidade aprimorada que rodam em Trusted Execution Environments (TEE) e suportam criptografia ponta a ponta (E2EE). Esses modelos fornecem garantias criptográficas de que seus dados permanecem privados — mesmo da Venice.

## Entendendo os níveis de privacidade

| Tipo | Prefixo | O que significa |
|------|---------|-----------------|
| **TEE** | `e2ee-*` | O modelo roda em um enclave protegido por hardware. A Venice não consegue acessar a computação. Você pode verificar isso por atestação. |
| **E2EE** | `e2ee-*` | Criptografia ponta a ponta completa. Seus prompts são criptografados no lado do cliente antes de serem enviados. Apenas o TEE consegue decifrá-los. |

<Info>
Tanto os IDs de modelos TEE quanto E2EE usam o prefixo `e2ee-*`. Isso ocorre porque todo modelo com atestação TEE na Venice também é compatível com E2EE — a atestação TEE é o mecanismo que torna a criptografia ponta a ponta possível —, então eles compartilham uma única família de modelos e prefixo. Seja usando inferência TEE simples ou o fluxo E2EE completo, o ID do modelo é o mesmo. Modelos E2EE incluem proteção TEE além de criptografia no lado do cliente. Modelos TEE fornecem segurança de enclave sem exigir criptografia no lado do cliente.
</Info>
...
Veja a [página de modelos](/overview/models) para a lista completa com preços e limites de contexto.

## Modelos TEE

Modelos TEE rodam dentro de enclaves protegidos por hardware (Intel TDX, NVIDIA Confidential Computing). Os pesos do modelo e seus dados são protegidos do sistema host — incluindo a infraestrutura da Venice.

### Uso básico

Modelos TEE funcionam exatamente como modelos regulares:
...
### Verificando a atestação TEE

Você pode verificar criptograficamente que um modelo está rodando em um TEE genuíno buscando seu relatório de atestação:
...
# Fetch attestation
curl "https://api.venice.ai/api/v1/tee/attestation?model=e2ee-qwen3-5-122b-a10b&nonce=$NONCE" \
  -H "Authorization: Bearer $API_KEY_VENICE"
```
...
response = requests.get(
    ...
    params={"model": "e2ee-qwen3-5-122b-a10b", "nonce": nonce},
...
curl "https://api.venice.ai/api/v1/tee/signature?model=e2ee-qwen3-5-122b-a10b&request_id=chatcmpl-abc123" \
  -H "Authorization: Bearer $API_KEY_VENICE"
```
...
response = requests.get(
    ...
    params={"model": "e2ee-qwen3-5-122b-a10b", "request_id": completion_id},
    headers={"Authorization": f"Bearer {api_key}"}
)
