...
'og:title': "Reasoning Models | Documentação da API Venice"
...
See the full list of models, pricing and context limits on the [Models page](/pt-BR/overview/models). Not all reasoning models support the [`reasoning_effort`](#reasoning-effort) parameter. Veja [model support](#model-support) for details.
...
<Warning>
Not all models support all values. Venice does **not** auto-map to the nearest supported level. Unsupported values return a 400 error from the upstream provider. Por exemplo, sending `xhigh` to Claude or `max` to GPT-5.2 will fail.
...
Each model's upper bound is available as `maxCompletionTokens` on the [`/v1/models`](/pt-BR/api-reference/endpoint/models/list) endpoint.
...
Check what a model supports via the [`/v1/models`](/pt-BR/api-reference/endpoint/models/list) endpoint:
