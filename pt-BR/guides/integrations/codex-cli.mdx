...
"og:title": "Codex CLI with Venice | Documentação da API Venice"
"og:description": "Configure Codex CLI to send requests to Venice AI using a simple model provider config"
---

Este guia shows how to run OpenAI Codex CLI with Venice using Codex's official config paths: `~/.codex/config.toml` (user-level) or `.codex/config.toml` (project-level).
...
<CardGroup cols={3}>
  ...
  <Card title="OpenAI Compatible" icon="plug">
    Uses Venice's compatível com OpenAI API
  </Card>
...
<Steps>
  ...
  <Step title="Replace the two placeholders">
    ...
    You can browse available model IDs in the [text model catalog](/pt-BR/models/text).
  </Step>
...
<CardGroup cols={2}>
  <Card title="Venice API Reference" icon="book" href="/pt-BR/api-reference/api-spec">
