---
title: Jan AI
description: Use Jan AI with Venice through an compatível com OpenAI remote model provider
"og:title": "Jan AI with Venice | Documentação da API Venice"
"og:description": "Configure Jan AI to use Venice models through an compatível com OpenAI custom endpoint"
---

[Jan](https://jan.ai/) is an open-source AI assistant for running local and remote models from one desktop app. Venice works in Jan through its compatível com OpenAI remote model provider support.
...
<CardGroup cols={3}>
  ...
  <Card title="OpenAI Compatible" icon="plug">
    Use Venice's compatível com OpenAI chat API
  </Card>
...
- A Venice text model ID from the [model catalog](/pt-BR/models/text)
...
<Steps>
  ...
  <Step title="Open Jan's model provider settings">
    ...
    Choose Jan's compatível com OpenAI custom provider flow. Depending on your Jan version, this may appear as **OpenAI** with **Custom Endpoint** enabled, or as an **Any OpenAI Compatible API** provider.
  </Step>
  ...
  <Step title="Configure the Venice endpoint">
    ...
    | API type | compatível com OpenAI |
  ...
  <Step title="Add Venice models">
    ...
    You can add any supported text model from the [model catalog](/pt-BR/models/text).
  </Step>
  ...
  <Step title="Start a chat">
    ...
    Jan will send the request to Venice using your API key and the compatível com OpenAI base URL.
  </Step>
</Steps>
...
Você também pode verify your API key outside Jan with:
...
- Model IDs must match Venice exactly. If a model does not appear in Jan's selector, add it manually using the model ID from the [model catalog](/pt-BR/models/text).
- Keep Venice configured as an compatível com OpenAI provider. Do not use Jan's native Anthropic provider for Venice Claude models.
...
<AccordionGroup>
  ...
  <Accordion title="Requests still go to another provider">
    Certifique-se de the chat is using a model attached to your Venice provider. If Jan is using a built-in OpenAI or Anthropic model entry, switch to the Venice model you added.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Venice Text Models" icon="list" href="/pt-BR/models/text">
