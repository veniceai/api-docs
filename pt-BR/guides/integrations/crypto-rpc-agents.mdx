...
"og:title": "Crypto RPC for Agents | Documentação da API Venice"
...
|---|---|
| **Inference** | 230+ text, image, video, audio, and embedding models through one compatível com OpenAI endpoint |
...
| **x402 wallet** | Autonomous, crypto-native, or short-lived agents | Wallet signs a Sign-In-With-X message, pays per request in USDC on Base or Solana. No Venice account needed. Veja o [x402 guide](/pt-BR/guides/integrations/x402-venice-api). |

Both methods share the same rate limits and billing in Venice credits.

<Tip>
Truly autonomous agents can mint their own API key by staking VVV on Base. Veja [Autonomous Agent API Key Creation](/pt-BR/guides/getting-started/generating-api-key-agent).
</Tip>
...
Use [`GET /crypto/rpc/networks`](/pt-BR/api-reference/endpoint/crypto/networks) for the live, authoritative list.
...
Full list and pricing detail in the [Crypto RPC API reference](/pt-BR/api-reference/endpoint/crypto/rpc).
...
Veja [Autonomous Agent API Key Creation](/pt-BR/guides/getting-started/generating-api-key-agent) for the complete walkthrough with code and the full error reference.
...
The same wallet auth works against `/crypto/rpc/{network}` for blockchain reads and writes. Full protocol details in the [x402 guide](/pt-BR/guides/integrations/x402-venice-api).
...
|---|---|---|
| `400` | Unsupported or unmapped JSON-RPC method, or malformed batch | Verify the method against the [allowlist](/pt-BR/api-reference/endpoint/crypto/rpc). The error body names the offending method. |
...
<CardGroup cols={2}>
  <Card title="Crypto RPC API Reference" icon="code" href="/pt-BR/api-reference/endpoint/crypto/rpc">
    Full method list, pricing, and response headers
  </Card>
  <Card title="Supported Networks" icon="link" href="/pt-BR/api-reference/endpoint/crypto/networks">
  ...
  <Card title="Autonomous Agent API Key" icon="robot" href="/pt-BR/guides/getting-started/generating-api-key-agent">
  ...
  <Card title="Pricing" icon="coins" href="/pt-BR/overview/pricing">
