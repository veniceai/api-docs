...
<Steps>
  ...
  <Step title="Emita uma chave de API">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` retorna um desafio EIP-4361 de uso único, vinculado a essa carteira, que expira 15 minutos após a emissão. Assine a `message` exatamente como recebida com a carteira de staking, depois faça `POST` do endereço, assinatura e mensagem de volta. A Venice retorna uma chave de API vinculada à conta de usuário derivada dessa carteira.
  </Step>
</Steps>
