...
<Note>
Use `OpenAIChatModel` (Chat Completions), não `OpenAIResponsesModel` nem o atalho `openai:`. A Venice atualmente não expõe a API Responses da OpenAI; seu caminho de geração de texto é `/chat/completions`. Fixar o Chat Completions evita o comportamento exclusivo do Responses que os modelos padrão da OpenAI usam em versões mais recentes do PydanticAI.
</Note>
