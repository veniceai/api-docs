...
"og:title": "NanoClaw with Venice | Documentação da API Venice"
...
Veja o [model catalog](/pt-BR/models/text) for all available Venice models.
...
<AccordionGroup>
  ...
  <Accordion title="Claude Code shows 403 error or 'Please run /login'">
    ...
    2. **Certifique-se de you're in the right folder.** Always `cd nanoclaw-venice` first.
  ...
  <Accordion title="Model errors ('model does not exist')">
    ...
    **Linux:**
    ...
    Check available models at the [model catalog](/pt-BR/models/text).
  </Accordion>

  <Accordion title="Bot doesn't respond to messages">
    Work through these steps in order:

    1. **Check your trigger word.** Certifique-se de you're using the right prefix (e.g., `@Andy hello`).
  ...
  <Accordion title="Container build fails during setup">
    Certifique-se de Docker Desktop is open and running. Wait 10 seconds for Docker to fully start, then type `continue` in the wizard to retry.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Using Claude Code through Venice (no bot)">
    ...
    **Dica:** Add this to your `~/.zshrc` (or `~/.bashrc`) so you can quickly switch any terminal to Venice:
...
<AccordionGroup>
  ...
  <Accordion title="Can I use open-source models?">
    Yes. Venice hosts many models. Tell the bot "switch to zai-org-glm-5" or any Venice model ID. Veja o [model catalog](/pt-BR/models/text).
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Venice Model Catalog" icon="list" href="/pt-BR/models/text">
    Browse available models
  </Card>
  <Card title="Venice Privacy" icon="shield-halved" href="/pt-BR/overview/privacy">
