...
<Steps>
  <Step title="Get a Venice API key">
    Generate one from [venice.ai/settings/api](https://venice.ai/settings/api). Veja o [API key guide](/pt-BR/guides/getting-started/generating-api-key) for step-by-step instructions.
  </Step>
...
| Tool | Description |
|---|---|
| `venice_chat` | compatível com OpenAI chat completion against Venice's full LLM catalog. |
| `venice_responses` | compatível com OpenAI Responses API with single- or multi-turn tool support. |
...
Only relevant if you authenticate with a wallet via [x402](/pt-BR/guides/integrations/x402-venice-api) instead of an API key.
...
| `VENICE_SIWX_TOKEN` | _(none)_ | x402 wallet-mode auth token. Veja [x402 below](#x402-wallet-mode). |

If both `VENICE_API_KEY` and `VENICE_SIWX_TOKEN` are set, the API key wins.

## x402 wallet mode

Venice supports authenticating with a [Sign-In-With-X wallet token](/pt-BR/guides/integrations/x402-venice-api) backed by prepaid USDC credit on Base or Solana, in addition to the normal API key flow. No email, phone, or KYC required: your wallet is the only identity.
...
<CardGroup cols={2}>
  ...
  <Card title="Venice Skills" icon="book" href="/pt-BR/guides/integrations/venice-skills">
  ...
  <Card title="MCP Spec" icon="arrow-up-right-from-square" href="https://modelcontextprotocol.io/">
    Saiba mais sobre the Model Context Protocol
  </Card>
</CardGroup>
