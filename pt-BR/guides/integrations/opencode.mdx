---
title: OpenCode
description: Use OpenCode with Venice AI through an compatível com OpenAI custom provider
"og:title": "OpenCode with Venice | Documentação da API Venice"
"og:description": "Configure OpenCode to send coding-agent requests to Venice AI using a custom provider"
---

[OpenCode](https://opencode.ai/) is an AI coding agent for the terminal. Venice works through OpenCode's custom provider config using the compatível com OpenAI adapter and a custom base URL.
...
Você deve see the Venice models from your config. Select one, then send a small test prompt.

If you set `"model": "venice/zai-org-glm-5-1"` as shown above, OpenCode should use that Venice model by default. You can change the default by replacing `zai-org-glm-5-1` with any model ID from the [text model catalog](/pt-BR/models/text) and adding it to the `models` map.

---

## Config Notes

- Keep `baseURL` set to `https://api.venice.ai/api/v1`. OpenCode's compatível com OpenAI adapter uses this base URL for chat completions.
- Keep `npm` set to `@ai-sdk/openai-compatible` for Venice's compatível com OpenAI chat endpoint.
...
<Info>
OpenCode already supports custom provider `baseURL` values and compatível com OpenAI providers through `@ai-sdk/openai-compatible`. No OpenCode core provider changes are required for Venice.
</Info>
