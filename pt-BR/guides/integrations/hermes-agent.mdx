...
"og:title": "Hermes Agent with Venice | Documentação da API Venice"
...
The Venice API gives your Hermes Agent access to the full Venice platform through a single compatível com OpenAI endpoint.
...
<Tip>
Você também pode configure Venice directly in `~/.hermes/config.yaml`:
...
Venice models in Hermes Agent follow the same [privacy tiers](/pt-BR/overview/privacy) as the Venice API:
