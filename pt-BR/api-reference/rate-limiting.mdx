...
Jobs de vídeo, música e trocador de voz não têm limite de taxa. Eles são cobrados por geração contra seu saldo de créditos, então a restrição prática é o custo, e não um teto de requisições. Calcule o preço de um job primeiro com [`POST /video/quote`](/pt-BR/api-reference/endpoint/video/quote), [`POST /audio/quote`](/pt-BR/api-reference/endpoint/audio/quote) ou [`POST /audio/voice-changer/quote`](/pt-BR/api-reference/endpoint/audio/voice-changer/quote).
