---
title: Introdução
description: "Referência da Venice API com autenticação, depuração, compatibilidade OpenAI, cabeçalhos de resposta, erros e endpoints suportados."
...
A API Venice usa chaves de API para autenticação. Crie e gerencie suas chaves de API em suas [configurações de API](https://venice.ai/settings/api?utm_source=venice-api-documentation).


A maioria das requisições da API exige autenticação Bearer HTTP. Alguns endpoints de descoberta — incluindo `GET /models`, `GET /models/traits`, `GET /models/compatibility_mapping` e `GET /image/styles` — podem ser lidos sem uma chave:
...
As chaves são do tipo `INFERENCE` ou `ADMIN`. Chaves de inferência podem chamar os endpoints de inferência — chat, imagem, vídeo, áudio, embeddings e os endpoints de augmentação. Endpoints de gerenciamento de conta exigem uma chave `ADMIN`:

| Endpoint | Chave necessária |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`, `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | Qualquer uma |
| `GET /billing/balance`, `/billing/usage-history`, `/billing/usage-analytics` | `ADMIN` |

Chamar um endpoint exclusivo de admin com uma chave de inferência retorna `401` com `{"error":"Admin API key required"}`. Observe que `GET /api_keys/rate_limits` é a única exceção nesse grupo — ele funciona com qualquer um dos dois tipos de chave.
...
<Warning>
A URL base é `https://api.venice.ai/api/v1`, e não `https://venice.ai`. Requisições enviadas para o domínio do site chegam ao aplicativo web em vez da API e falham com erros como `405 Method Not Allowed`.
</Warning>
...
| `enable_web_citations` | boolean | Quando a busca na web está habilitada, solicita que o LLM cite suas fontes usando sobrescritos com circunflexo — `^1^` para uma única fonte e `^1,2^` para várias. Formas com colchetes como `[1]`, `(1)` e `<sup>1</sup>` são explicitamente suprimidas | `false` |
...
6. **Entrada de embeddings**: `input` deve ser uma string ou um array de strings. Arrays de IDs de tokens são rejeitados com HTTP 400. `OpenAIEmbeddings` do LangChain deve definir `check_embedding_ctx_length=False`
