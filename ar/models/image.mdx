...
<Tip>
**مستويات الجودة.** يقبل `gpt-image-2` (التوليد) و `gpt-image-2-edit` (التحرير/التحرير المتعدد) معاملًا اختياريًا `quality` — `low` أو `medium` أو `high` (الافتراضي). المستويات الأقل تكلف أقل؛ راجع [نظرة عامة على التسعير](/overview/pricing#image-generation) للمصفوفة الكاملة أو اجلب `model_spec.pricing.quality` من [endpoint النماذج](/api-reference/endpoint/models/list).
</Tip>