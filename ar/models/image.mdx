...
<Note>
راجع [واجهة Image Generate API](/ar/api-reference/endpoint/image/generate) لـ text-to-image، و[واجهة Upscale API](/ar/api-reference/endpoint/image/upscale) للتحسين، و[واجهة Edit API](/ar/api-reference/endpoint/image/edit) لـ inpainting.
</Note>

<Tip>
تحديد أبعاد توليد الصور خاص بكل نموذج. النماذج المعتمدة على البكسل مثل `venice-sd35` تستخدم `width` و `height`؛ نماذج نسبة العرض إلى الارتفاع مثل `qwen-image-2` تستخدم `aspect_ratio`؛ النماذج التي تسرد خيارات الدقة مثل `1K` و `2K` أو `4K` تستخدم كلاً من `resolution` و `aspect_ratio`. عائلة Qwen Image — `qwen-image` و `qwen-image-3` و `qwen-image-3-pro` — ترفض `width` و `height` بخطأ `400` وتتطلب `aspect_ratio`.
</Tip>

<Tip>
**مستويات الجودة.** يقبل `gpt-image-2` (التوليد) و `gpt-image-2-edit` (التحرير/التحرير المتعدد) معاملًا اختياريًا `quality` — `low` أو `medium` أو `high` (الافتراضي). المستويات الأقل تكلف أقل؛ راجع [نظرة عامة على التسعير](/ar/overview/pricing#توليد-الصور) للمصفوفة الكاملة أو اجلب `model_spec.pricing.quality` من [endpoint النماذج](/ar/api-reference/endpoint/models/list).
</Tip>