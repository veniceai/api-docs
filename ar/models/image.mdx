---
title: "نماذج الصور"
description: "نماذج الصور في Venice لتوليد الصور من النص وتحريرها وإزالة الخلفية والتكبير، بما في ذلك نماذج Flux 2 وGPT Image وGrok Imagine وChroma."
...
استخدم قيمة `id` كمعامل `model` في طلبات API. عدد النماذج المتوفرة حاليًا: 66.
...
- **التوليد:** أنشئ الصور من مطالبات نصية
- **ترقية الدقّة:** حسِّن دقة الصورة وجودتها
- **التحرير:** عدّل الصور الموجودة عبر الرسم الداخلي (inpainting)

<Note>
راجع [واجهة Image Generate API](/ar/api-reference/endpoint/image/generate) لتحويل النص إلى صورة، و[واجهة Upscale API](/ar/api-reference/endpoint/image/upscale) لتحسين الدقة، و[واجهة Edit API](/ar/api-reference/endpoint/image/edit) للرسم الداخلي.
</Note>

<Tip>
تحديد أبعاد توليد الصور خاص بكل نموذج. النماذج القائمة على البكسل مثل `venice-sd35` تستخدم `width` و`height`؛ ونماذج نسبة العرض إلى الارتفاع مثل `qwen-image-2` تستخدم `aspect_ratio`؛ والنماذج التي تسرد خيارات دقة مثل `1K` أو `2K` أو `4K` تستخدم كلًا من `resolution` و`aspect_ratio`. تعائلة Qwen Image — `qwen-image` و`qwen-image-3` و`qwen-image-3-pro` — ترفض `width` و`height` بخطأ `400` وتتطلب `aspect_ratio`.
</Tip>

<Tip>
**مستويات الجودة.** يقبل `gpt-image-2` (توليد) و`gpt-image-2-edit` (تحرير/تحرير متعدد) معاملًا اختياريًا `quality` — `low` أو `medium` أو `high` (الافتراضي). المستويات الأدنى أرخص؛ راجع [نظرة عامة على التسعير](/ar/overview/pricing#image-generation) للحصول على المصفوفة الكاملة أو اجلب `model_spec.pricing.quality` من [نقطة نهاية Models](/ar/api-reference/endpoint/models/list).
</Tip>
