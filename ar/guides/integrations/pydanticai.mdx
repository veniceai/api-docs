...
<Note>
استخدم `OpenAIChatModel` (Chat Completions)، وليس `OpenAIResponsesModel` / اختصار `openai:`. لا توفر Venice حاليًا واجهة Responses API الخاصة بـ OpenAI؛ مسار إنشاء النصوص هو `/chat/completions`. تثبيت Chat Completions يتفادى سلوكيات خاصة بـ Responses تستخدمها نماذج OpenAI الافتراضية في إصدارات PydanticAI الأحدث.
</Note>
