---
title: Venice CLI
description: "واجهة سطر أوامر Venice الرسمية للدردشة والبحث على الويب وتوليد الصور وتحويل النص إلى كلام والنسخ الصوتي والفيديو من طرفيتك، مع E2EE اختياري."
...
### استكشاف الأخطاء وإصلاحها: الأمر `venice` غير موجود

إذا أبلغتك الطرفية بأن `venice` غير معروف بعد تشغيل `npm install -g veniceai-cli`، فهذا يعني أن مجلد الملفات التنفيذية العام لـ npm غير موجود في `PATH`. هذا شائع على Windows.

1. شغّل `npm config get prefix` لمعرفة مجلد التثبيت العام لـ npm.
2. أضف مجلد الملفات التنفيذية العام إلى `PATH`. على Windows، هو مجلد البادئة نفسه (عادةً `%AppData%\npm`). على macOS وLinux، هو المجلد الفرعي `bin` داخل مجلد البادئة.
3. افتح نافذة طرفية جديدة وشغّل `venice chat "Hello"`.

لتخطي إعداد `PATH`، شغّل CLI عبر `npx` بدلًا من ذلك، مثل `npx veniceai-cli chat "Hello"`.
