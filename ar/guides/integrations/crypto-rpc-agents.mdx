...
<Steps>
  ...
  <Step title="إصدار مفتاح API">
    يُرجع `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` رسالة تحدٍّ EIP-4361 للاستخدام مرة واحدة، مرتبطة بتلك المحفظة، وتنتهي صلاحيتها بعد 15 دقيقة من الإصدار. وقّع الـ `message` كما هي بمحفظة الرهن، ثم `POST` العنوان والتوقيع والرسالة مرة أخرى. يُرجع Venice مفتاح API مرتبطاً بحساب المستخدم المشتق من تلك المحفظة.
  </Step>
</Steps>
