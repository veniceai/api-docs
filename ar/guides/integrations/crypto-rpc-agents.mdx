...
description: "استخدم Venice كمزوّد نماذج وطبقة RPC للبلوكتشين لوكلاء الذكاء الاصطناعي المستقلين: مفتاح API واحد، وأكثر من 230 نموذجًا، وJSON-RPC عبر 11 سلسلة."
...
|---|---|---|
| **مفتاح API** | الوكلاء من جانب الخادم، عمليات النشر الثابتة | رأس `Authorization: Bearer <key>`. احصل على مفتاح من [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). |
...
<Tip>
يمكن للوكلاء المستقلين بحق إصدار مفتاح API الخاص بهم عن طريق رهن VVV على Base. راجع [إنشاء مفتاح API للوكيل المستقل](/guides/integrations/generating-api-key-agent).
</Tip>
...
القائمة أعلاه لقطة ثابتة. استدعِ [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks) — وهو نقطة نهاية عامة لا تتطلب مصادقة — للحصول على القائمة الحية الموثوقة.
...
| **شحن بالدولار أو بالعملات الرقمية عبر لوحة التحكم** | لا (متصفح) | سجل الدخول إلى [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) بنفس المحفظة (Sign-In-With-Ethereum)، ثم أضف الائتمانات في الإعدادات > API. كل من Stripe (بطاقة) وCoinbase (عملات رقمية) يعملان خلف هذه الصفحة ويتطلبان متصفحاً. لا تنتهي صلاحية الائتمانات أبداً. |
...
راجع [إنشاء مفتاح API للوكيل المستقل](/guides/integrations/generating-api-key-agent) للاطلاع على الشرح الكامل مع الكود والمرجع الكامل للأخطاء.
