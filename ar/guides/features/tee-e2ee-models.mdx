...
|------|--------|---------------|
| **TEE** | `e2ee-*` | يعمل النموذج داخل جيب مؤمَّن عتاديًا. لا يستطيع Venice الوصول إلى الحسابات. يمكنك التحقق من ذلك عبر التصديق (attestation). |
| **E2EE** | `e2ee-*` | تشفير كامل من طرف إلى طرف. تُشفَّر مطالباتك على جانب العميل قبل إرسالها. فقط TEE يمكنه فك تشفيرها. |

<Info>
تستخدم معرفات نماذج TEE وE2EE معًا البادئة `e2ee-*`. والسبب أن كل نموذج مُصدَّق بـ TEE على Venice قادر أيضًا على E2EE — فتصديق TEE هو الآلية التي تجعل التشفير من طرف إلى طرف ممكنًا — لذلك تتشارك في عائلة نماذج واحدة وبادئة واحدة. سواء استخدمت الاستدلال عبر TEE فقط أو التدفق الكامل لـ E2EE، فإن معرف النموذج هو نفسه. تشمل نماذج E2EE حماية TEE بالإضافة إلى تشفير من جانب العميل. أما نماذج TEE فتوفر أمان الجيب دون اشتراط تشفير من جانب العميل.
</Info>
