...
<Info>
**معاملات عرض السعر هي مدخلات تسعير فقط.** الحقول التي ترسلها إلى `/video/quote` — بما في ذلك `aspect_ratio` و`audio` و`reference_video_total_duration` — تُستخدم بحتًا لحساب السعر. **لا** تُمرَّر إلى `/video/queue` ولا تؤثر في الفيديو المولَّد. يُقبل إرسال `aspect_ratio` إلى quote حتى بالنسبة للنماذج (مثل `seedance-2-0-image-to-video-api`) التي ترفض `aspect_ratio` وقت التوليد، لأن نماذج الصورة إلى فيديو تشتق نسبة العرض إلى الارتفاع للإخراج من صورة الإدخال.
</Info>
