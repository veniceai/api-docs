...
بالنسبة للنماذج التي أعادت `download_url` وقت الإرسال للطابور، يُعيد retrieve دائمًا JSON. اجلب الفيديو بـ `GET download_url` (بلا رأس المصادقة). راجع [روابط التنزيل الخاصة](#روابط-التنزيل-الخاصة) لمعرفة كيفية عمل هذه الروابط وإعادة المحاولات و`DELETE` الاختياري.
...
| `prompt` | string | نعم | - | ما تريد توليده. الحد الأقصى للطول خاص بكل نموذج ويتفاوت بشكل كبير — من بضع مئات من الأحرف حتى 20,000. النماذج التي لا تعلن عن حدها الخاص تعتمد افتراضيًا 2,500. اقرأ `model_spec.constraints.prompt_character_limit` من `GET /models?type=video` بدلًا من افتراض حد واحد |
...
| `bitrate_mode` | string | لا | `"standard"` | لعائلتَي Seedance 2.0 / 2.5 فقط. `"standard"` أو `"high"`. تطلب `high` ترميزًا بجودة أعلى وملف أكبر. لا يغيّر السعر. احذفه للعائلات الأخرى |
...
4. إذا كان `Content-Type` هو `application/json` و`status` هو `"COMPLETED"`، استدعِ `GET` على `download_url` من استجابة الطابور لجلب الفيديو (راجع [روابط التنزيل الخاصة](#روابط-التنزيل-الخاصة))
...
بخلاف توليد الصور، لا تقبل نقطة نهاية طابور الفيديو معامل `safe_mode`. دعم المحتوى للبالغين يعتمد على النموذج: اختر نسخة نموذج مصمّمة للمخرجات غير الخاضعة للرقابة. على سبيل المثال، تُوفّر عائلة Wan 2.7 نسخ Enhanced إلى جانب النسخ الافتراضية:
