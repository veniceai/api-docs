...
description: "أسعار لكل رمز ولكل طلب لنماذج Venice للنص والصور والصوت والفيديو والتضمينات، إلى جانب تسعير بحث الويب وخيارات الدفع."
...
| ↳ سياق >272K | | $2.50 | $11.25 | $0.25 | $3.13 | | |
...
| ↳ سياق >272K | | $12.50 | $56.25 | $1.25 | $15.63 | | |
...
| ↳ سياق >272K | | $6.25 | $28.13 | $0.63 | $7.81 | | |
...
استخدام الـ API يكون دائمًا بالدفع حسب الاستخدام. لا توجد فئة API مجانية ولا خطة API غير محدودة: كل نموذج في هذه الصفحة يُحاسَب لكل رمز أو لكل صورة أو لكل طلب.

اشتراكات تطبيق Venice (الخطط التي تستخدمها واجهة المحادثة على [venice.ai](https://venice.ai/?utm_source=venice-api-documentation)) تُفوتَر بشكل منفصل عن استخدام الـ API. فئات خططها وأسعارها ومزاياها داخل التطبيق غير مشمولة في توثيق الـ API هذا. أدرها من حساب Venice الخاص بك.

الأرصدة المضمّنة مع الاشتراك، مثل رصيد Pro لمرة واحدة بقيمة 10$، تظهر على حسابك بوصفها **أرصدة مُجمّعة (bundled credits)**. تنفق الـ API أرصدة الحساب بهذا الترتيب: DIEM أولًا، ثم الأرصدة المُجمّعة، ثم USD. راجع [VVV و DIEM](/overview/vvv-diem#how-funding-works) لمعرفة كيفية عمل كل مصدر تمويل وكيفية التحقق من أرصدتك.
