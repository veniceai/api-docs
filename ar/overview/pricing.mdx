...
description: "أسعار Venice API لكل مليون رمز لنماذج النص والصورة والفيديو والصوت والتضمين، إضافةً إلى البحث على الويب وRPC وDIEM."
...
Venice API تعمل بنظام **الدفع حسب الاستخدام**: يُقاس كل طلب ويُحتسب من رصيد اعتمادات Venice في حسابك (المموَّل بالدولار الأمريكي أو عبر DIEM المرهون). لا توجد طبقة مجانية، ولا حصة شهرية للـ API، ولا خطة اشتراك مرتبطة بتسجيل الدخول للـ API — يتم التحقق من الوصول عبر مفتاح API وتُحتسب الفوترة لكل طلب.

<Note>
خطط Free وPro وMax في تطبيق [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) للمستهلك (الدردشة على الويب وسطح المكتب والجوال) منفصلة عن الـ API. تنطبق حدود اشتراكات المستهلك — مثل عدد الصور أو حصص الأحرف أو الدردشة غير المحدودة — على تلك التطبيقات فقط، **وليس** على استخدام الـ API. تُحتسب استدعاءات الـ API دائمًا من رصيد اعتمادات Venice الخاص بك بصرف النظر عن خطتك كمستهلك. راجع [مستخدمو Pro](#مستخدمو-pro) أدناه للاطلاع على الرصيد لمرة واحدة الذي يحصل عليه مشتركو Pro.
</Note>
...
| MiniMax H3 Enhanced (Beta) | `minimax-h3-enhanced-text-to-video` | Text to Video | متغيّر | Anonymized |
| MiniMax H3 R2V | `minimax-h3-reference-to-video` | Text to Video | متغيّر | Anonymized |
| MiniMax H3 R2V Enhanced (Beta) | `minimax-h3-enhanced-reference-to-video` | Text to Video | متغيّر | Anonymized |
...
لا تتضمّن Venice API طبقة مجانية. تُحتسب رسوم كل استدعاء للدردشة والصور والصوت والفيديو والتضمينات عبر الأرصدة أو DIEM أو [x402/USDC](/guides/integrations/x402-venice-api). تبدأ أسعار نماذج النص الأقل تكلفة من $0.05 لكل مليون رمز إدخال، وتبدأ نماذج التضمين من $0.01 لكل مليون رمز.

<Note>
الحصّة اليومية المجانية من الطلبات على [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) هي ميزة في تطبيق الويب الموجَّه للمستهلك وليست في API. تُخصم طلبات API دائمًا من رصيدك المموَّل.
</Note>

### استخدام مفاتيح Venice مع مزوّدين آخرين

تعمل مفاتيح Venice API فقط مع `https://api.venice.ai/api/v1`. وهي ليست قابلة للتبادل مع مفاتيح OpenRouter أو OpenAI أو Anthropic أو غيرهم من المزوّدين، كما أن مفاتيح هؤلاء المزوّدين لن تنجح في المصادقة مع Venice. لاستخدام نماذج Venice، أنشئ مفتاحًا من [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) ووجِّه عميلك إلى عنوان Venice الأساسي.
