---
title: أسعار API
"og:title": "الأسعار | مستندات Venice API"
"og:description": "تعرّف على أسعار Venice API."
---

الأسعار لكل 1M tokens ما لم يُذكر خلاف ذلك. جميع الأسعار بالدولار الأمريكي. 1 Diem = $1/يوم من الحوسبة.
...
| النموذج | المعرّف | سعر الإدخال | سعر الإخراج | قراءة الكاش | كتابة الكاش | السياق | الخصوصية |
|---|---|---|---|---|---|---|---|
...
*الأسعار لكل 1M tokens. [عرض جميع النماذج ←](/ar/models/text)*

### Embeddings

<div id="pricing-embedding-placeholder">

| النموذج | المعرّف | الإدخال (لكل 1M tokens) | الإخراج (لكل 1M tokens) | الخصوصية |
|---|---|---|---|---|
...
| النموذج | المعرّف | لكل 1M حرف | الخصوصية |
|---|---|---|---|
...
<Info>
للحصول على سعر دقيق قبل التوليد، استخدم [Audio Quote API](/ar/api-reference/endpoint/audio/quote). تمتلك النماذج القائمة على المدة مستويات سعرية ثابتة، أما النماذج القائمة على الثانية فتُحسب بناءً على طول الإخراج.
</Info>
...
يختلف سعر الفيديو حسب الدقة والمدة. تفضّل بزيارة [صفحة نماذج الفيديو](/ar/models/video) للحصول على أسعار دقيقة، أو استخدم [Video Quote API](/ar/api-reference/endpoint/video/quote).
...
### البحث على الويب والاستخراج

<div id="pricing-websearch-placeholder">

| الميزة | الإعداد | السعر |
|---|---|---|
| البحث على الويب | `enable_web_search: true` | $10.00 لكل 1K طلب |
| استخراج الويب | `enable_web_scraping: true` | $10.00 لكل 1K رابط |
| بحث X (xAI) | `enable_x_search: true` | $10.00 لكل 1K نتيجة |

</div>

<Info>
**استخراج الويب** يكتشف تلقائيًا حتى 5 روابط لكل طلب، ويستخرج المحتوى ويحوّله إلى Markdown منظَّم، ويضيف النص المستخرج إلى سياق النموذج. لا تُحتسب إلا الروابط التي تم استخراجها بنجاح.

**بحث X** يُفعّل البحث الأصلي من xAI لنماذج Grok المدعومة (مثل `grok-4-20-beta`). يبحث في كلٍّ من الويب وX/Twitter للحصول على معلومات في الوقت الفعلي. يُحاسب لكل نتيجة بحث يُعيدها النموذج (على سبيل المثال، إذا أعاد النموذج 10 نتائج بحث، فستُحاسب على 10 نتائج بسعر $0.01 لكل واحدة = $0.10).

تُضاف هذه الرسوم إلى التسعير القياسي لـ tokens النموذج.
</Info>
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api">
    اشترِ أرصدة API ببطاقة الائتمان. الأرصدة لا تنتهي صلاحيتها.
  </Card>
  <Card title="عملات مشفّرة" icon="bitcoin" href="https://venice.ai/settings/api">
    اشترِ أرصدة API بالعملات المشفّرة. نفس أسعار USD.
  </Card>
  <Card title="راهن على DIEM" icon="coins" href="https://venice.ai/token">
    كل Diem = $1/يوم من الأرصدة التي تتجدد يوميًا.
  </Card>
</CardGroup>

### مستخدمو Pro

يحصل مشتركو Pro على رصيد API لمرة واحدة بقيمة $10 عند الترقية إلى Pro. استخدمه للاختبار وبناء التطبيقات الصغيرة.
