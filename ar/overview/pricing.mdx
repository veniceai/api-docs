...
description: "أسعار Venice API لكل نموذج: تسعير لكل توكن لإكمالات المحادثة، وتسعير لكل عملية توليد للصور والفيديو والصوت، بالدولار الأمريكي وVVV وDIEM."
...
| ↳ سياق >272K | | $5.00 | $18.75 | $0.50 | $6.25 | | |
...
| ↳ سياق >272K | | $5.00 | $22.50 | $0.50 | $6.25 | | |
...
<Info>
للحصول على التسعير الدقيق قبل التوليد، استخدم [Audio Quote API](/ar/api-reference/endpoint/audio/quote). تتمتع النماذج القائمة على المدة بفئات سعر ثابتة، بينما النماذج لكل ثانية تُحاسب بناءً على طول الإخراج.
</Info>
...
يختلف تسعير الفيديو حسب الدقّة والمدة. زر [صفحة نماذج الفيديو](/ar/models/video) للحصول على عروض أسعار دقيقة، أو استخدم [Video Quote API](/ar/api-reference/endpoint/video/quote).
...
### اشتراكات تطبيق Venice.ai مقابل فوترة الـ API

تنطبق خطط اشتراك المستهلكين في Venice.ai (مثل Pro وMax على [venice.ai](https://venice.ai)) على تطبيقات المحادثة والصور والفيديو من Venice على [venice.ai](https://venice.ai). وهي منفصلة عن فوترة الـ API.

* يُحتسب استخدام الـ API دائمًا لكل طلب وفق الأسعار المعروضة أعلاه لكل توكن أو صورة أو ثانية أو عملية توليد. لا يوجد أي طبقة API غير محدودة أو غير مُقاسة.
* مزايا اشتراك التطبيق — رسائل محادثة غير محدودة، وحصص شهرية من الأرصدة لتوليد الفيديو والموسيقى والنماذج الرائدة داخل التطبيق، وإمكانية ترحيل الأرصدة (credit banking) — تنطبق فقط داخل تطبيقات Venice. ولا تمنح وصولًا إلى الـ API ولا تتحول إلى أرصدة API، باستثناء رصيد الـ API لمرة واحدة بقيمة $10 لمشتركي Pro المذكور أعلاه.
* لتمويل طلبات الـ API، اشترِ أرصدة Venice بالدولار أو بالعملات المشفرة من صفحة [إعدادات الـ API](https://venice.ai/settings/api?utm_source=venice-api-documentation)، أو قم بعمل stake لـ DIEM للحصول على حصة يومية (انظر [VVV & DIEM](/overview/vvv-diem)).
