...
<Info>
**أرصدة الـ API مُقوَّمة بالدولار الأمريكي.** 1$ يشتري ما قيمته 1$ من أرصدة الـ API — لا توجد رمز منفصل أو معدّل تحويل أو مضاعف. الأرصدة لا تنتهي صلاحيتها أبدًا.

تُفوتر الـ API **بشكل منفصل** عن اشتراكات المستهلك على venice.ai (Pro و Plus و Max). تغطي تلك الخطط الاستخدام داخل تطبيق venice.ai و**لا** تشمل استخدام الـ API المستمر. راجع قسم [مستخدمو Pro](#مستخدمو-pro) أدناه للاطلاع على رصيد الـ API لمرة واحدة المُضمَّن عند الترقية إلى Pro.
</Info>
...
<Note>
تغطي اشتراكات المستهلك على venice.ai (Pro و Plus و Max) الاستخدام **داخل تطبيق venice.ai** — وهي لا تتضمن مخصّصًا متكرّرًا من أرصدة الـ API. تُحتسب طلبات الـ API من رصيدك بالدولار الأمريكي أو DIEM وفق أسعار كل نموذج المذكورة أعلاه، بغضّ النظر عن خطة المستهلك التي تمتلكها. الرصيد الذي قيمته 10$ لمرة واحدة عند الترقية إلى Pro هو مخصّص الـ API الوحيد المُضمَّن في أي اشتراك.
</Note>
