...
الأرصدة المُجمّعة هي أرصدة API مُلحقة بحساب Venice الخاص بك عبر خطة اشتراك Venice (مثلًا، الرصيد الذي يُمنح لمرة واحدة لمشتركي Pro الجدد). تُنفَق من قِبَل الـ API جنبًا إلى جنب مع أرصدة DIEM و USD، وتظهر في استجابات الفوترة بوصفها العملة `BUNDLED_CREDITS`.

- **ما الذي تغطّيه** — يمكن إنفاق الأرصدة المُجمّعة على أي نقطة نهاية مدفوعة في Venice API (الدردشة، والصور، والصوت، والفيديو، والتضمينات). وتُسعَّر وفق قائمة الأسعار نفسها بالدولار الأمريكي الموجودة في هذه الصفحة.
- **ترتيب الإنفاق** — تسحب مفاتيح API من حسابك بهذا الترتيب: **DIEM ← الأرصدة المُجمّعة ← USD**. لست بحاجة إلى اختيار عملة لكل طلب. راجع [كيف يعمل التمويل](/overview/vvv-diem#how-funding-works).
- **من أين تتحقق من الرصيد** — اطّلع على الأرصدة الحالية على [venice.ai/settings/api](https://venice.ai/settings/api)، أو استدعِ نقطة النهاية [Billing Balance](/api-reference/endpoint/billing/balance) واقرأ إدخال `BUNDLED_CREDITS`.

<Note>
  أي مخصصات رصيد شهرية تظهر في صفحة [خطط Venice للمستهلكين](https://venice.ai) (مثل الأرصدة الخاصة بتطبيق الدردشة على venice.ai أو مولّدات الوسائط) منفصلة عن الأرصدة المُجمّعة الخاصة بالـ API وقد لا تكون قابلة للإنفاق عبر هذا الـ API. لا ينفق Venice API إلا من أرصدة DIEM والأرصدة المُجمّعة و USD التي تُبلِّغ عنها نقطة النهاية [Billing Balance](/api-reference/endpoint/billing/balance).
</Note>
