---
title: مقدمة
description: "مرجع Venice API يغطي المصادقة وتصحيح الأخطاء والتوافق مع OpenAI ورؤوس الاستجابة ومعالجة الأخطاء والقائمة الكاملة لـ endpoints المدعومة."
...
تستخدم Venice API مفاتيح API للمصادقة. أنشئ وأدر مفاتيح API الخاصة بك في [إعدادات API الخاصة بك](https://venice.ai/settings/api?utm_source=venice-api-documentation).


تتطلب معظم طلبات API مصادقة HTTP Bearer. عدد قليل من endpoints الاستكشاف — بما في ذلك `GET /models` و`GET /models/traits` و`GET /models/compatibility_mapping` و`GET /image/styles` — يمكن قراءتها بدون مفتاح:
...
المفاتيح إما `INFERENCE` أو `ADMIN`. يمكن لمفاتيح الاستدلال استدعاء endpoints الاستدلال — الدردشة والصور والفيديو والصوت و embeddings و endpoints التعزيز. أما endpoints إدارة الحساب فتتطلب مفتاح `ADMIN`:

| Endpoint | المفتاح المطلوب |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`، `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | أي منهما |
| `GET /billing/balance`، `/billing/usage-history`، `/billing/usage-analytics` | `ADMIN` |

استدعاء endpoint مخصص للمشرف فقط بمفتاح استدلال يعيد `401` مع `{"error":"Admin API key required"}`. لاحظ أن `GET /api_keys/rate_limits` هو الاستثناء الوحيد في تلك المجموعة — فهو يعمل مع أي من نوعي المفاتيح.
...
<Warning>
عنوان base URL هو `https://api.venice.ai/api/v1` وليس `https://venice.ai`. الطلبات المُرسلة إلى نطاق الموقع تصل إلى تطبيق الويب بدلًا من الـ API وتفشل بأخطاء مثل `405 Method Not Allowed`.
</Warning>
...
| `enable_web_citations` | boolean | عند تفعيل البحث في الويب، اطلب من الـ LLM الاستشهاد بمصادره باستخدام أرقام مرتفعة بعلامة caret — `^1^` لمصدر واحد و`^1,2^` لعدة مصادر. أما الصيغ ذات الأقواس مثل `[1]` و`(1)` و`<sup>1</sup>` فيتم كبتها صراحةً | `false` |
...
6. **مدخلات Embeddings**: يجب أن يكون `input` سلسلة نصية أو مصفوفة من السلاسل النصية. تُرفض مصفوفات معرّفات الـ tokens بـ HTTP 400. يجب على `OpenAIEmbeddings` في LangChain ضبط `check_embedding_ctx_length=False`
