...
عند حدوث خطأ، تُعيد Venice API رمز حالة HTTP ورسالة وصفية. تتضمن بعض الأخطاء أيضًا حقولًا إضافية، مثل حالة الاسترداد، والنماذج المقترحة لإعادة المحاولة، وتفاصيل التحقق، أو معرفات الطلب.

## تنسيق استجابة الخطأ

تُعيد معظم الأخطاء حقل `error` برسالة قابلة للقراءة:
...
تتضمن بعض نقاط النهاية حقولًا إضافية خاصة بنقطة النهاية. على سبيل المثال، يمكن أن تتضمن أخطاء الإشراف على الفيديو ما إذا تم استرداد الأرصدة:
...
قد تتضمن أخطاء التحقق حقلَي `details` و `issues` مع معلومات تحقق على مستوى الحقل:
...
قد تُعيد بعض نقاط النهاية المتوافقة مع OpenAI كائنات أخطاء بأسلوب OpenAI لحالات معينة، مثل أخطاء طول السياق:
