---
title: "Image Models"
description: "Venice image models for generation, editing, background removal, and upscaling, including Flux 2, GPT Image, Grok Imagine, and Chroma."
...
Use the `id` value as the `model` parameter in API requests. 66 models currently available.
...
<Tip>
**Quality tiers.** `gpt-image-2` (generate) and `gpt-image-2-edit` (edit/multi-edit) accept an optional `quality` parameter — `low`, `medium`, or `high` (default). Lower tiers cost less; see the [Pricing overview](/overview/pricing#image-generation) for the full matrix or fetch `model_spec.pricing.quality` from the [Models endpoint](/api-reference/endpoint/models/list).
</Tip>