...
<Tip>
Image generation sizing is model-specific. Models that list resolution options such as `1K`, `2K`, or `4K` should be called with `resolution` and `aspect_ratio`; examples include `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro`. Models without resolution tiers generally use `width` and `height`.
</Tip>