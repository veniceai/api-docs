...
<Note>
`gpt-image-2` and `gpt-image-2-edit` use token-aware dynamic pricing based on quality, resolution, prompt length, and for edit requests, billable input images. The Models API exposes details under `model_spec.pricing.gpt_image_tokens`.
</Note>
