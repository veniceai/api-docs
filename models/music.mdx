...
description: "Venice music and audio models for songs, instrumental tracks, and sound effects, including ElevenLabs Music, Lyria 3 Pro, and MiniMax Music."
...
Use the `id` value as the `model` parameter in API requests. 16 models currently available.
...
<Note>
Audio generation uses an async queue system. Follow the [Music & Sound Effects guide](/guides/media/music-and-sound-effects) for a complete walkthrough, or see the [Audio Queue API](/api-reference/endpoint/audio/queue) to start generation and [Audio Retrieve API](/api-reference/endpoint/audio/retrieve) to fetch results. Models with `model_spec.voice_changer` set to `true` use the [Voice Changer](/guides/media/voice-changer) endpoints instead.
</Note>
