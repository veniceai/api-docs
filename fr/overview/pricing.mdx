...
description: "Tarifs de l'API Venice par million de tokens pour les modèles texte, image, vidéo, audio et embeddings, plus crédits pour la recherche web, les appels RPC et autres services."
...
| Hunyuan Image 3.0 (Beta) | `hunyuan-image-v3` | Par image : $0.09 | Private |
| Krea v2 Large (Beta) | `krea-v2-large` | Par image : $0.07 | Anonymized |
| Krea v2 Medium (Beta) | `krea-v2-medium` | Par image : $0.04 | Anonymized |
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api">
    Achetez des crédits API par carte de crédit. Les crédits n'expirent jamais.
  </Card>
  <Card title="Crypto" icon="bitcoin" href="https://venice.ai/settings/api">
    Achetez des crédits API en cryptomonnaie. Mêmes tarifs qu'en USD.
  </Card>
...
### Existe-t-il un palier API gratuit ?

Il n'existe aucun modèle à coût nul sur l'API Venice — chaque modèle a un prix par token, par image, par seconde ou par requête. Les quotas quotidiens d'images et de messages de l'abonnement Venice.ai Pro s'appliquent uniquement à l'application web et à l'application mobile, pas aux appels API.

Les deux façons d'utiliser l'API sans frais par requête en USD sont :

- **Staker du VVV pour obtenir des crédits DIEM quotidiens.** Les stakers de VVV reçoivent une allocation quotidienne de DIEM proportionnelle à leur part du pool de staking. Chaque 1 DIEM = 1 $/jour de crédits d'inférence qui se rafraîchissent à 00:00 UTC. Voir la section [Gagnez des crédits quotidiens en stakant du VVV](#gagnez-des-credits-quotidiens-en-stakant-du-vvv) ci-dessous.
- **Crédit d'abonnement Pro.** Les abonnés Pro reçoivent un crédit API unique de 10 $ lors de leur passage à Pro. Il est destiné aux tests et aux petites applications, pas à un usage gratuit durable.

La transcription vocale avec `stt-xai-v1` est affichée à 0,0000 $ par seconde audio, mais il s'agit d'un artefact d'arrondi — l'utilisation est toujours décomptée de votre solde et facturée par le fournisseur en amont.

### Gagnez des crédits quotidiens en stakant du VVV

Staker du VVV sur Base donne à votre wallet une allocation quotidienne de DIEM qui est dépensée automatiquement avant tout solde en USD.

1. **Obtenez du VVV sur Base.** Achetez ou échangez pour obtenir du VVV (contrat `0xacfE6019Ed1A7Dc6f7B508C02d1b04ec88cC21bf`) sur un DEX tel qu'Aerodrome ou Uniswap. Vous avez également besoin d'une petite quantité d'ETH sur Base pour le gas.
2. **Stakez sur [venice.ai/token](https://venice.ai/token).** Le bouton de staking se trouve sur cette page. En coulisses, il s'agit de deux transactions on-chain sur le [contrat de staking Venice](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff) : `approve` sur le token VVV, puis `stake` sur le contrat de staking. Votre solde VVV diminue et votre solde sVVV augmente du même montant.
3. **Gagnez du DIEM chaque jour.** L'allocation quotidienne de DIEM de votre wallet est proportionnelle à sa part du pool de staking et se rafraîchit à chaque époque à 00:00 UTC. Votre compte doit avoir accumulé au moins **0,1 DIEM** avant que le DIEM ne devienne dépensable.
4. **Dépensez sur l'API.** Lorsque vous appelez l'API, la clé émise dépense d'abord depuis le DIEM, puis depuis les crédits groupés, puis depuis l'USD. Aucun frais par requête n'est appliqué tant que le DIEM couvre la dépense.

Pour les agents qui émettent leur propre clé API depuis un wallet de staking sans intervention humaine, voir [Création autonome de clé API par un agent](/guides/getting-started/generating-api-key-agent).

### Utilisateurs Pro

Les abonnés Pro reçoivent un crédit API unique de 10 $ lors du passage à Pro. Utilisez-le pour tester et créer de petites applications. Les quotas quotidiens de messages et d'images de l'application Venice.ai sont distincts de ce crédit et ne s'appliquent pas aux requêtes API.

## Modèles non censurés et permissifs

La politique de contenu de Venice est plus permissive que celle de la plupart des fournisseurs en amont, et plusieurs modèles du catalogue ci-dessus sont livrés sans la couche de sécurité du fournisseur en amont. Vous pouvez les repérer par leur ID ou leur nom d'affichage :

- **`*-uncensored*`** — checkpoints ajustés ou sélectionnés par Venice qui contournent l'entraînement de refus par défaut du fournisseur. Exemples : `venice-uncensored-1-2`, `gemma-4-uncensored`, `qwen-edit-uncensored`, `wan-2-7-uncensored-image-to-video`.
- **`*-heretic*`** — checkpoints ajustés par la communauté (par ex. `olafangensan-glm-4.7-flash-heretic`) qui passent par un fine-tune de décensure avant d'être proposés sur Venice. Les poids de base proviennent du fournisseur en amont mentionné dans l'ID du modèle ; le suffixe `heretic` identifie le fine-tune, pas le modèle de base.
- **Variantes role-play** — par ex. `venice-uncensored-role-play`, ajustées pour l'écriture longue de personnages et l'écriture créative.

Les modèles non « uncensored » sur Venice renvoient toujours `CONTENT_POLICY_VIOLATION` (HTTP 422) pour les prompts que le fournisseur en amont refuse. Voir [Codes d'erreur](/api-reference/error-codes) pour la liste complète. Si vous souhaitez le comportement le plus permissif, choisissez une variante `uncensored` ou `heretic` et désactivez le prompt système par défaut de Venice avec `venice_parameters.include_venice_system_prompt: false` — voir [Spécification de l'API](/api-reference/api-spec) pour la référence du paramètre.
