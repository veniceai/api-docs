---
title: Tarification de l'API Venice
description: "Tarifs de l'API Venice par 1M de tokens pour les modèles texte, image, vidéo, audio et embeddings, plus la recherche web, RPC et DIEM."
...
L'API Venice fonctionne **à l'usage** : chaque requête est mesurée et facturée sur le solde de crédits Venice de votre compte (approvisionné en USD ou via des DIEM stakés). Il n'y a pas d'offre gratuite, pas de quota API mensuel, ni de plan d'abonnement lié à une connexion pour l'API — l'accès est authentifié par une clé API et facturé à la requête.

<Note>
Les plans Free, Pro et Max de l'application consommateur [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) (chat web, desktop et mobile) sont distincts de l'API. Les limites des abonnements consommateurs — telles que les nombres d'images, les quotas de caractères ou le chat illimité — s'appliquent uniquement à ces applications, **et non** à l'utilisation de l'API. Les appels API sont toujours décomptés de votre solde de crédits Venice, quel que soit votre plan consommateur. Voir [Utilisateurs Pro](#utilisateurs-pro) ci-dessous pour le crédit unique dont bénéficient les abonnés Pro.
</Note>
...
| MiniMax H3 Enhanced (Beta) | `minimax-h3-enhanced-text-to-video` | Text to Video | Variable | Anonymized |
| MiniMax H3 R2V | `minimax-h3-reference-to-video` | Text to Video | Variable | Anonymized |
| MiniMax H3 R2V Enhanced (Beta) | `minimax-h3-enhanced-reference-to-video` | Text to Video | Variable | Anonymized |
...
L'API Venice ne propose pas d'offre gratuite. Chaque appel de chat, d'image, d'audio, de vidéo et d'embedding est payé via des crédits, des DIEM ou [x402/USDC](/guides/integrations/x402-venice-api). Les modèles de texte les moins chers démarrent à $0.05 par 1M de tokens en entrée, et les modèles d'embedding démarrent à $0.01 par 1M de tokens.

<Note>
L'allocation gratuite de prompts quotidiens sur [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) est une fonctionnalité de l'application web grand public, et non de l'API. Les requêtes API sont toujours débitées de votre solde approvisionné.
</Note>

### Utiliser des clés Venice avec d'autres fournisseurs

Les clés d'API Venice ne fonctionnent qu'avec `https://api.venice.ai/api/v1`. Elles ne sont pas interchangeables avec les clés d'OpenRouter, OpenAI, Anthropic ou d'autres fournisseurs, et les clés de ces fournisseurs ne s'authentifieront pas auprès de Venice. Pour utiliser les modèles Venice, générez une clé sur [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) et configurez votre client pour pointer vers l'URL de base de Venice.
