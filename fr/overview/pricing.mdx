...
<Info>
**Les crédits API sont libellés en USD.** 1 $ USD achète 1 $ de crédits API — il n'existe ni token distinct, ni multiplicateur, ni taux de conversion. Les crédits n'expirent jamais.

La facturation de l'API est **distincte** des abonnements grand public venice.ai (Pro, Plus, Max). Ces formules couvrent l'utilisation dans l'application venice.ai et **n'incluent pas** l'utilisation continue de l'API. Voir [Utilisateurs Pro](#utilisateurs-pro) ci-dessous pour le crédit API unique inclus avec le passage à Pro.
</Info>
...
<Note>
Les abonnements grand public venice.ai (Pro, Plus, Max) couvrent l'utilisation **au sein de l'application venice.ai** — ils n'incluent pas d'allocation récurrente de crédits API. Les requêtes API sont facturées sur votre solde en USD ou DIEM aux tarifs par modèle indiqués ci-dessus, quel que soit votre abonnement grand public. Le crédit unique de $10 lors du passage à Pro est la seule allocation API incluse dans un abonnement.
</Note>
