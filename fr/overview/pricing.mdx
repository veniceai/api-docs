...
Les crédits groupés sont des crédits API rattachés à votre compte Venice par un plan d'abonnement Venice (par exemple, le crédit unique accordé aux nouveaux abonnés Pro). Ils sont dépensés par l'API aux côtés des crédits DIEM et USD et apparaissent dans les réponses de facturation sous la devise `BUNDLED_CREDITS`.

- **Ce qu'ils couvrent** — les crédits groupés peuvent être dépensés sur tout point de terminaison payant de l'API Venice (chat, images, audio, vidéo, embeddings). Ils sont facturés selon la même grille tarifaire en USD que celle de cette page.
- **Ordre de dépense** — les clés API puisent dans votre compte dans cet ordre : **DIEM → crédits groupés → USD**. Vous n'avez pas besoin de choisir une devise par requête. Voir [Fonctionnement du financement](/overview/vvv-diem#how-funding-works).
- **Où consulter le solde** — consultez les soldes actuels sur [venice.ai/settings/api](https://venice.ai/settings/api), ou appelez le point de terminaison [Billing Balance](/api-reference/endpoint/billing/balance) et lisez l'entrée `BUNDLED_CREDITS`.

<Note>
  Toute allocation mensuelle de crédits affichée sur la page des [plans grand public Venice](https://venice.ai) (par exemple, les crédits pour l'application de chat venice.ai ou les générateurs de médias) est distincte des crédits API groupés et peut ne pas être dépensable via cette API. L'API Venice ne dépense que les soldes DIEM, groupés et USD rapportés par le point de terminaison [Billing Balance](/api-reference/endpoint/billing/balance).
</Note>
