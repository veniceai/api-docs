...
"og:title": "Tarification | Documentation Venice API"
"og:description": "Découvrez la tarification de l'API de Venice."
---

Prix par 1M de tokens sauf indication contraire. Tous les prix sont en USD. 1 Diem = 1 $/jour de calcul.
...
| Modèle | ID | Prix input | Prix output | Cache Read | Cache Write | Contexte | Confidentialité |
|---|---|---|---|---|---|---|---|
...
| Modèle | ID | Input (par 1M de tokens) | Output (par 1M de tokens) | Confidentialité |
|---|---|---|---|---|
...
| Modèle | ID | Par 1M de caractères | Confidentialité |
|---|---|---|---|
...
| Modèle | ID | Par seconde d'audio | Confidentialité |
|---|---|---|---|
...
#### Génération de chansons (basée sur la durée)

| Modèle | ID | Tarification par durée | Confidentialité |
|---|---|---|---|
| ACE-Step 1.5 | `ace-step-15` | 60 s : $0.03, 90 s : $0.04, 120 s : $0.05, 150 s : $0.06, 180 s : $0.07, 210 s : $0.08 | Anonymized |
| ElevenLabs Music | `elevenlabs-music` | 60 s : $0.87, 120 s : $1.73, 180 s : $2.59, 240 s : $3.45, 300 s : $4.32, 360 s : $5.18, 420 s : $6.04, 480 s : $6.90, 540 s : $7.77, 600 s : $8.63 | Anonymized |

#### Génération de chansons (par génération)
...
<Info>
Pour des prix exacts avant la génération, utilisez l'[API Audio Quote](/fr/api-reference/endpoint/audio/quote). Les modèles basés sur la durée ont des paliers de prix fixes, tandis que les modèles par seconde facturent en fonction de la durée de sortie.
</Info>
...
La tarification vidéo varie selon la résolution et la durée. Consultez la [page Modèles vidéo](/fr/models/video) pour des devis exacts, ou utilisez l'[API Video Quote](/fr/api-reference/endpoint/video/quote).
...
|---|---|---|
| Recherche web | `enable_web_search: true` | $10.00 par 1K requêtes |
| Web scraping | `enable_web_scraping: true` | $10.00 par 1K URL |
| X Search (xAI) | `enable_x_search: true` | $10.00 par 1K résultats |
...
**X Search** active la recherche native de xAI pour les modèles Grok pris en charge (par ex. `grok-4-20-beta`). Cela recherche à la fois sur le web et sur X/Twitter pour des informations en temps réel. Facturé par résultat de recherche renvoyé par le modèle (par ex. si le modèle renvoie 10 résultats de recherche, vous êtes facturé pour 10 résultats à $0.01 chacun = $0.10).
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api">
    Achetez des crédits API par carte bancaire. Les crédits n'expirent jamais.
  </Card>
  <Card title="Crypto" icon="bitcoin" href="https://venice.ai/settings/api">
    Achetez des crédits API en crypto-monnaie. Mêmes tarifs qu'en USD.
  </Card>
  <Card title="Stake DIEM" icon="coins" href="https://venice.ai/token">
    Chaque Diem = 1 $/jour de crédits qui se rechargent quotidiennement.
  </Card>
</CardGroup>

### Utilisateurs Pro

Les abonnés Pro reçoivent un crédit API unique de 10 $ lors du passage à Pro. Utilisez-le pour tester et construire de petites applications.
