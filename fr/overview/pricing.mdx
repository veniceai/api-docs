...
<Note>
L'allocation gratuite de prompts quotidiens sur [venice.ai](https://venice.ai) est une fonctionnalité de l'application web grand public, et non de l'API. Les requêtes API sont toujours débitées de votre solde approvisionné.
</Note>

### Utiliser des clés Venice avec d'autres fournisseurs

Les clés d'API Venice ne fonctionnent qu'avec `https://api.venice.ai/api/v1`. Elles ne sont pas interchangeables avec les clés d'OpenRouter, OpenAI, Anthropic ou d'autres fournisseurs, et les clés de ces fournisseurs ne s'authentifieront pas auprès de Venice. Pour utiliser les modèles Venice, générez une clé sur [venice.ai/settings/api](https://venice.ai/settings/api) et configurez votre client pour pointer vers l'URL de base de Venice.
