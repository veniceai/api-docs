---
title: Venice API
description: "Documentation de l'API Venice — accès privé et sans restriction à des modèles de chat, d'image, d'audio et de vidéo compatibles OpenAI avec une seule clé d'API."
...
<Accordion title="Exemples de code de recherche web">
Ajoutez une recherche web en temps réel avec citations à n'importe quel modèle de texte via `enable_web_search`.
...
<Accordion title="Exemples de code de Web Scraping">
Définissez `enable_web_scraping: true` et le modèle récupérera et lira toutes les URL du message utilisateur avant de répondre.
...
Attachez des PDF, des documents Office, du code et des fichiers texte (jusqu'à 25 Mo) directement à une requête de chat. Voir le [guide Entrées de fichiers](/guides/features/file-inputs) pour la liste complète des formats.

<CodeGroup>
```bash Curl
# Encoder un fichier local en URL de données base64, puis l'envoyer en ligne
...
Transmettez des appels JSON-RPC 2.0 sur 11 chaînes prises en charge avec votre clé Venice ou un portefeuille x402. Voir la [référence Crypto RPC](/api-reference/endpoint/crypto/rpc) pour les chaînes, méthodes et niveaux de crédit.
