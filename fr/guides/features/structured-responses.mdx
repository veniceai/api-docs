...
description: "Forcez les modèles de chat Venice à renvoyer du JSON conforme à votre schéma via response_format et JSON Schema pour des sorties fiables."
'og:title': "Réponses structurées | Venice API Docs"
'og:description': "Utiliser les réponses structurées dans l'API Venice"
---

Venice utilise Chat Completions pour la génération de texte et n'expose actuellement pas l'API Responses d'OpenAI. Les réponses structurées utilisent `response_format` sur `POST /chat/completions`.
