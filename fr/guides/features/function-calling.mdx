...
description: "Laissez les modèles de chat Venice appeler vos outils et fonctions via l'appel de fonctions compatible OpenAI, les schémas d'outils et l'API /chat/completions."
...
L'exemple ci-dessus s'arrête après un aller-retour. Les vrais agents continuent, car le modèle a généralement besoin de plusieurs outils avant de pouvoir répondre. [Construire un agent qui utilise des outils avec l'appel de fonctions](/guides/features/tool-using-agent) parcourt la boucle complète face à une base de données que le modèle n'a jamais vue, y compris les appels d'outils parallèles, le retour d'erreurs dont le modèle se remet, et la différence entre un modèle qui décline et un garde-fou qui refuse.
...
- [Construire un agent qui utilise des outils avec l'appel de fonctions](/guides/features/tool-using-agent)
