---
title: "Embeddings"
description: "Générez des embeddings vectoriels avec l'API Venice via /embeddings, compatible OpenAI, pour la recherche sémantique, le RAG, le clustering et la similarité."
...
<Warning>
Venice n'accepte que du texte. Envoyez une chaîne de caractères ou un tableau de chaînes. Les tableaux d'ID de tokens sont rejetés avec HTTP 400. `OpenAIEmbeddings` de LangChain doit définir `check_embedding_ctx_length=False`, sinon il tokenise localement avec tiktoken et envoie des tableaux d'entiers.
</Warning>
...
Passez un tableau de chaînes pour embarquer plusieurs textes en une seule requête. Chaque élément doit être du texte, pas un tableau d'ID de tokens :

Deux limites s'appliquent, et toutes deux sont vérifiées avant que la requête n'atteigne un modèle :

| Limite | Valeur | Erreur en cas de dépassement |
|---|---|---|
| Éléments par requête | 2 048 | `400` — *"Array must contain at most 2048 element(s)"* |
| Tokens par élément | 8 192 | `400` — le nombre de tokens est vérifié par entrée du tableau, pas sur l'ensemble du lot |

Dimensionnez vos lots d'ingestion en fonction du plafond de 2 048 et découpez les documents longs en dessous de 8 192 tokens par entrée. Le corps de la requête est également strict : un champ non reconnu renvoie un `400` au lieu d'être ignoré.
...
Pour une implémentation complète, consultez [Créer un bot RAG privé](/learn/private-rag-bot).
...
- [Guide du bot RAG privé](/learn/private-rag-bot)
