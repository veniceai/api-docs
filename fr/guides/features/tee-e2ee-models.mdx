---
title: "Modèles TEE et E2EE"
description: "IA à confidentialité renforcée avec Trusted Execution Environments et chiffrement de bout en bout"
'og:title': "Modèles TEE et E2EE | Documentation Venice API"
'og:description': "Apprenez à utiliser les modèles TEE et E2EE pour une confidentialité maximale dans vos applications IA"
---

Venice propose des modèles à confidentialité renforcée qui s'exécutent dans des Trusted Execution Environments (TEE) et prennent en charge le chiffrement de bout en bout (E2EE). Ces modèles offrent des garanties cryptographiques que vos données restent privées — même de Venice.

## Comprendre les niveaux de confidentialité

| Type | Préfixe | Ce que cela signifie |
|------|--------|---------------|
| **TEE** | `tee-*` | Le modèle s'exécute dans une enclave sécurisée matériellement. Venice ne peut pas accéder au calcul. Vous pouvez le vérifier avec l'attestation. |
| **E2EE** | `e2ee-*` | Chiffrement de bout en bout complet. Vos prompts sont chiffrés côté client avant d'être envoyés. Seul le TEE peut les déchiffrer. |

<Info>
Les modèles E2EE incluent la protection TEE plus un chiffrement côté client. Les modèles TEE fournissent la sécurité de l'enclave sans nécessiter de chiffrement côté client.
</Info>
...
Consultez la [page Modèles](/fr/overview/models) pour la liste complète avec tarifs et limites de contexte.

## Modèles TEE

Les modèles TEE s'exécutent à l'intérieur d'enclaves sécurisées matériellement (Intel TDX, NVIDIA Confidential Computing). Les poids du modèle et vos données sont protégés du système hôte — y compris de l'infrastructure de Venice.

### Utilisation de base

Les modèles TEE fonctionnent exactement comme des modèles ordinaires :
...
| `intel_quote` | Quote Intel TDX brute (base64) pour vérification côté client |
| `nvidia_payload` | Données d'attestation GPU NVIDIA (si applicable) |
...
<Tip>
Pour un usage en production, vérifiez l'attestation côté client en parsant la quote Intel TDX et en vérifiant l'attestation NVIDIA.
</Tip>

<Note>
Pour la vérification d'un modèle TEE simple, `signing_address` et les champs de vérification côté serveur sont suffisants pour les contrôles d'attestation de base. Une `signing_key` est requise lorsque vous avez besoin d'un accord de clé E2EE côté client et de contrôles stricts de liaison de clé.
</Note>
...
<Warning>
E2EE nécessite une implémentation côté client. Les exemples ci-dessous montrent le protocole complet.
</Warning>

### Comment fonctionne E2EE

<Steps>
  <Step title="Générer une paire de clés éphémère">
    Le client génère une paire de clés secp256k1 pour cette session.
  </Step>
  <Step title="Récupérer l'attestation TEE">
    Le client demande `/api/v1/tee/attestation` et reçoit la clé publique du modèle, les preuves d'attestation et le nonce.
  </Step>
  <Step title="Vérifier l'attestation">
    Le client vérifie la correspondance du nonce, que le mode debug est désactivé et la validité de l'attestation.
  </Step>
  ...
  <Step title="Traitement TEE">
    Le TEE déchiffre la requête, la traite et chiffre la réponse.
  </Step>
  <Step title="Déchiffrer la réponse">
    Le client reçoit les chunks chiffrés et les déchiffre avec la clé privée.
  </Step>
</Steps>
...
### Étape 1 : vérifier la prise en charge E2EE du modèle

Vérifiez d'abord que le modèle prend en charge E2EE en consultant l'endpoint `/models`.
...
### Étape 2 : générer une paire de clés éphémère

Générez une nouvelle paire de clés pour chaque session. La clé privée doit rester en mémoire uniquement et être effacée en toute sécurité après utilisation.
...
Utilisez ces fonctions auxiliaires pour valider les clés et le contenu chiffré avant d'envoyer les requêtes.
...
<Info>
**Important : longueur du nonce** — le nonce client doit faire **32 octets (64 caractères hex)**. Certains fournisseurs TEE exigent exactement 32 octets et rejetteront les nonces plus courts.
</Info>
...
Chiffrez les messages utilisateur et système avant l'envoi. Seuls les messages de rôle `user` et `system` doivent être chiffrés.

<Warning>
Lorsque les en-têtes E2EE sont présents, **tous** les messages de rôle `user` et `system` doivent être chiffrés. Envoyer du contenu en clair dans ces rôles entraînera une erreur "Encrypted field is not valid hex".
</Warning>
...
Les réponses des modèles E2EE sont des chunks chiffrés encodés en hex. Déchiffrez chaque chunk en utilisant votre clé privée.
...
<Warning>
E2EE présente certaines contraintes en raison des exigences de chiffrement :
</Warning>

| Fonctionnalité | Statut |
|---------|--------|
| Streaming | **Requis** (non-streaming non pris en charge) |
| Recherche web | **Désactivée** (cela divulguerait le contenu) |
| Téléversement de fichiers | **Non pris en charge** |
| Function calling | **Non pris en charge** |
| System prompt Venice | **Désactivé** (doit être chiffré côté client) |

### Bonnes pratiques de sécurité

1. **Générer de nouvelles paires de clés par session** — ne réutilisez pas les clés éphémères
2. **Effacer les clés privées** — videz les bytes de clé privée de la mémoire une fois terminé
3. **Vérifier l'attestation** — vérifiez toujours `verified: true` et la correspondance du nonce
4. **Vérifier le mode debug** — rejetez les attestations des enclaves debug
5. **Utiliser le streaming** — E2EE nécessite le streaming pour un découpage correct du chiffrement
6. **Gérer les erreurs avec élégance** — n'exposez pas les erreurs de déchiffrement aux utilisateurs
7. **Utiliser des nonces de 32 octets** — les fournisseurs TEE exigent exactement 32 octets

## Bonnes pratiques

<AccordionGroup>
  <Accordion title="Vérifiez toujours l'attestation en production">
    Ne vous contentez pas de faire confiance à la réponse `verified: true`. Parsez la quote Intel TDX côté client et vérifiez que les mesures correspondent aux valeurs attendues. Pour les GPU NVIDIA, vérifiez l'attestation via le service de vérification de NVIDIA.
  </Accordion>
  ...
  <Accordion title="Vérifiez la liaison de clé">
    La clé de signature doit être liée au champ TDX REPORTDATA. Cela prouve que la clé a été générée à l'intérieur de l'enclave.
  </Accordion>
  
  <Accordion title="Vérifiez le mode debug">
    Vérifiez que l'attestation TDX n'a pas les flags debug activés. Une enclave debug peut être inspectée et ne doit pas être utilisée en production.
  </Accordion>
  
  <Accordion title="Utilisez nos SDK pour E2EE">
    E2EE nécessite une implémentation cryptographique soigneuse. Utilisez nos SDK officiels plutôt que d'implémenter le protocole vous-même.
  </Accordion>
</AccordionGroup>
...
| `Attestation nonce mismatch` | Possible attaque par rejeu | Générez un nouveau nonce |
| `TDX debug mode detected` | L'enclave est en mode debug | Ne l'utilisez pas en production |
| `Failed to decrypt field` | Échec du déchiffrement E2EE côté serveur | Vérifiez votre implémentation de chiffrement |
| `E2EE requires streaming` | Requête non-streaming vers un modèle E2EE | Définissez `stream: true` |
...
<AccordionGroup>
  <Accordion title="502 Bad Gateway ou 'Nonce must be exactly 32 bytes'">
    La longueur du nonce est incorrecte. Les fournisseurs TEE exigent exactement **32 octets (64 caractères hex)**.
  ...
  <Accordion title="Échec de la vérification de l'attestation">
    - Vérifiez que le modèle prend en charge E2EE (`supportsE2EE: true`)
    - Vérifiez que votre clé API est valide et a accès au modèle demandé
    - Vérifiez la connectivité réseau à l'API Venice
  </Accordion>
  
  <Accordion title="Échec du déchiffrement">
    - Assurez-vous d'utiliser la même clé privée que celle qui a généré la clé publique envoyée dans les en-têtes
    - Vérifiez que le contenu de la réponse est effectivement encodé en hex (E2EE actif)
  ...
  <Accordion title="Encrypted field is not valid hex">
    - Tous les messages de rôle `user` et `system` doivent être chiffrés lorsque des en-têtes E2EE sont présents
    - Vérifiez que votre contenu chiffré passe la validation `isValidEncrypted()` (minimum 186 caractères hex)
    - Vérifiez que la sortie de chiffrement est en hex minuscules sans préfixe
  </Accordion>
  
  <Accordion title="Erreurs Invalid public key">
    - La clé publique du client doit faire exactement **130 caractères hex** commençant par `04`
    - Utilisez le helper `validateClientPubkey()` pour vérifier le format avant l'envoi
    - Assurez-vous d'utiliser le format de clé publique non compressé (65 octets = 130 caractères hex)
  </Accordion>
  
  <Accordion title="Modèle introuvable">
    - Vérifiez que l'ID du modèle est correct et que le modèle prend en charge E2EE
