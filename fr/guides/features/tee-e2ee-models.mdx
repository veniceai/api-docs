...
description: "Exécutez une inférence privée sur Venice avec les modèles TEE et le chiffrement de bout en bout qui rendent vos prompts illisibles pour l'hôte."
...
|------|--------|---------------|
| **TEE** | `e2ee-*` | Le modèle s'exécute dans une enclave sécurisée matériellement. Venice ne peut pas accéder au calcul. Vous pouvez le vérifier par attestation. |
...
<Info>
Les identifiants des modèles TEE et E2EE utilisent tous deux le préfixe `e2ee-*`. En effet, tout modèle avec attestation TEE sur Venice est également compatible E2EE — l'attestation TEE est le mécanisme qui rend possible le chiffrement de bout en bout —, ils partagent donc une même famille de modèles et un même préfixe. Que vous utilisiez l'inférence TEE simple ou le flux E2EE complet, l'identifiant du modèle est le même. Les modèles E2EE incluent la protection TEE plus le chiffrement côté client. Les modèles TEE fournissent la sécurité de l'enclave sans nécessiter de chiffrement côté client.
</Info>
...
Consultez la [page Modèles](/overview/models) pour la liste complète avec les prix et les limites de contexte.
