...
description: "Générez des images à partir de prompts textuels avec l'API Venice, couvrant les options de dimensionnement des modèles, les presets de style, la force des LoRA, le mode sûr et la sortie binaire."
...
<Warning>
`qwen-image`, `qwen-image-3` et `qwen-image-3-pro` n'acceptent plus `width` ni `height`. L'envoi de l'un ou l'autre renvoie `400` avec *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."* Vérifiez `model_spec.constraints` depuis `GET /models?type=image` avant de choisir les champs de dimensionnement.
</Warning>
...
Les noms de styles se trouvent dans `data`, pas au niveau supérieur — lisez `response.data` plutôt que d'itérer directement sur l'objet de réponse.
...
| `width` | integer | Non | `1024` | Largeur de sortie en pixels pour les modèles basés sur les pixels tels que `venice-sd35`. Rejeté par la famille Qwen Image — voir la note ci-dessous |
| `height` | integer | Non | `1024` | Hauteur de sortie en pixels pour les modèles basés sur les pixels tels que `venice-sd35`. Rejeté par la famille Qwen Image — voir la note ci-dessous |
...
| `lora_strength` | integer | Non | - | Force (`0`-`100`) des LoRA intégrés au modèle ; ignoré par les modèles sans LoRA additionnels |
...
Certains modèles d'image sont livrés avec des LoRA additionnels intégrés. Le paramètre `lora_strength` (entier, `0` à `100`) contrôle l'intensité avec laquelle ces LoRA intégrés sont appliqués. Il n'affecte que les modèles qui utilisent des LoRA additionnels. Les autres modèles l'ignorent.
...
| Valeur | Comportement |
|---|---|
| `auto` (par défaut) | Active le mode Safe Venice et floute le contenu adulte |
| `low` | Désactive le mode Safe Venice ; renvoie la sortie non floutée |
