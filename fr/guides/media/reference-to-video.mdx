...
description: "Générez des vidéos IA cohérentes avec éléments de personnages et références de scène via Kling O3 et Grok Imagine R2V dans le Venice Video Studio."
...
Cette fonctionnalité est disponible sur les modèles **Kling O3** et **Grok Imagine R2V** dans le [Venice Video Studio](https://venice.ai/video?utm_source=venice-api-documentation). Chaque famille de modèles utilise une approche différente des images de référence — voir les sections spécifiques à chaque modèle ci-dessous.
...
Vous pouvez ajouter jusqu'à **4 éléments** par génération. Référencez-les dans votre prompt avec `@Element1`, `@Element2`, etc.
...
Chaque type d'entrée est plafonné indépendamment par l'API :
...
| `reference_image_urls` par élément | 1–3 |

<Note>
L'API applique ces plafonds indépendamment — il n'existe pas de budget d'images combiné entre `elements`, `scene_image_urls` et les images de début/fin, et l'ajout d'une image de début ou de fin ne réduit pas le nombre d'éléments que vous pouvez envoyer. Dépasser un seul plafond renvoie un `400` nommant ce champ, par exemple *"elements must have at most 4 items"*.
</Note>
...
Allez sur [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). Dans le navigateur de modèles à gauche, sélectionnez l'un des modèles **Kling O3 Reference to Video** :
...
Répétez pour des personnages ou objets supplémentaires (jusqu'à 4 éléments au total).

<Warning>
`elements` et `scene_image_urls` sont chacun plafonnés à **4**, et les plafonds s'appliquent indépendamment. Voir [Limitations](#limitations) pour plus de détails.
</Warning>
...
| Erreur `"elements must have at most 4 items"` | Plus de 4 éléments envoyés | Envoyez au plus 4 éléments ; le plafond ne change pas lorsqu'une image de début ou de fin est présente |
| Erreur `"scene_image_urls must have at most 4 images"` | Plus de 4 images de scène envoyées | Envoyez au plus 4 images de scène. Ce plafond est indépendant du plafond d'éléments |
...
| Durée | Toute seconde entière de 1s à 10s | 8s |
...
Allez sur [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). Dans le navigateur de modèles, sélectionnez **Grok Imagine R2V**.
...
| `reference_image_urls` | string[] | **Oui** | 1–7 URL d'image ou URL data |
| `duration` | string | **Oui** | Toute seconde entière de `"1s"` à `"10s"`. Le suffixe `s` est requis |
| `aspect_ratio` | string | **Oui** | par exemple, `"16:9"`, `"9:16"`, `"1:1"` |
| `resolution` | string | Non | `"480p"` (par défaut) ou `"720p"` |

<Note>
Grok Imagine R2V n'utilise pas les champs `elements`, `scene_image_urls` ou `image_url`. Toutes les images de référence sont passées via `reference_image_urls`.
</Note>
...
| Erreur « At least one reference image is required » | `reference_image_urls` est vide ou manquant | Fournissez au moins une URL d'image dans `reference_image_urls` |
...
| Vidéo trop courte | La durée par défaut est de 8 s | Définissez la durée sur `"10s"` pour des vidéos plus longues |
| Erreur `"Invalid enum value ... received '8'"` | `duration` a été envoyé sans le suffixe `s` | Envoyez `"8s"`, pas `"8"`. L'API vérifie exactement la forme affichée |
