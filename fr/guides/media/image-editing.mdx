...
description: "Éditez, retouchez par inpainting et supprimez les arrière-plans d'images avec l'API Venice en utilisant /image/edit, les couches multi-édition, les masques et les contrôles du mode sûr."
...
L'édition d'images sur Venice est synchrone. Envoyez votre image source à `/image/edit` ou `/image/multi-edit` et le résultat édité revient dans la même réponse sous forme de fichier image brut. Pour les détourages, `/image/background-remove` renvoie un PNG transparent.
...
<Note>
Venice ne fournit pas d'endpoint `/images/edits` compatible OpenAI. Le seul chemin d'image compatible OpenAI est `POST /images/generations` pour la génération. Pour l'édition, utilisez l'endpoint natif `POST /image/edit`.
</Note>
...
**Réponse (200) :**
Le corps de la réponse est constitué de données binaires d'image brutes. Enregistrez-les directement dans un fichier. Voir [Format de réponse](#format-de-réponse) pour savoir comment `output_format` contrôle le type de fichier.
...
Comme pour `/image/edit`, le corps de la réponse est constitué de données d'image brutes.
...
`/image/edit` et `/image/multi-edit` renvoient toujours l'image éditée sous forme de données binaires brutes (`image/png`, `image/jpeg` ou `image/webp`). Les endpoints d'édition n'acceptent pas de paramètre `response_format`. Il n'existe pas d'option `b64_json` ou `url` comme sur `/images/generations`. Si vous avez besoin du résultat dans du JSON, encodez la réponse binaire en base64 dans votre propre code.

Contrôlez le type de fichier avec `output_format` :

- `output_format` accepte `jpeg`, `jpg`, `png` ou `webp`.
- Lorsqu'il est omis, le format est déduit de `resolution` : PNG pour les éditions 1K et JPEG pour les éditions 2K et 4K.
...
| `resolution` | string | Non | `1K` | Palier de résolution pour l'image de sortie (`1K`, `2K`, `4K`) ; les valeurs prises en charge varient selon le modèle |
| `output_format` | string | Non | déduit de `resolution` | Type de fichier de sortie : `jpeg`, `jpg`, `png` ou `webp` |
...
|---|---|---|---|
| `/image/edit` | Chaîne base64 ou URL | Téléversement de fichier | `image/png`, `image/jpeg` ou `image/webp` selon `output_format` |
| `/image/multi-edit` | Chaînes base64 ou URL | Téléversements de fichiers | `image/png`, `image/jpeg` ou `image/webp` selon `output_format` |
