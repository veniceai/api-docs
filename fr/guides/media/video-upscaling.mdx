...
description: "Améliorez la résolution et la qualité vidéo avec le modèle topaz-video-upscale sur la file vidéo asynchrone de Venice : facteurs 1x, 2x ou 4x au choix."
...
`video_url` est **requis** pour les devis sur les modèles d'upscaling — le endpoint de devis récupère le fichier et détecte la durée, le FPS et la hauteur à partir des octets, il est donc inutile de les fournir. L'omettre renvoie `400` avec *"video_url is required for quotes on this model"*.

<Warning>
Les endpoints de devis et de file d'attente nomment actuellement le facteur d'upscaling différemment : `/video/queue` prend `upscale_factor` (`1`, `2` ou `4`), tandis que `/video/quote` lit le facteur depuis `resolution` (`"2x"` ou `"4x"`). Envoyer `upscale_factor` au endpoint de devis n'a aucun effet — il retombe sur la valeur par défaut `"2x"` et renvoie un prix 2x pour ce qui est peut-être une tâche 4x. Pour obtenir un devis d'upscaling 4x, passez `resolution: "4x"`.
</Warning>
...
| `"Insufficient balance"` | Crédits du compte trop bas | Ajoutez des crédits sur [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) |
