...
Ce guide couvre les variantes, les quatre workflows, la **politique média de l'API publique**, les **limites multimodales spécifiques à chaque famille**, le débit binaire de sortie, la tarification et des exemples `curl`.
...
| `seedance-2-5-text-to-video-basic` | T2V | 480p / 720p / 1080p | Sortie jusqu'à 30 s ; audio natif |
| `seedance-2-5-image-to-video-basic` | I2V | 480p / 720p / 1080p | Sortie jusqu'à 30 s ; ancrage sur première frame (et facultativement dernière frame) |
| `seedance-2-5-reference-to-video-basic` | R2V | 480p / 720p / 1080p | Jusqu'à 30 images + 10 vidéos + 10 donneurs audio ; mêmes workflows Reference / Edit / Extend / Stitch |
...
Passez `resolution` parmi : `480p`, `720p`, `1080p` ou `4k` (en minuscules). Seedance **2.0** (non Fast) accepte les quatre ; **2.5** accepte `480p` / `720p` / `1080p` ; **2.0 Fast** n'accepte que `480p` / `720p`. Découvrez les ID de modèles disponibles et leurs capacités avec `GET /models?type=video` — ne codez pas la disponibilité en dur.

## Débit binaire de sortie

Vous pouvez définir le débit binaire d'encodage de sortie sur chaque modèle public Seedance **2.0** (y compris Fast) et **2.5**. Passez `bitrate_mode` vous-même sur `/video/queue` — Venice ne le choisit pas automatiquement.

| Valeur | Comportement |
|---|---|
| `standard` | Débit binaire par défaut. Identique à l'omission du champ. |
| `high` | Encodage de meilleure qualité produisant des fichiers plus volumineux. |

C'est une option d'encodage réservée à la mise en file d'attente : elle ne change ni le prix ni le coût en tokens, et `/video/quote` n'accepte pas `bitrate_mode`. Les autres familles vidéo (Wan, Kling, LTX, etc.) ne prennent pas en charge ce champ.
...
Reliez des clips d'entrée avec des transitions générées par IA. Respectez les plafonds **spécifiques à chaque famille** de durée combinée et de nombre de clips dans [Limites d'entrée multimodale](#limites-dentrée-multimodale) (Seedance 2.0 : ≤3 clips / ≤15 s combinés ; Seedance 2.5 : plafonds vidéo plus élevés).
...
| Résolutions de sortie | 480p / 720p / 1080p / **4k** (Fast : 480p / 720p uniquement) | 480p / 720p / 1080p |
| Débit binaire de sortie (`bitrate_mode`) | `standard` (par défaut) ou `high` | `standard` (par défaut) ou `high` |
...
La réponse est du JSON (`{ "status": "PROCESSING", ... }`) pendant que la tâche s'exécute, et le corps bascule sur des octets `video/mp4` une fois qu'elle se termine. `status` vaut `PROCESSING` ou `COMPLETED` — il n'existe pas de statut `failed`. Une tâche en échec renvoie à la place une réponse non-200 portant l'erreur enregistrée. Voir [Génération vidéo](/guides/media/video-generation) pour le modèle complet d'interrogation.
...
Les soumissions reference-to-video doivent inclure au moins l'un de `reference_image_urls`, `reference_video_urls` ou `image_references`. La génération purement à partir de texte n'est pas un workflow R2V valide — utilisez plutôt un ID de modèle text-to-video. `reference_audio_urls` seul ne suffit pas (voir la section Audio ci-dessus). Notez que `video_references` n'est **pas** accepté sur Seedance — il est spécifique aux modèles prenant en charge l'audio vocal par référence, et le passer renvoie *"This model does not support per-reference video entities"*.
