...
Contrairement à la génération d'image, l'endpoint de file d'attente vidéo n'accepte pas de paramètre `safe_mode`. La prise en charge du contenu adulte est spécifique au modèle : choisissez une variante de modèle conçue pour une sortie non censurée. La famille Wan 2.7, par exemple, expose des variantes non censurées aux côtés des variantes par défaut :

- `wan-2-7-uncensored-text-to-video`
- `wan-2-7-uncensored-image-to-video`

Les requêtes soumises à des variantes qui ne sont pas non censurées peuvent être rejetées comme violations de contenu (`422`). Voir [Modèles vidéo](/models/video) et [Tarification](/overview/pricing) pour la liste actuelle des variantes et leur tarification à la seconde.
