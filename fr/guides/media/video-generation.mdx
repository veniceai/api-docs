...
description: "Générez des vidéos depuis texte ou images sur l'API asynchrone de Venice : devis de prix, envoi à /video/queue et récupération du MP4 via /video/retrieve."
...
| `prompt` | string | Oui | - | Ce qu'il faut générer. La longueur maximale est spécifique au modèle et varie fortement — de quelques centaines de caractères jusqu'à 20 000. Les modèles qui ne déclarent pas leur propre limite ont une valeur par défaut de 2 500. Lisez `model_spec.constraints.prompt_character_limit` depuis `GET /models?type=video` plutôt que de supposer une limite unique |
...
| `bitrate_mode` | string | Non | `"standard"` | Seedance 2.0 / 2.5 uniquement. `"standard"` ou `"high"`. `high` demande un encodage de meilleure qualité produisant des fichiers plus volumineux. Ne change pas le prix. Omettre pour les autres familles |
...
Contrairement à la génération d'image, l'endpoint de file d'attente vidéo n'accepte pas de paramètre `safe_mode`. La prise en charge du contenu adulte est spécifique au modèle : choisissez une variante de modèle conçue pour une sortie non censurée. La famille Wan 2.7, par exemple, expose des variantes Enhanced aux côtés des variantes par défaut :

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

Les requêtes soumises aux variantes par défaut peuvent être rejetées comme violations de contenu (`422`). Voir [Modèles vidéo](/models/video) et [Tarification](/overview/pricing) pour la liste actuelle des variantes et leur tarification à la seconde.
