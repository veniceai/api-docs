...
<Note>
Utilisez `OpenAIChatModel` (Chat Completions), et non `OpenAIResponsesModel` / le raccourci `openai:`. Venice n'expose actuellement pas l'API Responses d'OpenAI ; son chemin de génération de texte est `/chat/completions`. Fixer Chat Completions évite le comportement spécifique à Responses que les modèles OpenAI par défaut utilisent dans les versions plus récentes de PydanticAI.
</Note>
