---
title: OpenCode
description: "Connectez OpenCode à Venice via un fournisseur personnalisé dans opencode.json : votre agent de codage en terminal tourne sur des modèles privés."
...
Si vous définissez `"model": "venice/z-ai-glm-5-3-flash"` comme montré ci-dessus, OpenCode devrait utiliser ce modèle Venice par défaut. Vous pouvez changer le modèle par défaut en remplaçant `z-ai-glm-5-3-flash` par n'importe quel ID de modèle du [catalogue de modèles de texte](/models/text) et en l'ajoutant à la map `models`.
