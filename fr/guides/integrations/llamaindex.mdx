...
async def main() -> None:
    response = await agent.run("How much does z-ai-glm-5-3-flash cost?")
    print(str(response))
...
| Agents / appel d'outils | `z-ai-glm-5-3-flash` | Modèle privé rapide pour les agents |
| Raisonnement complexe | `z-ai-glm-5-3-flash` | Meilleure planification en plusieurs étapes |
