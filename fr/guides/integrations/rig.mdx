---
title: "Rig"
description: "Créez des agents Rust typés avec le fournisseur Venice natif de Rig : chat, streaming, outils, sortie structurée, embeddings et génération d'images."
...
Si vous préférez un proxy Axum brut plutôt qu'un framework d'agents, consultez [Créer une passerelle LLM en Rust](/learn/rust-llm-gateway).
...
<Note>
Préférez `venice::Client` plutôt que de pointer le client OpenAI de Rig vers Venice. Le client par défaut `openai::Client` cible l'API Responses d'OpenAI. Le fournisseur Venice parle `/chat/completions` et expose [`VeniceParameters`](#paramètres-spécifiques-à-venice).
</Note>
