...
<Steps>
  ...
  <Step title="Frapper une clé API">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` renvoie un challenge EIP-4361 à usage unique, lié à ce wallet, qui expire 15 minutes après émission. Signez le `message` tel quel avec le wallet de staking, puis `POST` l'adresse, la signature et le message en retour. Venice renvoie une clé API liée au compte utilisateur dérivé de ce wallet.
  </Step>
</Steps>
