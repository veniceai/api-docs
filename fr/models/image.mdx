...
<Tip>
**Niveaux de qualité.** `gpt-image-2` (generate) et `gpt-image-2-edit` (edit/multi-edit) acceptent un paramètre `quality` optionnel — `low`, `medium` ou `high` (par défaut). Les niveaux inférieurs coûtent moins ; voir l'[aperçu de la tarification](/fr/overview/pricing#image-generation) pour la matrice complète ou récupérez `model_spec.pricing.quality` depuis l'[endpoint Models](/fr/api-reference/endpoint/models/list).
</Tip>
