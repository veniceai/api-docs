---
title: "Modèles de synthèse vocale (Text-to-Speech)"
description: "Modèles de synthèse vocale Venice avec voix multilingues, sélection de voix et sortie audio en streaming via l'endpoint /audio/speech compatible OpenAI."
"og:title": "Modèles de synthèse vocale | Venice API Docs"
---

<div id="model-search-placeholder" data-filter="tts">
{/* AUTO-GENERATED:MODELS:START */}

Utilisez la valeur `id` comme paramètre `model` dans les requêtes API. 11 modèles actuellement disponibles.
...
<Note>
Les ID de voix sont sensibles à la casse et **valides uniquement pour le `model` correspondant**. Transmettez
les deux champs ensemble dans le payload de votre requête. Voir l'[API Audio Speech](/fr/api-reference/endpoint/audio/speech) pour des exemples.
</Note>
