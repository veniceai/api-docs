...
`download_url` is optional. Some models include a pre-signed URL you will use to download the finished video instead of reading it from the retrieve response. If present, persist it alongside `queue_id` since it is only returned once.
