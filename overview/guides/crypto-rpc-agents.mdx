...
"og:description": "Use Venice as both the model provider and the blockchain RPC layer for autonomous AI agents. One API key, 230+ models, 11 chains."
---

Venice is the only AI platform that gives your agent inference (230+ models) and blockchain access (11 chains) through a single credential. Your agent can think, sign, and send transactions without juggling separate API keys for OpenAI, Alchemy, and Infura.

<CardGroup cols={2}>
  <Card title="One credential, two superpowers" icon="key">
    A single API key (or wallet) for both LLM inference and JSON-RPC calls.
  </Card>
  <Card title="11 chains, 21 networks" icon="link">
    Ethereum, Base, Arbitrum, Optimism, Polygon, Linea, Avalanche, BSC, Blast, zkSync Era, Starknet (mainnet + testnets).
  </Card>
...
| **Crypto RPC** | JSON-RPC 2.0 proxy to 11 EVM chains and Starknet (mainnet + testnets) |
...
Crypto RPC is billed in Venice credits at `~$6.25 × 10⁻⁷` per credit.
...
|---|---|---|
| `eth_call` on Ethereum (20 × 1x) | 20 | $0.0000125 |
| `trace_transaction` on Ethereum (20 × 2x) | 40 | $0.0000250 |
| `trace_replayTransaction` on Ethereum (20 × 4x) | 80 | $0.0000500 |
| `eth_call` on zkSync (30 × 1x) | 30 | $0.0000188 |

Errored items in batch requests are billed at a flat 5 credits each.
