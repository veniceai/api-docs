...
<Warning>
**Newer Claude model IDs must be prefixed with `venice-` in Cursor.**

Cursor auto-detects model IDs starting with `claude-` and rewrites the request into Anthropic's native format. Venice expects OpenAI-compatible requests, so this breaks the API. The `venice-` prefix bypasses Cursor's detection — Venice strips the prefix during model resolution.
</Warning>

| Model | ❌ Don't use in Cursor | ✅ Use in Cursor |
|-------|----------------------|-----------------|
