...
Sizing is model-specific. Some models accept explicit `width` and `height`; others expose `aspect_ratio` plus a `resolution` tier such as `1K`, `2K`, or `4K`. Resolution-tier models include `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro`.
...
Use [Image Models](/models/image) or the [Models API](/api-reference/endpoint/models/list) to confirm whether a model expects pixel dimensions or resolution tiers.
...
| `width` | integer | No | `1024` | Output width in pixels for models that support pixel-based sizing |
| `height` | integer | No | `1024` | Output height in pixels for models that support pixel-based sizing |
...
| `aspect_ratio` | string | Conditional | - | Used by models that support ratio-based sizing, such as `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro` |
...
Some image models support `aspect_ratio` and `resolution` instead of simple `width` and `height`. For example, `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro` support `1K`, `2K`, and `4K`:
