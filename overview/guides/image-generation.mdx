...
Sizing is model-specific. Some models accept explicit `width` and `height`; others expose `aspect_ratio` plus a `resolution` tier such as `1K`, `2K`, or `4K`.
...
| `aspect_ratio` | string | Conditional | - | Used by models that support ratio-based sizing, such as `gpt-image-2` |
...
Some image models support `aspect_ratio` and `resolution` instead of simple `width` and `height`. For example, `gpt-image-2` supports `1K`, `2K`, and `4K`:
