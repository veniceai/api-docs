...
For models requiring explicit cache markers (like Claude), Venice automatically places cache breakpoints on the **second-to-last user message** in multi-turn conversations. This ensures:

- **Previous conversation history is read from cache** (not reprocessed)
- **Only the new turn (latest user message) is treated as fresh input**
- **Cache grows incrementally** with each turn instead of being rewritten entirely

Example with Claude Opus 4.5:
...
This happens automatically—you don't need to add any `cache_control` markers for conversation history.
...
<Tip>
Venice automatically adds `cache_control` to system prompts and strategically places cache breakpoints on conversation history for multi-turn conversations. For most use cases, you don't need to add manual markers—Venice optimizes caching automatically. Manual markers are only needed for special cases like caching large documents within a single user message.
</Tip>

### Claude Opus 4.5 (Anthropic)

Claude requires explicit cache breakpoints, which Venice adds automatically to system prompts and conversation history. You only need manual breakpoints for special cases like caching large documents within a single user message.
...
<Note>
In multi-turn conversations, Venice automatically adds `cache_control` to conversation history. The manual markers shown above are only needed if you want to ensure caching on the first turn before a follow-up message exists, or for caching large documents that appear later in a single user message.
</Note>
...
### Let Venice handle conversation caching

For multi-turn conversations, Venice automatically:
- Caches the system prompt
- Places cache breakpoints on conversation history
- Ensures only new content is processed as input

You don't need to manually add `cache_control` to each message. Just send your conversation normally and Venice optimizes caching for you.
