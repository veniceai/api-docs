...
<Note>
**qwen3-235b Deprecation Details**: Starting December 14, 2025, `qwen3-235b` will be split into two specialized models:
- **`qwen3-235b-a22b-thinking-2507`** ($0.45 / $3.50): Replaces the default `qwen3-235b` behavior with thinking enabled. This model ignores the `disable_thinking` parameter and always uses reasoning capabilities.
- **`qwen3-235b-a22b-instruct-2507`** ($0.15 / $0.75): For standard tasks without thinking, offering significantly improved pricing.

**Important**: After December 14, calling `qwen3-235b` will automatically route to `qwen3-235b-a22b-thinking-2507` and will maintain the $0.45 / $3.50 pricing, even if `disable_thinking` is set. To take advantage of the lower $0.15 / $0.75 pricing for non-thinking tasks, you must explicitly specify `qwen3-235b-a22b-instruct-2507` in your API calls.
</Note>
