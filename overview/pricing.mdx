---
title: API Pricing
description: "Per-model token, image, audio, and video rates for the Venice API, plus payment options — USD, crypto, DIEM staking, and bundled subscription credits."
...
Bundled credits are API credits attached to your Venice account by a Venice subscription plan (for example, the one-time credit granted to new Pro subscribers). They are spent by the API alongside DIEM and USD credits and appear in billing responses as the `BUNDLED_CREDITS` currency.

- **What they cover** — bundled credits can be spent on any paid Venice API endpoint (chat, images, audio, video, embeddings). They are priced against the same USD rate sheet on this page.
- **Spend order** — API keys draw from your account in this order: **DIEM → bundled credits → USD**. You do not need to pick a currency per request. See [How funding works](/overview/vvv-diem#how-funding-works).
- **Where to check the balance** — view current balances at [venice.ai/settings/api](https://venice.ai/settings/api), or call the [Billing Balance](/api-reference/endpoint/billing/balance) endpoint and read the `BUNDLED_CREDITS` entry.

<Note>
  Any monthly credit allowances shown on the [Venice consumer plans](https://venice.ai) page (for example, credits for the venice.ai chat app or media generators) are separate from API bundled credits and may not be spendable through this API. The Venice API only spends the DIEM, bundled, and USD balances reported by the [Billing Balance](/api-reference/endpoint/billing/balance) endpoint.
</Note>
