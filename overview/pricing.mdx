...
<Info>
**API credits are denominated in USD.** $1 USD purchases $1 of API credits — there is no separate token, multiplier, or conversion rate. Credits never expire.

API billing is **separate** from venice.ai consumer subscriptions (Pro, Plus, Max). Those plans cover usage in the venice.ai app and do **not** include ongoing API usage. See [Pro Users](#pro-users) below for the one-time API credit included with a Pro upgrade.
</Info>
...
| Qwen3 VL 235B | `qwen3-vl-235b-a22b` | $0.25 | $1.50 | - | - | 128K | Private |
...
<Note>
The venice.ai consumer subscriptions (Pro, Plus, Max) cover usage **inside the venice.ai app** — they do not include a recurring API credit allowance. API requests are billed against your USD or DIEM balance at the per-model rates above, regardless of which consumer plan you have. The one-time $10 credit at Pro upgrade is the only API allowance bundled with a subscription.
</Note>
