...
<Info>
**API vs. consumer plans.** The Venice API is billed **pay-per-use** in USD credits (or via staked DIEM). It is separate from the Venice consumer web app's monthly subscription plans (Free, Pro, Max), whose allowances — such as image counts or unlimited chat — apply only to the web/desktop/mobile apps, **not** to API usage. All API calls are metered against your API credit balance regardless of your consumer plan. See [Pro Users](#pro-users) below for the one-time credit Pro subscribers receive.
</Info>
...
| Inkling (Beta) | `inkling` | $2.34 | $5.85 | - | - | 1000K | Private |
...
Beyond that credit, API usage is billed the same pay-per-use rates listed above — the Pro subscription itself does not include additional API allowances such as included images, characters, or tokens. Some models are gated to Pro accounts (`PRO_ONLY_MODEL`); calling them from an account without an active Pro subscription returns `AUTHENTICATION_FAILED_INACTIVE_KEY`. See [Error Codes](/api-reference/error-codes).
