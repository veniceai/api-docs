---
title: API Pricing
description: "Venice API pricing for every model: per-token rates for chat completions and per-generation prices for image, video, and audio, in USD, VVV, or DIEM."
...
### Venice.ai app subscriptions vs. API billing

Venice.ai's consumer subscription plans (for example, Pro and Max on [venice.ai](https://venice.ai)) apply to the Venice chat, image, and video apps at [venice.ai](https://venice.ai). They are separate from API billing.

* API usage is always metered per request at the per-token, per-image, per-second, or per-generation rates listed above. There is no unlimited or unmetered API tier.
* App subscription perks — unlimited chat prompts, monthly credit allowances for in-app video, music, and frontier generation, and credit banking — apply only inside the Venice apps. They do not grant API access or convert into API credits, apart from the one-time $10 API credit for Pro subscribers noted above.
* To fund API requests, buy Venice credits in USD or crypto from the [API settings](https://venice.ai/settings/api?utm_source=venice-api-documentation) page, or stake DIEM for a daily allowance (see [VVV & DIEM](/overview/vvv-diem)).
