---
title: API Pricing
description: "Venice API per-1M-token prices for text, image, video, audio, and embedding models, plus credit costs for web search, RPC, and other services."
...
| Seedance 2.0 Mini Enhanced (Beta) | `seedance-2-0-mini-enhanced-text-to-video` | Text to Video | Variable | Anonymized |
| Seedance 2.0 Mini R2V | `seedance-2-0-mini-reference-to-video` | Text to Video | Variable | Anonymized |
| Seedance 2.0 Mini R2V Enhanced (Beta) | `seedance-2-0-mini-enhanced-reference-to-video` | Text to Video | Variable | Anonymized |
| Seedance 2.0 R2V | `seedance-2-0-reference-to-video` | Text to Video | Variable | Anonymized |
| Seedance 2.0 R2V Enhanced (Beta) | `seedance-2-0-enhanced-reference-to-video` | Text to Video | Variable | Anonymized |
...
There is no zero-cost model on the Venice API — every model has a per-token, per-image, per-second, or per-request price. The Venice.ai Pro subscription's daily image and message allowances apply to the web app and mobile app only, not to API calls.

The two ways to use the API without a per-request USD charge are:

- **Stake VVV for daily DIEM credits.** VVV stakers receive a daily DIEM allocation proportional to their share of the staking pool. Each 1 DIEM = $1/day of inference credits that refresh at 00:00 UTC. See the [Earn daily credits by staking VVV](#earn-daily-credits-by-staking-vvv) section below.
...
2. **Stake at [venice.ai/token](https://venice.ai/token).** The staking button lives on that page. Under the hood it's two on-chain transactions on the [Venice staking contract](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff): `approve` on the VVV token, then `stake` on the staking contract. Your VVV balance decreases and your sVVV balance increases by the same amount.
...
For agents that mint their own API key from a staking wallet with no human in the loop, see [Autonomous agent API key creation](/guides/getting-started/generating-api-key-agent).
...
Non-uncensored models on Venice still return `CONTENT_POLICY_VIOLATION` (HTTP 422) for prompts the upstream provider refuses. See [Error codes](/api-reference/error-codes) for the full list. If you want the most permissive behavior, pick an `uncensored` or `heretic` variant and disable Venice's default system prompt with `venice_parameters.include_venice_system_prompt: false` — see [API Specification](/api-reference/api-spec) for the parameter reference.
