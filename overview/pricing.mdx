---
title: API Pricing
description: "Per-token and per-request rates for Venice text, image, audio, video, and embedding models, plus web search pricing and payment options."
...
API usage is always pay-per-use. There is no free API tier and no unlimited API plan: every model on this page bills per token, per image, or per request.

Venice app subscriptions (the plans used by the chat interface at [venice.ai](https://venice.ai/?utm_source=venice-api-documentation)) are billed separately from API usage. Their plan tiers, prices, and in-app benefits are not covered in this API documentation. Manage them in your Venice account.

Credits included with a subscription, such as the one-time $10 Pro credit, appear on your account as **bundled credits**. The API spends account balances in this order: DIEM first, then bundled credits, then USD. See [VVV & DIEM](/overview/vvv-diem#how-funding-works) for how each funding source works and how to check your balances.
