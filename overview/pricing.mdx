...
The Venice API is **pay-as-you-go**: every request is metered and charged against your account's API credit balance. There is no free tier, no monthly quota, and no login-based subscription plan for the API — access is authenticated by an API key and billed per request.

<Note>
The free and Pro tiers on the [venice.ai](https://venice.ai) consumer app (web and mobile chat) are separate from the API. Consumer subscription limits, "Pro model" access, and account-based quotas apply only to that app — they do not grant free or expanded API usage. API usage is always billed against your API credit balance, regardless of your venice.ai account plan.
</Note>
...
Pro subscribers on the [venice.ai](https://venice.ai) consumer app receive a one-time $10 API credit when upgrading to Pro. Use it to test and build small apps. Beyond this one-time credit, API usage is billed per request against your credit balance — a Pro subscription does not grant ongoing free API usage or a login-based API quota.
