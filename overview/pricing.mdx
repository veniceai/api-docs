...
| GPT Image 2 | `gpt-image-2` | Per Image: $0.02 – $0.83 [^gpt-image-2-quality] | Anonymized |
...
| GPT Image 2 | `gpt-image-2-edit` | $0.02 – $0.91 [^gpt-image-2-quality] |
...
[^flux2max-edit-billing]: Flux 2 Max edit pricing can vary with the number and size of input
images. The listed API price covers supported requests with up to three input images.

[^gpt-image-2-quality]: GPT Image 2 (`gpt-image-2`) and GPT Image 2 Edit (`gpt-image-2-edit`)
charge per request based on the selected `quality` (`low`, `medium`, `high`), the resolved
output `resolution` (`1K`, `2K`, `4K`), prompt length, and — for edit requests — the number of
input images. The ranges above are typical customer-facing API charge ranges; final per-request
charges are calculated from the submitted request and reflected in usage and billing records.
