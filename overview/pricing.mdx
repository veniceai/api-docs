---
title: API Pricing
description: "Venice API pricing for text, image, video, audio, and music models, including per-token, per-generation, and per-second rates in USD, VVV, and DIEM."
...
Pro is a subscription for the Venice web and mobile app. It does not include free or unlimited API usage. All API requests are billed per token, per image, or per generation at the rates in the tables above, regardless of subscription tier.

Pro subscribers do receive a **one-time $10 Venice credit** when they upgrade. That credit is applied to API usage as a bundled credit: API keys spend from the linked account in the order **DIEM → bundled credits → USD**, so the $10 is drawn on only after any staked DIEM allowance for the current epoch is exhausted. See [VVV & DIEM](/overview/vvv-diem) for the full spend order and [Billing Balance](/api-reference/endpoint/billing/balance) to check which currency your key will charge next.

### Free tier

There is no free API tier and no free trial key. A freshly generated key authenticates, but paid endpoints (chat completions, image generation, audio, video, embeddings) return `402 INSUFFICIENT_BALANCE` until the account has a spendable balance in DIEM, bundled credits, or USD. Fund the account with USD or crypto at [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation), or stake DIEM for a refreshing daily allowance.
