...
VVV and DIEM are Venice's on-chain funding layer for the API. Stake DIEM to receive a fixed daily inference allowance — **1 DIEM = $1 per day** of Venice credit — without buying prepaid USD credits for every request.
...
| **DIEM** | Tokenized compute unit. Stake DIEM to unlock a perpetual daily Venice credit equal to $1 per DIEM. |
...
* Your daily allocation equals the amount of DIEM you have staked (1 staked DIEM → $1 of Venice credit that epoch).
...
Check your current and next-epoch DIEM balance in the dashboard at [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation), or via the [Billing Balance](/api-reference/endpoint/billing/balance) endpoint.

### Bundled credits

Bundled credits are prepaid Venice credit attached to your account, such as the one-time $10 credit Pro subscribers receive (see [API Pricing](/overview/pricing#pro-users)). The API spends them automatically after DIEM and before USD. No request parameter is needed to select them.

The [Billing Balance](/api-reference/endpoint/billing/balance) response does not include a numeric bundled-credits field. `balances` only reports `diem` and `usd`. When bundled credits are the source your next request will draw from, `consumptionCurrency` returns `BUNDLED_CREDITS`. To see the remaining bundled-credit amount, check the dashboard at [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation).

## Get DIEM and stake it

Use the token dashboard at [venice.ai/token](https://venice.ai/token?utm_source=venice-api-documentation). Connect a Base wallet — do not send tokens directly to a contract address.
...
<Steps>
  ...
  <Step title="Stake VVV (if minting DIEM)">
    On [venice.ai/token](https://venice.ai/token?utm_source=venice-api-documentation), stake VVV to receive sVVV. Staked VVV earns emissions. The staking contract is [`0x321b7ff75154472B18EDb199033fF4D116F340Ff`](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff#code).
  </Step>
  ...
  <Step title="Stake DIEM for Venice credit">
    Stake DIEM on the same token dashboard. Each staked DIEM grants **$1 per day** of Venice credit for as long as it remains staked.
  ...
  <Step title="Create or use an API key">
    Sign in to Venice with the same wallet (Sign-In-With-Ethereum) so the staked DIEM attaches to your account, then create a key from [API settings](https://venice.ai/settings/api?utm_source=venice-api-documentation) or the [API key guide](/guides/getting-started/generating-api-key).

    Autonomous agents can mint a key from a wallet that holds staked VVV — see [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent).
  </Step>
</Steps>

<Warning>
  Never send VVV or DIEM directly to a contract address. Tokens sent that way cannot be recovered. Always stake through [venice.ai/token](https://venice.ai/token?utm_source=venice-api-documentation) or a known staking transaction flow.
</Warning>
...
If a request would exceed remaining DIEM and no USD or bundled credits cover it, the API returns `402` with `INSUFFICIENT_BALANCE`. Stake more DIEM, wait for the next epoch, or add USD credits in [Settings → API](https://venice.ai/settings/api?utm_source=venice-api-documentation).
...
<CardGroup cols={2}>
  ...
  <Card title="Token dashboard" icon="chart-line" href="https://venice.ai/token?utm_source=venice-api-documentation">
    Stake VVV, mint DIEM, and stake DIEM for Venice credit.
  </Card>
