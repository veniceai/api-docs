...
| `enable_web_scraping` | boolean | Enable web scraping of up to 5 URLs detected in the user message (90s batch timeout). Scraped content augments responses and bypasses web search. Only successfully scraped URLs are billed.<br/>Additional usage-based pricing applies, see [pricing](/overview/pricing#web-search-and-scraping). | `false` |
