---
title: 'Generate Images'
openapi: 'POST /image/generate'
"og:title": "Image Generate | Venice API Docs"
"og:description": "Documentation covering Venice's image generation API."
---

## Sizing Options

Image models use model-specific sizing parameters:

- Models with pixel-based sizing accept `width` and `height`.
- Models with resolution tiers accept `aspect_ratio` and `resolution` instead.

For example, `gpt-image-2`, `nano-banana-2`, and `nano-banana-pro` support `resolution` values of `1K`, `2K`, and `4K`, with aspect ratios such as `16:9`:

```json
{
  "model": "gpt-image-2",
  "prompt": "a cinematic wide shot of Venice at sunset",
  "aspect_ratio": "16:9",
  "resolution": "4K",
  "format": "png"
}
```

The same sizing pattern also applies to other resolution-tier models:

```json
{
  "model": "nano-banana-pro",
  "prompt": "a serene canal in venice at sunset",
  "aspect_ratio": "16:9",
  "resolution": "2K"
}
```

For models that use direct pixel dimensions, send `width` and `height` instead:

```json
{
  "model": "qwen-image-2",
  "prompt": "a serene canal in venice at sunset",
  "width": 1024,
  "height": 1024
}
```

Check [Image Models](/models/image) or the [Models endpoint](/api-reference/endpoint/models/list) for each model's supported sizing options before mixing sizing fields across models.

## Postman Collection
For additional examples, please see this [Postman Collection](https://www.postman.com/veniceai/workspace/venice-ai-workspace/folder/38652128-0adc004d-2edf-4b88-a3bb-0f868c791c9c?action=share&source=copy-link&creator=38652128&ctx=documentation).

-------
