---
title: 'Edit (aka Inpaint)'
openapi: 'POST /image/edit'
"og:title": "Image Edit | Venice API Docs"
"og:description": "Documentation covering Venice's image edit (aka inpaint) API."
---

<Warning>
This is an experimental endpoint and may be subject to change.
</Warning>

<Info>
**Pricing:** Image editing/inpainting pricing varies by model. The default model (`firered-image-edit`) is **$0.04 per edit**. See the [Models endpoint](/api-reference/endpoint/models/list) for all available inpaint models and their pricing.
</Info>

## Postman Collection
For additional examples, please see this [Postman Collection](https://www.postman.com/veniceai/workspace/venice-ai-workspace/folder/38652128-2d156cd6-a9bc-4586-8a8b-98e4b5c4435d?action=share&source=copy-link&creator=38652128&ctx=documentation).

-------

<Warning>
The default model (`firered-image-edit`) blocks requests that try to generate explicit sexual imagery, sexualize minors, or depict real-world violence. Other models may have different content policies.
</Warning>
