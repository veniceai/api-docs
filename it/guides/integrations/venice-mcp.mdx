...
description: "Il server ufficiale Venice Model Context Protocol espone 30 tool Venice per chat, immagini, video, audio, musica e character in qualsiasi host MCP."
...
Il [Venice MCP Server](https://github.com/veniceai/venice-mcp-server) è il server ufficiale [Model Context Protocol](https://modelcontextprotocol.io/) per Venice. Espone l'intera API Venice (chat, image, video, audio, music, embeddings, web augment e characters) come **30 tool** che qualsiasi agente compatibile con MCP può chiamare.
...
<CardGroup cols={3}>
  <Card title="30 tool" icon="toolbox">
...
**30 tool** che coprono ogni modalità Venice, **3 resource** (`venice://models`, `venice://styles`, `venice://voices`) e **3 template di prompt**.
