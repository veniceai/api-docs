...
<Note>
Usa `OpenAIChatModel` (Chat Completions), non `OpenAIResponsesModel` o la scorciatoia `openai:`. Venice attualmente non espone l'API Responses di OpenAI; il percorso di generazione del testo è `/chat/completions`. Fissare Chat Completions evita il comportamento specifico dell'API Responses che i modelli OpenAI di default utilizzano nelle versioni più recenti di PydanticAI.
</Note>
