...
description: "Crea una chiave API Venice in modo autonomo da un agente AI mettendo in staking VVV su Base e firmando un token di validazione con un wallet."
...
* Qualsiasi quantità non zero di VVV in staking. L'endpoint di minting richiede solo che il wallet abbia un saldo sVVV non zero, quindi 1 VVV è sufficiente per generare una chiave. Consulta [Pagare l'inferenza](#pagare-linferenza) per ciò che ti serve per chiamare effettivamente gli endpoint a pagamento.
...
<Steps>
  ...
  <Step title="Genera l'API key">
    ...
    - `consumptionLimit` — il tetto di spesa per questa chiave, in `usd`, `vcu` o `diem`
    - `limitPeriod` — la finestra su cui viene misurato il tetto: `EPOCH` (si azzera ogni giorno UTC), `MONTH` (si azzera il 1° di ogni mese UTC) o `LIFETIME` (non si azzera mai, quindi il tetto è permanente)
    - `modelPrivacy` — quali modelli la chiave può chiamare, per livello di privacy: `ALL`, `PRIVATE_TEXT` o `PRIVATE_ONLY`

    <Warning>
    `limitPeriod` ha come valore predefinito `EPOCH` quando omesso, il che significa che il tetto **si azzera ogni giorno** invece di essere un totale a vita. Una chiave creata con `consumptionLimit: {"usd": 50}` e senza `limitPeriod` può spendere $50 al giorno. Per una chiave di un agente non presidiato, imposta esplicitamente `limitPeriod: "LIFETIME"` in modo che il limite sia un tetto permanente.
    </Warning>
...
const mintResponse = await fetch("https://api.venice.ai/api/v1/api_keys/generate_web3_key", {
  ...
  body: JSON.stringify({
    ...
    // Un tetto permanente di $50. Senza limitPeriod si azzererebbe ogni giorno.
