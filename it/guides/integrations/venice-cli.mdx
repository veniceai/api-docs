---
title: Venice CLI
description: "CLI ufficiale Venice per chat, ricerca web, generazione di immagini, text-to-speech, trascrizione e video dal tuo terminale, con E2EE opzionale."
...
### Risoluzione dei problemi: comando `venice` non trovato

Se il terminale segnala che `venice` non è riconosciuto dopo `npm install -g veniceai-cli`, la cartella globale dei binari di npm non è nel tuo `PATH`. Succede spesso su Windows.

1. Esegui `npm config get prefix` per trovare la cartella di installazione globale di npm.
2. Aggiungi la cartella globale dei binari al tuo `PATH`. Su Windows è la cartella del prefisso stessa (di solito `%AppData%\npm`). Su macOS e Linux è la sottocartella `bin` del prefisso.
3. Apri una nuova finestra del terminale ed esegui `venice chat "Hello"`.

Per evitare la configurazione del `PATH`, esegui la CLI tramite `npx`, ad esempio `npx veniceai-cli chat "Hello"`.
