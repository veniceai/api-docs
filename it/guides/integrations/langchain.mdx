...
Venice esegue l'embedding solo di testo. `OpenAIEmbeddings` di LangChain ha come default `check_embedding_ctx_length=True`, che tokenizza con tiktoken e può inviare array di token interi. Venice li rifiuta con HTTP 400, quindi imposta `check_embedding_ctx_length=False`.
...
embeddings = OpenAIEmbeddings(
    ...
    api_key="your-venice-api-key",
    base_url="https://api.venice.ai/api/v1",
    check_embedding_ctx_length=False,  # Richiesto: invia testo, non token ID
)
...
| Ragionamento complesso | `z-ai-glm-5-3-flash` | Modello di ragionamento privato veloce |
| Function calling | `z-ai-glm-5-3-flash` | Uso affidabile dei tool |
