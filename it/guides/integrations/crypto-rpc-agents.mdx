...
description: "Usa Venice come provider di modelli e layer RPC blockchain per agenti autonomi: una sola chiave API, oltre 230 modelli e JSON-RPC su 11 chain."
...
|---|---|---|
| **API key** | Agenti server-side, deployment fissi | Header `Authorization: Bearer <key>`. Ottieni una chiave su [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). |
...
<Tip>
Gli agenti veramente autonomi possono generare la propria API key facendo staking di VVV su Base. Consulta [Creazione autonoma di API key per agenti](/guides/integrations/generating-api-key-agent).
</Tip>
...
L'elenco sopra è un'istantanea. Chiama [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks) — un endpoint pubblico che non richiede autenticazione — per l'elenco live autorevole.
...
| **Ricarica USD o crypto tramite dashboard** | No (browser) | Accedi a [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) con lo stesso wallet (Sign-In-With-Ethereum), quindi aggiungi crediti in Settings, API. Sia Stripe (carta) sia Coinbase (crypto) sono dietro quella pagina e richiedono un browser. I crediti non scadono mai. |
...
Consulta [Creazione autonoma di API key per agenti](/guides/integrations/generating-api-key-agent) per il walkthrough completo con codice e riferimento errori completo.
...
<CardGroup cols={2}>
  ...
  <Card title="API key per agenti autonomi" icon="robot" href="/guides/integrations/generating-api-key-agent">
