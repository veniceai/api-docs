...
<Steps>
  ...
  <Step title="Genera una API key">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` restituisce una challenge EIP-4361 monouso, vincolata a quel wallet, che scade 15 minuti dopo l'emissione. Firma il `message` così com'è con il wallet in staking, poi fai `POST` con address, firma e message. Venice restituisce una API key associata all'account utente derivato da quel wallet.
  </Step>
</Steps>
