---
title: "Vercel AI SDK"
description: "Costruisci app Next.js e React potenziate dall'AI con Venice AI e Vercel AI SDK"
"og:title": "Vercel AI SDK | Venice API Docs"
"og:description": "Esegui lo streaming delle risposte Venice AI in app Next.js con Vercel AI SDK"
---

Il [Vercel AI SDK](https://sdk.vercel.ai/) è il modo più popolare per costruire funzionalità AI in app Next.js, React, Svelte e Vue. Venice funziona out-of-the-box come provider compatibile OpenAI.
...
Crea un provider Venice usando l'adattatore compatibile OpenAI:
...
<Note>
Usare `.chat()` assicura che le richieste vadano all'endpoint `/chat/completions` di Venice. La sintassi predefinita `openai('model')` potrebbe usare nuovi endpoint OpenAI che Venice non supporta ancora.
</Note>

## Chat in streaming (Next.js App Router)
...
La generazione immagini di Venice può essere chiamata direttamente insieme all'AI SDK:
...
Lascia che gli utenti scelgano tra i modelli Venice:
...
Passa parametri Venice per la web search:
...
<Note>
Se `experimental_providerMetadata` non viene passato attraverso, puoi usare un wrapper fetch personalizzato o chiamare direttamente l'API Venice per le funzionalità di web search.
</Note>

## Embeddings

Per gli embedding, usa `textEmbeddingModel()` direttamente sul provider:
...
|----------|-------|-----|
| App di chat | `venice-uncensored` | Veloce, economico, niente filtri |
| Task complessi | `zai-org-glm-5-1` | Ragionamento flagship privato |
| App di vision | `qwen3-vl-235b-a22b` | Comprensione immagini avanzata |
