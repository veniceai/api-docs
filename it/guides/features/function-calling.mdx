...
description: "Consenti ai modelli di chat Venice di chiamare strumenti e funzioni della tua applicazione con function calling compatibile OpenAI, schemi tool e l'API /chat/completions."
...
L'esempio qui sopra si ferma dopo un solo giro. Gli agenti reali continuano, perché di solito il modello ha bisogno di più strumenti prima di poter rispondere. [Costruire un agente che usa strumenti con il function calling](/guides/features/tool-using-agent) percorre l'intero ciclo su un database che il modello non ha mai visto, incluse chiamate parallele agli strumenti, la restituzione di errori dai quali il modello si riprende e la differenza tra un modello che rifiuta e una guardia che nega.
...
- [Costruire un agente che usa strumenti con il function calling](/guides/features/tool-using-agent)
