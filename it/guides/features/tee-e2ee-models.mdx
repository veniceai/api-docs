...
description: "Esegui inferenza privata su Venice con modelli Trusted Execution Environment (TEE) e crittografia end-to-end che rendono i prompt illeggibili all'host."
...
|------|--------|---------------|
| **TEE** | `e2ee-*` | Il modello viene eseguito in un'enclave protetta hardware. Venice non può accedere al calcolo. Puoi verificarlo con l'attestation. |
...
<Info>
Gli ID dei modelli TEE ed E2EE usano entrambi il prefisso `e2ee-*`. Questo perché ogni modello con attestation TEE su Venice è anche compatibile con E2EE — l'attestation TEE è il meccanismo che rende possibile la cifratura end-to-end — quindi condividono un'unica famiglia di modelli e un unico prefisso. Che si usi l'inferenza TEE semplice o il flusso E2EE completo, l'ID del modello è lo stesso. I modelli E2EE includono la protezione TEE più la cifratura lato client. I modelli TEE forniscono la sicurezza dell'enclave senza richiedere la cifratura lato client.
</Info>
