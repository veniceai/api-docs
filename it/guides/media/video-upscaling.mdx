...
description: "Migliora risoluzione e qualità dei video con il modello topaz-video-upscale sulla coda video asincrona di Venice, con fattori di upscaling 1x, 2x o 4x."
...
`video_url` è **obbligatorio** per i preventivi sui modelli di upscale — l'endpoint quote scarica il file e rileva durata, FPS e altezza dai byte, quindi non c'è bisogno di fornirli. Ometterlo restituisce `400` con *"video_url is required for quotes on this model"*.

<Warning>
Gli endpoint quote e queue attualmente denominano il fattore di upscale in modo diverso: `/video/queue` accetta `upscale_factor` (`1`, `2` o `4`), mentre `/video/quote` legge il fattore da `resolution` (`"2x"` o `"4x"`). Inviare `upscale_factor` all'endpoint quote non ha effetto — ricade sul default `"2x"` e restituisce un prezzo 2x per quello che potrebbe essere un job 4x. Quando richiedi un preventivo per un upscale 4x, passa `resolution: "4x"`.
</Warning>
...
| `"Insufficient balance"` | Crediti dell'account troppo bassi | Aggiungi crediti su [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) |
