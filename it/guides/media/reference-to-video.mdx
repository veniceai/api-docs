...
Questa funzionalità è disponibile sui modelli **Kling O3** e **Grok Imagine R2V** nel [Venice Video Studio](https://venice.ai/video?utm_source=venice-api-documentation). Ogni famiglia di modelli usa un approccio diverso alle immagini di riferimento — consulta le sezioni specifiche del modello qui sotto.
...
Puoi aggiungere fino a **4 element** per generazione. Richiamali nel tuo prompt usando `@Element1`, `@Element2`, ecc.
...
Ogni tipo di input ha un limite indipendente applicato dall'API:

| Limite | Valore |
|---|---|
| **Minimo richiesto** | Almeno 1 input visivo (start frame, element o scene image) |
| `elements` | **4 massimo** |
| `scene_image_urls` | 4 massimo |
| `reference_image_urls` per element | 1–3 |

<Note>
L'API applica questi limiti in modo indipendente — non esiste un budget di immagini combinato tra `elements`, `scene_image_urls` e i frame iniziale/finale, e aggiungere uno start o end frame non riduce quanti element puoi inviare. Superare un singolo limite restituisce un `400` che indica quel campo, ad esempio *"elements must have at most 4 items"*.
</Note>
...
Vai su [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). Nel Model Browser a sinistra, seleziona uno dei modelli **Kling O3 Reference to Video**:
...
Ripeti per ulteriori personaggi o oggetti (fino a 4 element in totale).

<Warning>
`elements` e `scene_image_urls` hanno ciascuno un limite di **4**, e i limiti si applicano in modo indipendente. Vedi [Limitazioni](#limitazioni) per i dettagli.
</Warning>
...
| Errore `"elements must have at most 4 items"` | Inviati più di 4 element | Invia al massimo 4 element; il limite non cambia quando è presente uno start o end frame |
| Errore `"scene_image_urls must have at most 4 images"` | Inviate più di 4 scene image | Invia al massimo 4 scene image. Questo limite è indipendente dal limite degli element |
...
| Durata | Qualsiasi secondo intero da 1s a 10s | 8s |
...
Vai su [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). Nel Model Browser, seleziona **Grok Imagine R2V**.
...
| `reference_image_urls` | string[] | **Sì** | 1–7 URL di immagini o data URL |
| `duration` | string | **Sì** | Qualsiasi secondo intero da `"1s"` a `"10s"`. Il suffisso `s` è obbligatorio |
| `aspect_ratio` | string | **Sì** | es. `"16:9"`, `"9:16"`, `"1:1"` |
| `resolution` | string | No | `"480p"` (default) o `"720p"` |

<Note>
Grok Imagine R2V non usa i campi `elements`, `scene_image_urls` o `image_url`. Tutte le immagini di riferimento vengono passate tramite `reference_image_urls`.
</Note>
...
| Errore "At least one reference image is required" | `reference_image_urls` è vuoto o mancante | Fornisci almeno un URL di immagine in `reference_image_urls` |
...
| Video troppo corto | La durata di default è 8s | Imposta la durata su `"10s"` per video più lunghi |
| Errore `"Invalid enum value ... received '8'"` | `duration` è stato inviato senza il suffisso `s` | Invia `"8s"`, non `"8"`. L'API richiede una corrispondenza esatta con la forma visualizzata |
