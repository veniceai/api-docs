---
title: Introduzione
description: "Riferimento dell'API Venice: autenticazione, debug, compatibilità con OpenAI, header di risposta, gestione errori ed endpoint supportati."
...
L'API Venice utilizza API key per l'autenticazione. Crea e gestisci le tue API key nelle tue [impostazioni API](https://venice.ai/settings/api?utm_source=venice-api-documentation).


La maggior parte delle richieste API richiede l'autenticazione HTTP Bearer. Alcuni endpoint di discovery — tra cui `GET /models`, `GET /models/traits`, `GET /models/compatibility_mapping` e `GET /image/styles` — sono leggibili senza una key:
...
Le key sono di tipo `INFERENCE` o `ADMIN`. Le key di inference possono chiamare gli endpoint di inferenza — chat, immagini, video, audio, embeddings e gli endpoint di augmentation. Gli endpoint di gestione dell'account richiedono una key `ADMIN`:

| Endpoint | Key richiesta |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`, `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | Entrambe |
| `GET /billing/balance`, `/billing/usage-history`, `/billing/usage-analytics` | `ADMIN` |

Chiamare un endpoint riservato agli admin con una key di inference restituisce `401` con `{"error":"Admin API key required"}`. Nota che `GET /api_keys/rate_limits` è l'unica eccezione in quel gruppo — funziona con entrambi i tipi di key.
...
<Warning>
Il base URL è `https://api.venice.ai/api/v1`, non `https://venice.ai`. Le richieste inviate al dominio del sito web raggiungono la web app invece dell'API e falliscono con errori come `405 Method Not Allowed`.
</Warning>
...
| `enable_web_citations` | boolean | Quando la web search è abilitata, richiede che l'LLM citi le fonti usando apici a caret — `^1^` per una singola fonte e `^1,2^` per più fonti. Le forme tra parentesi come `[1]`, `(1)` e `<sup>1</sup>` sono esplicitamente soppresse | `false` |
...
3. **Ecosistema dei modelli**: Venice offre la propria [lineup di modelli](/overview/models) inclusi modelli senza restrizioni e di ragionamento - usa gli ID dei modelli Venice anziché le mappature OpenAI
...
6. **Input degli embeddings**: `input` deve essere una stringa o un array di stringhe. Gli array di token ID vengono rifiutati con HTTP 400. `OpenAIEmbeddings` di LangChain deve impostare `check_embedding_ctx_length=False`
