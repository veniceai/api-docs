...
- I modelli basati su pixel accettano `width` e `height`, ad esempio `venice-sd35`.
- I modelli basati su aspect ratio accettano `aspect_ratio`, ad esempio `qwen-image-2`. La famiglia Qwen Image — `qwen-image`, `qwen-image-3` e `qwen-image-3-pro` — **rifiuta** `width` e `height` con un `400`; per questi modelli usa `aspect_ratio`.
