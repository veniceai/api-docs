---
title: "Building a Voice Agent in Python"
description: "Build a terminal voice agent on Venice with streamed speech-to-text, chat, and text-to-speech."
slug: voice-agent
"og:title": "Building a Voice Agent in Python with Venice"
...
PipeWire is the only awkward requirement, and it's only for local recording and playback rather than anything Venice-facing. The app takes a `--text-only` flag that skips it entirely and still exercises chat and TTS, so you can follow along on a machine without a working mic.
...
The split matters more than it looks. `venice.py` is the portable part that you can lift into a web app, a Discord bot, or a phone integration. `audio.py` is Linux-specific plumbing that the API never sees — Venice only ever receives a WAV blob on the way in and hands back raw PCM on the way out.

## Setting Up

Create the project and add the two dependencies. The OpenAI SDK does all the HTTP work, and `python-dotenv` keeps the key out of your shell history:

```bash
uv init venice-voice-agent
cd venice-voice-agent
uv add "openai>=1.60" "python-dotenv>=1.0"
uv add --dev "pytest>=8"
```
...
# Optional PipeWire node names, if the defaults pick the wrong device:
# AUDIO_SOURCE=
# AUDIO_SINK=
```
...
This part is not Venice, so we'll move quickly. `audio.py` shells out to PipeWire: `pw-record` writes a temp WAV while the user talks, and `pw-play --raw` accepts concatenated PCM on stdin.

Recording until Enter, with a hard cap so a forgotten session doesn't fill the disk:
...
def record_until_enter() -> bytes:
    """Record 16 kHz mono WAV until Enter, a 30s cap, or cancel."""
    require_pipewire()
    handle = tempfile.NamedTemporaryFile(
        prefix="venice-voice-", suffix=".wav", delete=False
    )
    path = Path(handle.name)
    handle.close()
    try:
        command = [
            "pw-record", "--channels=1", f"--rate={RECORD_RATE}", "--format=s16",
        ]
        ...
        if len(recorded) <= WAV_HEADER_BYTES:
            raise AudioError(
                "That recording was empty. "
                "Press Enter, speak, then press Enter again."
            )
        return recorded
    finally:
...
That `try/finally` around the whole body is the bit worth copying. The temp file is deleted whether the turn succeeds, errors, or gets cancelled, so a recording of your voice never outlives the request that needed it. `start_new_session=True` keeps the recorder out of the terminal's process group, so Ctrl+C reaches our handler rather than killing `pw-record` mid-write.

Playback is one long-lived process per reply, so consecutive sentences run together as continuous speech instead of restarting the sink each time:
...
class PcmPlayer:
    ...
    def start(self) -> None:
        ...
        command = _play_command(
            "--raw", "--channels=1", f"--rate={self.sample_rate}", "--format=s16", "-",
        )
...
The full class in the repo also has `abort()` for Ctrl+C — terminate immediately, don't drain — and `close()` for the normal path, which closes stdin and waits for the sink to finish playing what it already has. Getting those two backwards means either cutting the last word off every reply or being unable to interrupt one.

<Note>
  This half is Linux-specific. On macOS or Windows, replace `audio.py` with `sounddevice`, `pyaudio`, or whatever your platform prefers. Nothing in `venice.py` changes.
</Note>
...
The player is created lazily on the first chunk of audio rather than up front, so a TTS failure doesn't leave an empty `pw-play` process holding the sink. And `raise_on_error=not failed` means that when the turn is already failing we tear playback down quietly instead of stacking a second error on top of the real one.
