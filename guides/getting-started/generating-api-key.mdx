...
To resolve the error, either raise or remove the key's Epoch Consumption Limits at [venice.ai/settings/api](https://venice.ai/settings/api), or call [Update API Key](/api-reference/endpoint/api_keys/update) with new `consumptionLimits`. Account-wide balances (USD, DIEM, and bundled credits) are independent payment sources and are consumed automatically as available; you do not need to choose one at request time.
