...
description: "Generate images from text prompts with the Venice API, covering model sizing options, style presets, LoRA strength, safe mode, and binary output."
...
| `lora_strength` | integer | No | - | Strength (`0`-`100`) of the LoRAs built into the model; ignored by models without additional LoRAs |
...
Some image models ship with additional LoRAs built in. The `lora_strength` parameter (integer, `0` to `100`) controls how strongly those built-in LoRAs are applied. It only affects models that use additional LoRAs. Other models ignore it.

The API does not support uploading or loading custom LoRA files. There is also no endpoint that lists the LoRAs bundled with a model. LoRA usage is limited to adjusting `lora_strength` on models that include them.
...
Some image models are specifically designed for adult / uncensored generation and pair well with `safe_mode: false`. Examples on the pricing page include `lustify-sdxl`, `lustify-v7`, and `lustify-v8`. See [Image Models](/models/image) and [Pricing](/overview/pricing) for the current list and per-model costs.

For editing existing images, the default edit model `qwen-edit` blocks explicit sexual imagery. If you need uncensored editing, use `qwen-edit-uncensored` via [Image Editing](/guides/media/image-editing).
...
### Controlling adult-content blur on `/images/generations`

The OpenAI-compatible endpoint does not accept `safe_mode`. Sending it returns a `400` with `Unrecognized key(s) in object: 'safe_mode'`. Use the OpenAI-style `moderation` parameter instead:

| Value | Behavior |
|---|---|
| `auto` (default) | Enables Safe Venice mode and blurs adult content |
| `low` | Disables Safe Venice mode; returns unblurred output |
...
`safe_mode` only works on the native `POST /image/generate` endpoint. `moderation` only works on the OpenAI-compatible `POST /images/generations` endpoint.
