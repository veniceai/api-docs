...
<Warning>
**Deprecated:** `topaz-video-upscale` is retiring. Use the Topaz enhancement suite instead — `topaz-video-precision` is the direct replacement (same Proteus default, 2x). See the [Video Enhancement Guide](/guides/media/video-enhancement) for the full model suite and settings.
</Warning>
