...
description: "Generate videos from text or images on Venice's async queue API: quote pricing, submit to /video/queue, and poll /video/retrieve for the MP4."
...
Unlike image generation, the video queue endpoint does not accept a `safe_mode` parameter. Adult content support is model-specific: choose a model variant that is designed for uncensored output. The Wan 2.7 family, for example, exposes Enhanced variants alongside the default variants:

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

Requests submitted to the default variants may be rejected as content violations (`422`). See [Video Models](/models/video) and [Pricing](/overview/pricing) for the current list of variants and their per-second pricing.
