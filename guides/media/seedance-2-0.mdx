...
description: "Generate, edit, extend, and stitch videos with Seedance 2.0 and 2.5 on Venice - text, image, and reference-to-video workflows, faces-off API behavior, and family-specific multimodal limits."
...
<Warning>
**Faces are not supported on the public Seedance API.** Venice does not run face-asset provisioning or face-consent attestation (`consents.seedance` / `needs_consent`) for public Seedance models. Face-bearing inputs may be rejected upstream as a content-policy / provider error. For likeness / face-asset generation, use the Venice app or Studio.
</Warning>
