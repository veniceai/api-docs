...
BytePlus classifies Seedance 2.5 all-modal reference jobs as **reference**, **edit**, or **extend** from the prompt. You can optionally pre-guide that classification on `/video/queue` with:

| Value | Meaning |
| --- | --- |
| `auto` | Let BytePlus classify from the prompt |
| `reference` | Multimodal reference generation (new clip guided by donors) |
| `edit` | Edit an existing clip (pairs with `duration: "auto"` / `"-1"` and usually `aspect_ratio: "adaptive"`) |
| `extend` | Extend an existing clip (fixed `duration` for how long to generate; usually `aspect_ratio: "adaptive"`) |

Aliases `editing`→`edit` and `extension`→`extend` are accepted. When omitted, Venice infers the value from the prompt if `reference_video_urls` are present. The prompt must still match the chosen type — a conflict can still fail asynchronously.
...
`omni_reference_task_type: "edit"` pre-guides BytePlus classification. `duration: "auto"` (or `"-1"`) and `aspect_ratio: "adaptive"` (or `"auto"`) make the output follow the source clip. See [Source-matched aspect ratio and duration](#source-matched-aspect-ratio-and-duration) and [`omni_reference_task_type`](#omni_reference_task_type-seedance-25-r2v).
