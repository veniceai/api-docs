...
In this tutorial, we'll build a private research agent using Python and the Venice API. By the end, you'll have a CLI that can research a topic, scrape public pages into Markdown, summarize source chunks, run follow-up research passes, and generate a cited report with optional local JSONL artifacts.
...
| Research agent | Plans searches, reads sources, extracts evidence, generates follow-up queries, and writes the final report |
| Artifact writer | Stores auditable JSONL records for queries, results, fetches, chunks, source notes, errors, and reports |
...
We use `VENICE_MODEL` so you can change the model without editing code. `zai-org-glm-5` is a good general-purpose default, but you can swap it for another chat model available to your Venice account.
...
After each research pass, the agent asks Venice to look at the current source notes and create better follow-up queries:
...
That staged approach is useful when you want long-form research output because one giant prompt often compresses too much. The tradeoff is latency and cost, since the agent makes more model calls.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", help="Venice model name. Defaults to VENICE_MODEL or zai-org-glm-5.")
...
These files are useful when you want to understand how the agent reached a conclusion. For example, `source_notes.jsonl` shows the summarized source evidence, while `errors.jsonl` shows pages that failed during search, scraping, or summarization.
