...
<AuthorByline name="Joshua Mo" date="16 June 2026"/>
...
![Architecture diagram showing a client calling the Rust gateway, Postgres, Venice AI, and OpenTelemetry](/images/guides/llm-gateway-architecture.svg)
...
There are three design choices worth calling out before we start writing code.

The first is **why the gateway keeps an OpenAI-compatible API**. If your internal clients already know how to call `/v1/chat/completions`, the gateway should not force every app to learn a new request shape. We only validate the fields the gateway needs for policy and telemetry, then pass the rest of the JSON through to Venice. That keeps the gateway useful without turning it into a second model API.

The second is **why API keys live in Postgres**. A production gateway needs to revoke keys, change per-key limits, and attribute usage to a subject. Storing a keyed record gives us that control. Storing only a prefix and hash means the database is useful for lookup without becoming a plaintext credential store.

The third is **why streaming is handled explicitly**. Non-streaming proxying can be as simple as "send JSON upstream, return JSON downstream." Streaming is different because clients expect server-sent events to arrive incrementally. The gateway has to relay chunks while still observing first-token latency, finish reasons, token usage, and billing metadata when those details appear.
...
Add the dependencies we need in `Cargo.toml`:
...
There are a few choices worth calling out:

- `axum` gives us a small, async HTTP server without hiding the request and response types we need for proxying.
- `reqwest` calls the Venice API, and the `stream` feature lets us handle upstream responses without buffering everything first.
- `reqwest-eventsource` lets us consume upstream server-sent events for streaming responses. That keeps the gateway compatible with SDKs that expect `stream: true` to behave like an OpenAI-style SSE stream.
- `sqlx` handles Postgres migrations and queries. We want the database layer to be explicit because auth and rate limits are policy decisions, not incidental storage.
- `sha2` and `subtle` let us store hashed API keys and compare them in constant time.
- `opentelemetry` and `tracing-opentelemetry` give us provider-level observability without coupling the rest of the code to one tracing backend.

This is more dependency-heavy than a minimal Axum "hello world", but each crate maps to a real gateway responsibility: HTTP serving, upstream proxying, persistent policy, streaming, and telemetry.

## Loading configuration

The gateway reads everything from environment variables. For infrastructure code like this, environment variables are a good default because the same binary can run locally, in Docker Compose, or in a hosted environment without a separate config file format.
...
Create `src/venice.rs`. The client only needs to know the upstream chat completions URL, the Venice API key, and how many times to retry transient failures.

Keeping this wrapper small is intentional. The gateway should not reimplement Venice's API surface. Its job is to attach the server-side credential, apply a timeout, retry requests that are safe to retry, and return the upstream response in a form the router can forward.
