...
description: "Let Venice chat models call your tools with OpenAI-compatible function calling: define a tools array, execute calls, and return results."
...
The example above stops after one round trip. Real agents keep going, because the model usually needs several tools before it can answer. [Building a Tool-Using Agent with Function Calling](/guides/features/tool-using-agent) works through the whole loop against a database the model has never seen, including parallel tool calls, returning errors the model recovers from, and the difference between a model that declines and a guard that refuses.
...
- [Building a Tool-Using Agent with Function Calling](/guides/features/tool-using-agent)
