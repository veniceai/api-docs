...
Venice uses Chat Completions for text generation and does not currently expose OpenAI's Responses API. Structured responses use `response_format` on `POST /chat/completions`.
