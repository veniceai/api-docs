...
|------|--------|---------------|
| **TEE** | `e2ee-*` | Model runs in a hardware-secured enclave. Venice cannot access the computation. You can verify this with attestation. |
...
<Info>
Both TEE and E2EE model IDs use the `e2ee-*` prefix. This is because every TEE-attested model on Venice is also E2EE-capable—TEE attestation is the mechanism that makes end-to-end encryption possible—so they share a single model family and prefix. Whether you use plain TEE inference or the full E2EE flow, the model ID is the same. E2EE models include TEE protection plus client-side encryption. TEE models provide enclave security without requiring client-side encryption.
</Info>
