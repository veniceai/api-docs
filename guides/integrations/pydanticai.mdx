...
<Note>
Use `OpenAIChatModel` (Chat Completions), not `OpenAIResponsesModel` / the `openai:` shorthand. Venice does not currently expose OpenAI's Responses API; its text-generation path is `/chat/completions`. Pinning Chat Completions avoids Responses-only behavior that OpenAI-default models use in newer PydanticAI versions.
</Note>
