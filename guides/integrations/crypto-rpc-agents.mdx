...
<Steps>
  ...
  <Step title="Mint an API key">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` returns a single-use EIP-4361 challenge, bound to that wallet, which expires 15 minutes after issuance. Sign the `message` verbatim with the staking wallet, then `POST` the address, signature, and message back. Venice returns an API key bound to the user account derived from that wallet.
  </Step>
</Steps>
