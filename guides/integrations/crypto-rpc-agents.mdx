...
description: "Use Venice as both model provider and blockchain RPC layer for autonomous agents: one API key, 230+ models, and JSON-RPC across 11 chains."
...
|---|---|---|
| **API key** | Server-side agents, fixed deployments | `Authorization: Bearer <key>` header. Get a key at [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). |
...
<Tip>
Truly autonomous agents can mint their own API key by staking VVV on Base. See [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent).
</Tip>
...
The list above is a snapshot. Call [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks) — a public endpoint that needs no authentication — for the live, authoritative list.
...
| **USD or crypto top-up via the dashboard** | No (browser) | Sign into [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) with the same wallet (Sign-In-With-Ethereum), then add credits in Settings, API. Both Stripe (card) and Coinbase (crypto) live behind that page and require a browser. Credits never expire. |
...
See [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent) for the complete walkthrough with code and the full error reference.
