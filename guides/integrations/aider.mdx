---
title: Aider
description: "Configure Aider for terminal AI pair programming with private Venice chat models via the OpenAI-compatible base URL and an openai/ model prefix."
...
| --------------------------- | ---------------------- | ------------------------------------- |
| Strong coding / refactoring | `z-ai-glm-5-3-flash`   | `--model openai/z-ai-glm-5-3-flash`   |
...
<AccordionGroup>
  <Accordion title="401 or authentication error">
    Confirm `OPENAI_API_KEY` is a Venice key from [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation), with no extra whitespace. On Windows after `setx`, restart the terminal.
  </Accordion>
  <Accordion title="Model not found or empty response">
    Confirm the model ID exists in the [text model catalog](/models/text) and that you launched Aider with the `openai/` prefix, for example `openai/z-ai-glm-5-3-flash`.
  </Accordion>
