---
title: Kilo Code
description: "Configure Kilo Code's native Venice provider in VS Code or the CLI for agentic coding with reasoning models and a shared kilo.json config."
...
- A Venice API key from [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)
  ...
  <Tab title="VS Code">
    <Steps>
      ...
      <Step title="Get your Venice API key">
        Generate a key from [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). Copy it immediately and store it securely.
      </Step>
  ...
  <Tab title="CLI">
    <Steps>
      ...
      <Step title="Configure the Venice provider">
        ...
        Replace `z-ai-glm-5-3-flash` with any model ID from the [text model catalog](/models/text). Kilo model references use the `venice/<model-id>` format.
      </Step>
...
Copy a model's `id` exactly into Kilo. In CLI config, prefix it with `venice/`, for example `venice/z-ai-glm-5-3-flash`.
...
<AccordionGroup>
  <Accordion title="Invalid API key or 401">
    Confirm the key comes from [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). For the CLI, verify `VENICE_API_KEY` is set in the same environment that launches Kilo. In VS Code, reconnect Venice AI under **Providers**.
  </Accordion>
