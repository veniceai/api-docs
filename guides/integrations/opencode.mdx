---
title: OpenCode
description: "Wire OpenCode to Venice via a custom provider in opencode.json so your terminal coding agent runs on private models with one config file."
...
If you set `"model": "venice/z-ai-glm-5-3-flash"` as shown above, OpenCode should use that Venice model by default. You can change the default by replacing `z-ai-glm-5-3-flash` with any model ID from the [text model catalog](/models/text) and adding it to the `models` map.
