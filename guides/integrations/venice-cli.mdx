---
title: Venice CLI
description: "Official Venice CLI for chat, web search, image generation, text to speech, transcription, and video from your terminal, with optional E2EE."
...
<Steps>
  ...
  <Step title="Get a Venice API key">
    Generate one from [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). See the [API key guide](/guides/getting-started/generating-api-key) for step-by-step instructions.
  </Step>
...
### Troubleshooting: `venice` command not found

If your terminal reports that `venice` is not recognized after `npm install -g veniceai-cli`, npm's global bin folder is not on your `PATH`. This is common on Windows.

1. Run `npm config get prefix` to find npm's global install folder.
2. Add the global bin folder to your `PATH`. On Windows, this is the prefix folder itself (usually `%AppData%\npm`). On macOS and Linux, it is the `bin` subfolder of the prefix.
3. Open a new terminal window and run `venice chat "Hello"`.

To skip `PATH` setup, run the CLI through `npx` instead, for example `npx veniceai-cli chat "Hello"`.
