...
Verwenden Sie den `id`-Wert als `model`-Parameter in API-Anfragen. Derzeit sind 62 Modelle verfügbar.
...
<Tip>
**Qualitätsstufen.** `gpt-image-2` (Generate) und `gpt-image-2-edit` (Edit/Multi-Edit) akzeptieren einen optionalen `quality`-Parameter — `low`, `medium` oder `high` (Standard). Niedrigere Stufen kosten weniger; die vollständige Matrix finden Sie in der [Preisübersicht](/de/overview/pricing#image-generation), oder Sie rufen `model_spec.pricing.quality` über den [Models-Endpoint](/de/api-reference/endpoint/models/list) ab.
</Tip>
