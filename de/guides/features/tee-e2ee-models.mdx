...
description: "Private Inferenz auf Venice mit Trusted Execution Environment (TEE)-Modellen und Ende-zu-Ende-Verschlüsselung — Prompts bleiben für den Host unlesbar."
...
|------|--------|---------------|
| **TEE** | `e2ee-*` | Modell läuft in einer hardware-gesicherten Enklave. Venice kann nicht auf die Berechnung zugreifen. Du kannst das per Attestation verifizieren. |
...
<Info>
Sowohl TEE- als auch E2EE-Modell-IDs verwenden den Präfix `e2ee-*`. Der Grund: Jedes TEE-attestierte Modell auf Venice ist zugleich E2EE-fähig – die TEE-Attestation ist der Mechanismus, der die Ende-zu-Ende-Verschlüsselung überhaupt möglich macht – deshalb teilen sie sich eine gemeinsame Modellfamilie und einen gemeinsamen Präfix. Egal ob du einfache TEE-Inferenz oder den vollständigen E2EE-Flow nutzt, die Modell-ID ist dieselbe. E2EE-Modelle umfassen den TEE-Schutz plus clientseitige Verschlüsselung. TEE-Modelle bieten Enklaven-Sicherheit ohne clientseitige Verschlüsselung.
</Info>
