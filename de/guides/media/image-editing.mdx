...
description: "Bilder bearbeiten, inpainten und Hintergründe entfernen mit der Venice API – über /image/edit, Multi-Edit-Layer, Masken und Safe-Mode-Steuerung."
...
Die Bildbearbeitung auf Venice ist synchron. Sende dein Quellbild an `/image/edit` oder `/image/multi-edit`, und das bearbeitete Ergebnis kommt in derselben Antwort als rohe Bilddatei zurück. Für Freisteller liefert `/image/background-remove` ein transparentes PNG.
...
<Note>
Venice bietet keinen OpenAI-kompatiblen `/images/edits`-Endpoint. Der einzige OpenAI-kompatible Image-Pfad ist `POST /images/generations` für die Generierung. Für die Bearbeitung nutze den nativen Endpoint `POST /image/edit`.
</Note>
...
**Antwort (200):**
Der Response-Body sind rohe Bild-Binärdaten. Speichere sie direkt in eine Datei. Siehe [Antwortformat](#antwortformat) dazu, wie `output_format` den Dateityp steuert.
...
Wie bei `/image/edit` sind die Response-Body rohe Bilddaten.
...
`/image/edit` und `/image/multi-edit` geben das bearbeitete Bild immer als rohe Binärdaten zurück (`image/png`, `image/jpeg` oder `image/webp`). Die Edit-Endpoints akzeptieren keinen `response_format`-Parameter. Es gibt keine `b64_json`- oder `url`-Option wie bei `/images/generations`. Wenn du das Ergebnis in JSON brauchst, kodiere die Binärantwort in deinem eigenen Code als Base64.

Steuere den Dateityp mit `output_format`:

- `output_format` akzeptiert `jpeg`, `jpg`, `png` oder `webp`.
- Wird der Parameter weggelassen, wird das Format aus `resolution` abgeleitet: PNG für 1K-Edits und JPEG für 2K- und 4K-Edits.
...
| `resolution` | string | Nein | `1K` | Auflösungsstufe für das Ausgabebild (`1K`, `2K`, `4K`); unterstützte Werte variieren je nach Modell |
| `output_format` | string | Nein | aus `resolution` abgeleitet | Ausgabedateityp: `jpeg`, `jpg`, `png` oder `webp` |
...
|---|---|---|---|
| `/image/edit` | Base64-String oder URL | Datei-Upload | `image/png`, `image/jpeg` oder `image/webp` je nach `output_format` |
| `/image/multi-edit` | Base64-Strings oder URLs | Datei-Uploads | `image/png`, `image/jpeg` oder `image/webp` je nach `output_format` |
