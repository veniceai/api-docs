...
description: "Generieren Sie Videos aus Text oder Bildern über Venices asynchrone Queue-API: Preis quotieren, an /video/queue senden und die MP4 per /video/retrieve abrufen."
...
| `prompt` | string | Ja | - | Was generiert werden soll. Die maximale Länge ist modellspezifisch und variiert stark – von einigen hundert Zeichen bis zu 20.000. Modelle, die kein eigenes Limit deklarieren, verwenden standardmäßig 2.500. Lies `model_spec.constraints.prompt_character_limit` aus `GET /models?type=video`, statt von einem einheitlichen Limit auszugehen |
...
| `bitrate_mode` | string | Nein | `"standard"` | Nur Seedance 2.0 / 2.5. `"standard"` oder `"high"`. `high` fordert einen Encode mit höherer Qualität und größerer Datei an. Ändert den Preis nicht. Bei anderen Familien weglassen |
...
Anders als bei der Bildgenerierung akzeptiert der Video-Queue-Endpoint keinen `safe_mode`-Parameter. Support für Adult-Inhalte ist modellspezifisch: wähl eine Modellvariante, die für unzensierte Ausgabe ausgelegt ist. Die Wan-2.7-Familie bietet zum Beispiel Enhanced-Varianten neben den Default-Varianten:

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

Requests an die Default-Varianten können als Content-Verstoß (`422`) abgelehnt werden. Die aktuelle Liste der Varianten und ihre Preise pro Sekunde findest du unter [Video-Modelle](/models/video) und [Pricing](/overview/pricing).
