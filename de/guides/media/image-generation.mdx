...
description: "Generieren Sie Bilder aus Text-Prompts mit der Venice API – mit Modell-Größenoptionen, Style-Presets, LoRA-Stärke, Safe Mode und Binärausgabe."
...
<Warning>
`qwen-image`, `qwen-image-3` und `qwen-image-3-pro` akzeptieren `width` oder `height` nicht mehr. Das Senden eines der beiden gibt `400` mit *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."* zurück. Prüfe `model_spec.constraints` aus `GET /models?type=image`, bevor du die Größenfelder wählst.
</Warning>
...
Die Style-Namen befinden sich in `data`, nicht auf der obersten Ebene – lies `response.data`, statt direkt über das Response-Objekt zu iterieren.
...
| `width` | integer | Nein | `1024` | Ausgabebreite in Pixeln für Pixel-basierte Modelle wie `venice-sd35`. Wird von der Qwen-Image-Familie abgelehnt – siehe Hinweis unten |
| `height` | integer | Nein | `1024` | Ausgabehöhe in Pixeln für Pixel-basierte Modelle wie `venice-sd35`. Wird von der Qwen-Image-Familie abgelehnt – siehe Hinweis unten |
...
| `lora_strength` | integer | Nein | - | Stärke (`0`–`100`) der ins Modell eingebauten LoRAs; wird von Modellen ohne zusätzliche LoRAs ignoriert |
...
Manche Image-Modelle bringen zusätzliche, fest eingebaute LoRAs mit. Der Parameter `lora_strength` (integer, `0` bis `100`) steuert, wie stark diese eingebauten LoRAs angewendet werden. Er wirkt nur bei Modellen, die zusätzliche LoRAs verwenden. Andere Modelle ignorieren ihn.
...
| Wert | Verhalten |
|---|---|
| `auto` (Default) | Aktiviert den Safe-Venice-Modus und macht Adult-Inhalte unscharf |
| `low` | Deaktiviert den Safe-Venice-Modus; liefert die Ausgabe ohne Unschärfe |
