...
`video_url` ist für Quotes auf Upscale-Modellen **erforderlich** – der Quote-Endpoint ruft die Datei ab und ermittelt Dauer, FPS und Höhe aus den Bytes, sodass du sie nicht angeben musst. Wird es weggelassen, kommt `400` mit *"video_url is required for quotes on this model"* zurück.

<Warning>
Die Quote- und Queue-Endpoints benennen den Upscale-Faktor derzeit unterschiedlich: `/video/queue` nimmt `upscale_factor` (`1`, `2` oder `4`), während `/video/quote` den Faktor aus `resolution` (`"2x"` oder `"4x"`) liest. Das Senden von `upscale_factor` an den Quote-Endpoint hat keine Wirkung – er fällt auf den Default `"2x"` zurück und gibt einen 2x-Preis für einen möglicherweise 4x-Job zurück. Übergib beim Quoten eines 4x-Upscales `resolution: "4x"`.
</Warning>
...
| `"Insufficient balance"` | Kontoguthaben zu niedrig | Credits unter [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) aufladen |
