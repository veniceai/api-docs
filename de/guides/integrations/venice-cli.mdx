---
title: Venice CLI
description: "Offizielle Venice CLI für Chat, Websuche, Bildgenerierung, Text-to-Speech, Transkription und Video direkt im Terminal, mit optionaler E2EE."
...
<Steps>
  ...
  <Step title="Venice API-Schlüssel besorgen">
    Erstelle einen unter [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). Schritt-für-Schritt-Anleitung im [API-Schlüssel-Guide](/guides/getting-started/generating-api-key).
  </Step>
...
### Fehlerbehebung: Befehl `venice` nicht gefunden

Wenn dein Terminal nach `npm install -g veniceai-cli` meldet, dass `venice` nicht erkannt wird, liegt der globale bin-Ordner von npm nicht in deinem `PATH`. Das kommt unter Windows häufig vor.

1. Führe `npm config get prefix` aus, um den globalen Installationsordner von npm zu finden.
2. Füge den globalen bin-Ordner zu deinem `PATH` hinzu. Unter Windows ist das der Präfix-Ordner selbst (meist `%AppData%\npm`). Unter macOS und Linux ist es der Unterordner `bin` des Präfixes.
3. Öffne ein neues Terminalfenster und führe `venice chat "Hello"` aus.

Um die `PATH`-Einrichtung zu überspringen, führe die CLI stattdessen über `npx` aus, zum Beispiel `npx veniceai-cli chat "Hello"`.
