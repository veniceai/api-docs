...
description: "Nutzen Sie Venice sowohl als Modellanbieter als auch als Blockchain-RPC-Layer für autonome KI-Agenten — ein Credential, 230+ Modelle, 11 Chains, kein MetaMask."
...
<Steps>
  ...
  <Step title="API-Schlüssel ausstellen">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` liefert eine einmalig verwendbare, an diese Wallet gebundene EIP-4361-Challenge, die 15 Minuten nach Ausstellung abläuft. Die `message` unverändert mit der Staking-Wallet signieren, dann Adresse, Signatur und Nachricht per `POST` zurücksenden. Venice gibt einen API-Schlüssel zurück, der an das von dieser Wallet abgeleitete User-Konto gebunden ist.
  </Step>
</Steps>
