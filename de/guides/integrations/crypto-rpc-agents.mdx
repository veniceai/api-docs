...
|---|---|---|
| **API-Schlüssel** | Serverseitige Agenten, feste Deployments | Header `Authorization: Bearer <key>`. Schlüssel unter [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) erhalten. |
...
<Tip>
Wirklich autonome Agenten können sich durch Staking von VVV auf Base ihren eigenen API-Schlüssel ausstellen lassen. Siehe [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent).
</Tip>
...
Die obige Liste ist eine Momentaufnahme. Rufe [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks) — einen öffentlichen Endpunkt, der keine Authentifizierung erfordert — für die aktuelle, maßgebliche Liste auf.
...
| **USD- oder Crypto-Top-up via Dashboard** | Nein (Browser) | Mit derselben Wallet bei [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) einloggen (Sign-In-With-Ethereum) und in „Settings", „API" Credits hinzufügen. Sowohl Stripe (Karte) als auch Coinbase (Crypto) liegen hinter dieser Seite und benötigen einen Browser. Credits verfallen nicht. |
...
Siehe [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent) für die vollständige Anleitung mit Code und der ausführlichen Fehler-Referenz.
...
<CardGroup cols={2}>
  ...
  <Card title="Autonomer Agent-API-Schlüssel" icon="robot" href="/guides/integrations/generating-api-key-agent">
