...
<Note>
Verwende `OpenAIChatModel` (Chat Completions), nicht `OpenAIResponsesModel` / die `openai:`-Kurzschreibweise. Venice stellt OpenAIs Responses API derzeit nicht bereit; der Textgenerierungspfad ist `/chat/completions`. Wenn du Chat Completions festlegst, vermeidest du Responses-spezifisches Verhalten, das OpenAI-Standardmodelle in neueren PydanticAI-Versionen nutzen.
</Note>
