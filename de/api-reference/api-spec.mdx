---
title: Einführung
description: "Venice API-Referenz: Authentifizierung, Debugging, OpenAI-Kompatibilität, Response-Header, Fehlerbehandlung und alle unterstützten Endpunkte."
...
Die Venice API verwendet API-Schlüssel zur Authentifizierung. Erstellen und verwalten Sie Ihre API-Schlüssel in Ihren [API-Einstellungen](https://venice.ai/settings/api?utm_source=venice-api-documentation).


Die meisten API-Anfragen erfordern HTTP-Bearer-Authentifizierung. Einige Discovery-Endpunkte — darunter `GET /models`, `GET /models/traits`, `GET /models/compatibility_mapping` und `GET /image/styles` — sind ohne Schlüssel lesbar:
...
Schlüssel sind entweder `INFERENCE` oder `ADMIN`. Inference-Schlüssel können die Inferenz-Endpunkte aufrufen — Chat, Bild, Video, Audio, Embeddings und die Augmentierungs-Endpunkte. Kontoverwaltungs-Endpunkte erfordern einen `ADMIN`-Schlüssel:

| Endpunkt | Erforderlicher Schlüssel |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`, `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | Beide |
| `GET /billing/balance`, `/billing/usage-history`, `/billing/usage-analytics` | `ADMIN` |

Der Aufruf eines Admin-only-Endpunkts mit einem Inference-Schlüssel gibt `401` mit `{"error":"Admin API key required"}` zurück. Beachten Sie, dass `GET /api_keys/rate_limits` die einzige Ausnahme in dieser Gruppe ist — er funktioniert mit beiden Schlüsseltypen.
...
<Warning>
Die Base-URL lautet `https://api.venice.ai/api/v1`, nicht `https://venice.ai`. Anfragen an die Website-Domain erreichen die Web-App statt der API und schlagen mit Fehlern wie `405 Method Not Allowed` fehl.
</Warning>
...
| `enable_web_citations` | boolean | Wenn Web-Search aktiviert ist, fordert dies das LLM auf, seine Quellen mit hochgestellten Caret-Zeichen zu zitieren — `^1^` für eine einzelne Quelle und `^1,2^` für mehrere. Klammerformen wie `[1]`, `(1)` und `<sup>1</sup>` werden explizit unterdrückt | `false` |
...
6. **Embeddings-Eingabe**: `input` muss ein String oder ein Array von Strings sein. Token-ID-Arrays werden mit HTTP 400 abgelehnt. Bei LangChain `OpenAIEmbeddings` muss `check_embedding_ctx_length=False` gesetzt werden
