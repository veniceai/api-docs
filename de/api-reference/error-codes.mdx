...
Wenn ein Fehler auftritt, gibt die Venice-API einen HTTP-Statuscode und eine beschreibende Meldung zurück. Einige Fehler enthalten zusätzliche Felder, etwa zum Erstattungsstatus, zu vorgeschlagenen Wiederholungsmodellen, Validierungsdetails oder Request-IDs.

## Format der Fehlerantwort

Die meisten Fehler geben ein `error`-Feld mit einer für Menschen lesbaren Meldung zurück:
...
Einige Endpoints enthalten zusätzliche Felder, die spezifisch für den jeweiligen Endpoint sind. Video-Moderationsfehler können beispielsweise mitteilen, ob Guthaben zurückerstattet wurden:
...
Validierungsfehler können `details`- und `issues`-Felder mit Validierungsinformationen auf Feldebene enthalten:
...
Einige OpenAI-kompatible Endpoints können in bestimmten Fällen OpenAI-Stil-Fehlerobjekte zurückgeben, etwa bei Fehlern wegen Kontextlänge:
...
| Fehlercode | HTTP-Status | Meldung | Log-Level |
| --- | --- | --- | --- |
