...
<Info>
**API-Credits werden in USD ausgewiesen.** $1 USD entspricht $1 an API-Credits — es gibt keinen separaten Token, Multiplikator oder Umrechnungskurs. Credits verfallen nie.

Die API-Abrechnung ist **getrennt** von den venice.ai-Consumer-Abonnements (Pro, Plus, Max). Diese Pläne decken die Nutzung in der venice.ai-App ab und beinhalten **keine** laufende API-Nutzung. Siehe [Pro-Nutzer](#pro-nutzer) unten für das einmalige API-Credit, das bei einem Pro-Upgrade enthalten ist.
</Info>
...
<Note>
Die venice.ai-Consumer-Abonnements (Pro, Plus, Max) decken die Nutzung **innerhalb der venice.ai-App** ab — sie beinhalten kein wiederkehrendes API-Credit-Kontingent. API-Anfragen werden zu den oben angegebenen Modellsätzen von Ihrem USD- oder DIEM-Guthaben abgebucht, unabhängig davon, welchen Consumer-Plan Sie haben. Das einmalige Guthaben von $10 beim Pro-Upgrade ist das einzige API-Kontingent, das in einem Abonnement enthalten ist.
</Note>
