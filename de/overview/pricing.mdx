---
title: API-Preise
description: "Preise der Venice API pro 1M Tokens für Text-, Bild-, Video-, Audio- und Embedding-Modelle sowie Credit-Kosten für Web-Suche, RPC und weitere Dienste."
...
| Model | ID | 2× Upscale | 4× Upscale |
|---|---|---|---|
| Image Upscaler | `upscaler` | $0.02 | $0.08 |

#### Bearbeitung

| Model | ID | Pro Bearbeitung |
|---|---|---|
...
| LTX Video 2.0 19B | `ltx-2-19b-full-text-to-video` | Text to Video | Variabel | Private |
| LTX Video 2.0 19B | `ltx-2-19b-full-image-to-video` | Image to Video | Variabel | Private |
| LTX Video 2.0 19B Distilled | `ltx-2-19b-distilled-text-to-video` | Text to Video | Variabel | Private |
| LTX Video 2.0 19B Distilled | `ltx-2-19b-distilled-image-to-video` | Image to Video | Variabel | Private |
| LTX Video 2.0 Fast | `ltx-2-fast-image-to-video` | Image to Video | Variabel | Anonymized |
| LTX Video 2.0 Fast | `ltx-2-fast-text-to-video` | Text to Video | Variabel | Anonymized |
| LTX Video 2.0 Full Quality | `ltx-2-full-image-to-video` | Image to Video | Variabel | Anonymized |
| LTX Video 2.0 Full Quality | `ltx-2-full-text-to-video` | Text to Video | Variabel | Anonymized |
...
| Seedance 1.5 Pro | `seedance-1-5-pro-text-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 1.5 Pro | `seedance-1-5-pro-image-to-video` | Image to Video | Variabel | Anonymized |
| Seedance 2.0 | `seedance-2-0-text-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 | `seedance-2-0-image-to-video` | Image to Video | Variabel | Anonymized |
| Seedance 2.0 Enhanced (Beta) | `seedance-2-0-enhanced-text-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 Enhanced R2V (Beta) | `seedance-2-0-enhanced-reference-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 Fast | `seedance-2-0-fast-text-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 Fast | `seedance-2-0-fast-image-to-video` | Image to Video | Variabel | Anonymized |
| Seedance 2.0 Fast R2V | `seedance-2-0-fast-reference-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 Mini | `seedance-2-0-mini-text-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 Mini | `seedance-2-0-mini-image-to-video` | Image to Video | Variabel | Anonymized |
| Seedance 2.0 Mini R2V | `seedance-2-0-mini-reference-to-video` | Text to Video | Variabel | Anonymized |
| Seedance 2.0 R2V | `seedance-2-0-reference-to-video` | Text to Video | Variabel | Anonymized |
...
| Wan 2.7 Uncensored | `wan-2-7-uncensored-image-to-video` | Image to Video | Variabel | Anonymized |
| Wan 2.7 Uncensored (Beta) | `wan-2-7-uncensored-text-to-video` | Text to Video | Variabel | Anonymized |
...
### Gibt es einen kostenlosen API-Tarif?

Es gibt kein Modell mit Nullkosten in der Venice API – jedes Modell hat einen Preis pro Token, pro Bild, pro Sekunde oder pro Anfrage. Die täglichen Bild- und Nachrichtenkontingente des Venice.ai-Pro-Abonnements gelten nur für die Web-App und die mobile App, nicht für API-Aufrufe.

Die beiden Möglichkeiten, die API ohne USD-Gebühr pro Anfrage zu nutzen, sind:

- **VVV staken für tägliche DIEM-Credits.** VVV-Staker erhalten eine tägliche DIEM-Zuteilung proportional zu ihrem Anteil am Staking-Pool. Jedes 1 DIEM = 1 $/Tag an Inferenz-Credits, die um 00:00 UTC erneuert werden. Siehe den Abschnitt [Verdienen Sie tägliche Credits durch das Staken von VVV](#verdienen-sie-taegliche-credits-durch-das-staken-von-vvv) weiter unten.
- **Pro-Abonnement-Credit.** Pro-Abonnenten erhalten beim Upgrade einmalig 10 $ API-Credit. Dieser ist für Tests und kleine Apps gedacht, nicht für dauerhafte kostenlose Nutzung.

Speech-to-Text mit `stt-xai-v1` wird mit 0,0000 $ pro Audiosekunde ausgewiesen, aber das ist ein Rundungsartefakt – die Nutzung wird weiterhin von Ihrem Guthaben abgezogen und vom vorgelagerten Anbieter bepreist.

### Verdienen Sie tägliche Credits durch das Staken von VVV

Wenn Sie VVV auf Base staken, erhält Ihr Wallet ein tägliches DIEM-Kontingent, das automatisch vor jedem USD-Guthaben ausgegeben wird.

1. **Beschaffen Sie sich VVV auf Base.** Kaufen oder tauschen Sie zu VVV (Contract `0xacfE6019Ed1A7Dc6f7B508C02d1b04ec88cC21bf`) auf einer DEX wie Aerodrome oder Uniswap. Sie benötigen außerdem eine kleine Menge ETH auf Base für Gas.
2. **Staken Sie auf [venice.ai/token](https://venice.ai/token).** Der Staking-Button befindet sich auf dieser Seite. Im Hintergrund sind es zwei On-Chain-Transaktionen auf dem [Venice-Staking-Contract](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff): `approve` auf dem VVV-Token, dann `stake` auf dem Staking-Contract. Ihr VVV-Guthaben verringert sich und Ihr sVVV-Guthaben erhöht sich um denselben Betrag.
3. **Verdienen Sie täglich DIEM.** Die tägliche DIEM-Zuteilung Ihres Wallets ist proportional zu seinem Anteil am Staking-Pool und wird bei jeder Epoche um 00:00 UTC erneuert. Ihr Konto benötigt mindestens **0,1 aufgelaufene DIEM**, bevor DIEM ausgabefähig wird.
4. **Nutzen Sie es für die API.** Wenn Sie die API aufrufen, gibt der geprägte Schlüssel zuerst DIEM aus, dann gebündelte Credits, dann USD. Es fällt keine Gebühr pro Anfrage an, solange DIEM die Ausgabe abdeckt.

Für Agents, die ihren eigenen API-Schlüssel aus einem Staking-Wallet ohne menschliches Zutun prägen, siehe [Autonome Agent-API-Schlüssel-Erstellung](/guides/getting-started/generating-api-key-agent).

### Pro-Nutzer

Pro-Abonnenten erhalten beim Upgrade auf Pro einmalig 10 $ API-Credit. Verwenden Sie diesen, um zu testen und kleine Apps zu entwickeln. Die täglichen Nachrichten- und Bildkontingente der Venice.ai-App sind von diesem Credit getrennt und gelten nicht für API-Anfragen.

## Unzensierte und permissive Modelle

Die Content-Richtlinie von Venice ist permissiver als bei den meisten vorgelagerten Anbietern, und mehrere Modelle im obigen Katalog werden ohne die Sicherheitsschicht des vorgelagerten Anbieters ausgeliefert. Sie erkennen sie an ihrer ID oder ihrem Anzeigenamen:

- **`*-uncensored*`** – Venice-getunte oder Venice-ausgewählte Checkpoints, die das standardmäßige Ablehnungstraining des Anbieters überspringen. Beispiele: `venice-uncensored-1-2`, `gemma-4-uncensored`, `qwen-edit-uncensored`, `wan-2-7-uncensored-image-to-video`.
- **`*-heretic*`** – Community-getunte Checkpoints (z. B. `olafangensan-glm-4.7-flash-heretic`), die vor der Bereitstellung auf Venice ein Fine-Tuning zur Entzensierung durchlaufen. Die Basisgewichte stammen vom in der Modell-ID genannten vorgelagerten Anbieter; das Suffix `heretic` identifiziert das Fine-Tune, nicht das Basismodell.
- **Rollenspiel-Varianten** – z. B. `venice-uncensored-role-play`, abgestimmt auf langformatige Charakter- und kreatives Schreiben.

Nicht-unzensierte Modelle auf Venice geben weiterhin `CONTENT_POLICY_VIOLATION` (HTTP 422) für Prompts zurück, die der vorgelagerte Anbieter ablehnt. Die vollständige Liste finden Sie unter [Fehlercodes](/api-reference/error-codes). Wenn Sie das permissivste Verhalten wünschen, wählen Sie eine `uncensored`- oder `heretic`-Variante und deaktivieren Sie den Standard-System-Prompt von Venice mit `venice_parameters.include_venice_system_prompt: false` – siehe [API-Spezifikation](/api-reference/api-spec) für die Parameterreferenz.
