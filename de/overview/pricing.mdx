...
description: "Preise der Venice API pro 1M Tokens für Text-, Bild-, Video-, Audio- und Embedding-Modelle sowie Web-Suche, RPC und DIEM."
...
Die Venice API ist **nutzungsbasiert (pay-as-you-go)**: Jede Anfrage wird gemessen und gegen das Venice-Credit-Guthaben Ihres Kontos abgerechnet (finanziert in USD oder über gestaktes DIEM). Es gibt keinen kostenlosen Tarif, kein monatliches API-Kontingent und keinen Login-basierten Abo-Plan für die API – der Zugriff wird über einen API-Schlüssel authentifiziert und pro Anfrage abgerechnet.

<Note>
Die Free-, Pro- und Max-Pläne der Consumer-App von [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) (Web-, Desktop- und Mobile-Chat) sind von der API getrennt. Die Limits der Consumer-Abonnements – wie Bildanzahlen, Zeichen-Kontingente oder unbegrenzter Chat – gelten nur für diese Apps, **nicht** für die API-Nutzung. API-Aufrufe werden unabhängig von Ihrem Consumer-Plan stets gegen Ihr Venice-Credit-Guthaben abgerechnet. Siehe [Pro-Nutzer](#pro-nutzer) unten für das einmalige Credit-Guthaben, das Pro-Abonnenten erhalten.
</Note>
...
| MiniMax H3 Enhanced (Beta) | `minimax-h3-enhanced-text-to-video` | Text to Video | Variabel | Anonymized |
| MiniMax H3 R2V | `minimax-h3-reference-to-video` | Text to Video | Variabel | Anonymized |
| MiniMax H3 R2V Enhanced (Beta) | `minimax-h3-enhanced-reference-to-video` | Text to Video | Variabel | Anonymized |
...
Die Venice API verfügt über keine kostenlose Stufe. Jeder Chat-, Bild-, Audio-, Video- und Embedding-Aufruf wird über Credits, DIEM oder [x402/USDC](/guides/integrations/x402-venice-api) bezahlt. Die kostengünstigsten Textmodelle beginnen bei 0,05 USD pro 1 Mio. Eingabetokens, und Embedding-Modelle beginnen bei 0,01 USD pro 1 Mio. Tokens.

<Note>
  Das kostenlose tägliche Prompt-Kontingent auf [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) ist eine Funktion der Consumer-Web-App, nicht der API. API-Anfragen werden immer von Ihrem aufgeladenen Guthaben abgezogen.
</Note>

### Verwendung von Venice-Schlüsseln mit anderen Anbietern

Venice-API-Schlüssel funktionieren ausschließlich mit `https://api.venice.ai/api/v1`. Sie sind nicht mit Schlüsseln von OpenRouter, OpenAI, Anthropic oder anderen Anbietern austauschbar, und Schlüssel dieser Anbieter werden sich nicht bei Venice authentifizieren. Um Venice-Modelle zu verwenden, generieren Sie einen Schlüssel unter [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) und richten Sie Ihren Client auf die Venice-Basis-URL aus.
