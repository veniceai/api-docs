...
<Note>
Die Free-, Pro- und Max-Pläne der Consumer-App von [venice.ai](https://venice.ai) (Web-, Desktop- und Mobile-Chat) sind von der API getrennt. Die Limits der Consumer-Abonnements – wie Bildanzahlen, Zeichen-Kontingente oder unbegrenzter Chat – gelten nur für diese Apps, **nicht** für die API-Nutzung. API-Aufrufe werden unabhängig von Ihrem Consumer-Plan stets gegen Ihr Venice-Credit-Guthaben abgerechnet. Siehe [Pro-Nutzer](#pro-nutzer) unten für das einmalige Credit-Guthaben, das Pro-Abonnenten erhalten.
</Note>
...
<Note>
  Das kostenlose tägliche Prompt-Kontingent auf [venice.ai](https://venice.ai) ist eine Funktion der Consumer-Web-App, nicht der API. API-Anfragen werden immer von Ihrem aufgeladenen Guthaben abgezogen.
</Note>

### Verwendung von Venice-Schlüsseln mit anderen Anbietern

Venice-API-Schlüssel funktionieren ausschließlich mit `https://api.venice.ai/api/v1`. Sie sind nicht mit Schlüsseln von OpenRouter, OpenAI, Anthropic oder anderen Anbietern austauschbar, und Schlüssel dieser Anbieter werden sich nicht bei Venice authentifizieren. Um Venice-Modelle zu verwenden, generieren Sie einen Schlüssel unter [venice.ai/settings/api](https://venice.ai/settings/api) und richten Sie Ihren Client auf die Venice-Basis-URL aus.
