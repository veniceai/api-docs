---
title: API-Preise
description: "Preise pro Token und pro Anfrage für Text-, Bild-, Audio-, Video- und Embedding-Modelle von Venice, plus Web-Search-Preise und Zahlungsoptionen."
...
Pro-Abonnenten erhalten beim Upgrade auf Pro einmalig 10 USD an Venice-Credits. Verwenden Sie diese, um zu testen und kleine Apps zu entwickeln.

## Abonnements und die API

Die API-Nutzung ist immer Pay-per-Use. Es gibt keine kostenlose API-Stufe und keinen unlimitierten API-Tarif: Jedes Modell auf dieser Seite rechnet pro Token, pro Bild oder pro Anfrage ab.

Venice-App-Abonnements (die Tarife, die von der Chat-Oberfläche unter [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) verwendet werden) werden getrennt von der API-Nutzung abgerechnet. Ihre Tarifstufen, Preise und In-App-Vorteile werden in dieser API-Dokumentation nicht behandelt. Verwalten Sie sie in Ihrem Venice-Konto.

Credits, die in einem Abonnement enthalten sind, wie das einmalige Pro-Guthaben von 10 USD, erscheinen auf Ihrem Konto als **gebündelte Credits**. Die API verbraucht Kontoguthaben in dieser Reihenfolge: zuerst DIEM, dann gebündelte Credits, dann USD. Siehe [VVV & DIEM](/overview/vvv-diem#how-funding-works) für die Funktionsweise der einzelnen Finanzierungsquellen und die Prüfung Ihrer Guthaben.
