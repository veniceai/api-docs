...
<CardGroup cols={3}>
  ...
  <Card title="DIEM staken" icon="coins" href="/overview/vvv-diem">
...
Gebündelte Credits sind API-Credits, die Ihrem Venice-Konto durch einen Venice-Abonnementplan hinzugefügt werden (zum Beispiel das einmalige Guthaben für neue Pro-Abonnenten). Sie werden von der API zusammen mit DIEM- und USD-Credits ausgegeben und erscheinen in Abrechnungsantworten als Währung `BUNDLED_CREDITS`.

- **Wofür sie verwendet werden können** – gebündelte Credits können für jeden kostenpflichtigen Venice-API-Endpoint (Chat, Bilder, Audio, Video, Embeddings) ausgegeben werden. Ihre Preise richten sich nach derselben USD-Preisliste auf dieser Seite.
- **Verbrauchsreihenfolge** – API-Schlüssel belasten Ihr Konto in dieser Reihenfolge: **DIEM → gebündelte Credits → USD**. Sie müssen keine Währung pro Anfrage auswählen. Siehe [So funktioniert die Finanzierung](/overview/vvv-diem#how-funding-works).
- **Wo Sie das Guthaben prüfen können** – aktuelle Guthaben ansehen unter [venice.ai/settings/api](https://venice.ai/settings/api) oder den [Billing Balance](/api-reference/endpoint/billing/balance)-Endpoint aufrufen und den Eintrag `BUNDLED_CREDITS` auslesen.

<Note>
  Alle monatlichen Credit-Kontingente, die auf der Seite [Venice consumer plans](https://venice.ai) angezeigt werden (zum Beispiel Credits für die venice.ai-Chat-App oder Media-Generatoren), sind von den gebündelten API-Credits getrennt und können möglicherweise nicht über diese API ausgegeben werden. Die Venice API gibt nur die DIEM-, gebündelten und USD-Guthaben aus, die vom [Billing Balance](/api-reference/endpoint/billing/balance)-Endpoint gemeldet werden.
</Note>
