---
title: API-Preise
description: "Venice API-Preise für jedes Modell: Preise pro Token für Chat Completions, Preise pro Generierung für Bild, Video und Audio, in USD, VVV und DIEM."
...
Pro-Abonnenten erhalten beim Upgrade auf Pro einmalig $10 an Venice-Credits. Verwenden Sie diese, um zu testen und kleine Apps zu entwickeln.

### Venice.ai-App-Abonnements vs. API-Abrechnung

Die Consumer-Abonnements von Venice.ai (zum Beispiel Pro und Max auf [venice.ai](https://venice.ai)) gelten für die Chat-, Bild- und Video-Apps von Venice auf [venice.ai](https://venice.ai). Sie sind von der API-Abrechnung getrennt.

* Die API-Nutzung wird immer pro Anfrage nach den oben aufgeführten Tarifen pro Token, pro Bild, pro Sekunde oder pro Generierung abgerechnet. Es gibt keinen unbegrenzten oder nicht gemessenen API-Tarif.
* Die Vorteile des App-Abonnements — unbegrenzte Chat-Prompts, monatliche Credit-Kontingente für In-App-Video, -Musik und Frontier-Generierung sowie Credit-Banking — gelten nur innerhalb der Venice-Apps. Sie gewähren keinen API-Zugriff und werden nicht in API-Credits umgewandelt, abgesehen von dem oben genannten einmaligen $10-API-Credit für Pro-Abonnenten.
* Um API-Anfragen zu finanzieren, kaufen Sie Venice-Credits in USD oder Krypto auf der Seite [API-Einstellungen](https://venice.ai/settings/api?utm_source=venice-api-documentation) oder staken Sie DIEM für ein tägliches Kontingent (siehe [VVV & DIEM](/overview/vvv-diem)).
