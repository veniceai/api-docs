...
(function() {
  ...
  function getModelModality(model) {
    const labels = {
      text: 'Text',
      image: 'Image',
      video: 'Video',
    ...
    return t(labels[model.type] || model.type || 'Model');
  }
  ...
  function renderMobileTypeTabs() {
    return FILTER_GROUPS.type.options.map(o =>
      `<button type="button" class="vmb-mobile-type-option" aria-pressed="false" data-group="type" data-value="${o.value}">${t(o.label)}</button>`
    ).join('');
  }

  function renderMobileFilterGroup(key) {
    const group = FILTER_GROUPS[key];
    const options = group.options.map(o =>
      `<button type="button" class="vmb-mobile-filter-option" aria-pressed="false" data-group="${key}" data-value="${o.value}">` +
        `<span>${t(o.label)}</span>${FILTER_CHECK}` +
      `</button>`
    ).join('');
    return `<section class="vmb-mobile-filter-group" data-mobile-group="${key}"><h3>${t(group.label)}</h3><div class="vmb-mobile-filter-options">${options}</div></section>`;
  }

  function renderMobileFilterLayer() {
    return (
      `<div class="vmb-mobile-filter-layer" hidden>` +
        `<button type="button" class="vmb-mobile-filter-backdrop" aria-label="${t('Close filters')}"></button>` +
        `<div class="vmb-mobile-filter-sheet" role="dialog" aria-modal="true" aria-label="${t('Filters')}">` +
          `<div class="vmb-mobile-filter-header"><strong>${t('Filters')}</strong><button type="button" class="vmb-mobile-filter-done">${t('Done')}</button></div>` +
          `<div class="vmb-mobile-filter-body">${renderMobileFilterGroup('image')}${ENABLE_VIDEO ? renderMobileFilterGroup('video') : ''}${renderMobileFilterGroup('capability')}${renderMobileFilterGroup('privacy')}</div>` +
          `<button type="button" class="vmb-mobile-filter-clear" hidden>${t('Clear filters')}</button>` +
        `</div>` +
      `</div>`
    );
  }
  ...
  function formatVideoPricing(modelId, model) {
    ...
    return `${prefix}<span class="vmb-video-price" data-model="${modelId}">${t('Variable')}</span>`;
  }

  async function updateVideoPrice(modelId, model, { resolution, duration, audio } = {}, container) {
    const priceEls = [...(container || document).querySelectorAll(`.vmb-video-price[data-model="${modelId}"]`)];
    if (priceEls.length === 0) return;

    const price = await fetchVideoQuote(modelId, model, { resolution, duration, audio });
    const label = price !== null ? formatPrice(price) : t('Variable');
    priceEls.forEach(priceEl => { priceEl.textContent = label; });
  }
  ...
  function getCapabilityIcons(caps) {
    if (!caps) return '';
    const icons = [];
    if (caps.supportsFunctionCalling) icons.push(`<span class="vmb-cap vmb-tooltip" data-tooltip="Function Calling">${CAP_ICONS.function}<span class="vmb-cap-label">${t('Function Calling')}</span></span>`);
    if (caps.supportsReasoning) icons.push(`<span class="vmb-cap vmb-tooltip" data-tooltip="Reasoning">${CAP_ICONS.reasoning}<span class="vmb-cap-label">${t('Reasoning')}</span></span>`);
    if (caps.supportsVision) icons.push(`<span class="vmb-cap vmb-tooltip" data-tooltip="Vision">${CAP_ICONS.vision}<span class="vmb-cap-label">${t('Vision')}</span></span>`);
    if (caps.optimizedForCode) icons.push(`<span class="vmb-cap vmb-tooltip" data-tooltip="Code Optimized">${CAP_ICONS.code}<span class="vmb-cap-label">${t('Code')}</span></span>`);
  ...
  function getPrivacyTag(model, variant) {
    ...
    const focusAttrs = (label, tooltip) => variant === 'vmb'
      ? ` tabindex="0" aria-label="${t(label)}: ${tooltip}"`
      : '';
  ...
  async function init() {
    ...
    container.innerHTML = `
      ...
      ${presetFilter ? '' : `<div class="vmb-mobile-type-tabs" role="toolbar" aria-label="${t('Type')}">${renderMobileTypeTabs()}</div>`}
    ...
    const mobileFilterClear = container.querySelector('.vmb-mobile-filter-clear');
    ...
    function updateClearVisibility() {
      ...
      container.querySelectorAll('.vmb-mobile-filter-option').forEach(option => {
        const key = option.dataset.group;
        const value = option.dataset.value;
        const on = key === 'capability' ? activeCapabilities.has(value) : getSingleState(key) === value;
        option.classList.toggle('active', on);
        option.setAttribute('aria-pressed', on ? 'true' : 'false');
      });
    }
    ...
    function syncTypeDependentDropdowns() {
      ...
      container.querySelectorAll('[data-mobile-group="image"]').forEach(el => {
        el.hidden = activeFilter !== 'image';
      });
      container.querySelectorAll('[data-mobile-group="video"]').forEach(el => {
        el.hidden = activeFilter !== 'video';
      });
      container.querySelectorAll('[data-mobile-group="capability"]').forEach(el => {
        el.hidden = !categoryAllowsCapabilityFilters(activeFilter);
      });
    }
    ...
    function setMobileFiltersOpen(open, { restoreFocus = true } = {}) {
      if (open) {
        filterReturnFocus = document.activeElement;
        mobileFilterLayer.hidden = false;
        mobileFiltersTrigger.setAttribute('aria-expanded', 'true');
        document.documentElement.classList.add('vmb-filters-open');
        requestAnimationFrame(() => mobileFilterLayer.querySelector('.vmb-mobile-filter-done')?.focus());
        return;
      }
      mobileFilterLayer.hidden = true;
      mobileFiltersTrigger.setAttribute('aria-expanded', 'false');
      document.documentElement.classList.remove('vmb-filters-open');
      if (restoreFocus && filterReturnFocus?.isConnected) filterReturnFocus.focus();
    ...
    container.addEventListener('click', (e) => {
      const typeOption = e.target.closest('.vmb-mobile-type-option');
      if (typeOption) {
      ...
      if (e.target.closest('.vmb-mobile-filter-backdrop, .vmb-mobile-filter-done')) {
        setMobileFiltersOpen(false);
        return;
      }
      if (e.target.closest('.vmb-mobile-filter-clear')) {
        clearSecondaryFilters();
        return;
      }
      const mobileOption = e.target.closest('.vmb-mobile-filter-option');
      if (mobileOption) {
        handleOptionSelect(mobileOption);
      }
    });
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') { closeAllPanels(null); setMobileFiltersOpen(false); return; }
      if (e.key !== 'Tab' || mobileFilterLayer.hidden) return;
      const focusable = [...mobileFilterLayer.querySelectorAll('.vmb-mobile-filter-sheet button:not([hidden]):not([disabled])')]
        .filter(button => button.getClientRects().length > 0 && !button.closest('[hidden]'));
    ...
    function renderModelCard(model, searchQuery) {
      ...
      // Pricing display
        ...
        if (model.type === 'video') {
          ...
          if (model._hasResDropdown) {
            ...
            videoControlsHtml += `<select class="vmb-res-select vmb-video-select" data-model="${model.id}" aria-label="${t('Video resolution')}" title="${t('Video resolution')}">${resOptions}</select>`;
          }
          if (model._hasDurDropdown) {
            const durOptions = durations.map((d, i) =>
              `<option value="${d}"${i === 0 ? ' selected' : ''}>${d}</option>`
            ).join('');
            videoControlsHtml += `<select class="vmb-dur-select vmb-video-select" data-model="${model.id}" aria-label="${t('Video duration')}" title="${t('Video duration')}">${durOptions}</select>`;
          }
          if (model._hasAudioToggle) {
            videoControlsHtml += `<button type="button" class="vmb-audio-toggle" data-model="${model.id}" data-audio="true" aria-pressed="true">♪ ${t('Audio on')}</button>`;
          }
          videoControlsHtml += `<span class="vmb-video-price" data-model="${model.id}">${t('Variable')}</span>`;
      ...
      const modalityBadge = `<span class="vmb-modality-badge">${escapeHtml(getModelModality(model))}</span>`;

      const videoTypeBadge = model.type === 'video' && constraints.model_type
        ? `<span class="vmb-video-type-badge ${constraints.model_type === 'text-to-video' ? 'ttv' : 'itv'}">${t(constraints.model_type === 'text-to-video' ? 'Text to Video' : 'Image to Video')}</span>`
        : '';

      const privacyBadge = getPrivacyTag(model, 'vmb');

      const betaBadge = isBetaModel(model)
        ? `<span class="vmb-beta-badge vmb-tooltip" data-tooltip="${TOOLTIPS.beta}" tabindex="0" aria-label="Beta: ${TOOLTIPS.beta}">Beta</span>`
        : '';

      const deprecatedBadge = isDeprecatedModel(model)
        ? `<span class="vmb-deprecated-badge vmb-tooltip" data-tooltip="Scheduled for removal on ${formatDeprecationDate(getModelRemovalDate(model))}. See the deprecations page for details." tabindex="0" aria-label="Deprecated. Scheduled for removal on ${formatDeprecationDate(getModelRemovalDate(model))}.">Deprecated</span>`
        : '';

      const uncensoredBadge = isUncensoredModel(model)
        ? `<span class="vmb-uncensored-badge vmb-tooltip" data-tooltip="${TOOLTIPS.uncensored}" tabindex="0" aria-label="Uncensored: ${TOOLTIPS.uncensored}">Uncensored</span>`
        : '';

      const upgradedBadge = isUpgradedModel(model)
        ? `<span class="vmb-upgraded-badge vmb-tooltip" data-tooltip="${TOOLTIPS.upgraded}" tabindex="0" aria-label="Upgraded: ${TOOLTIPS.upgraded}">Upgraded</span>`
        : '';

      const moderationBadge = hasContentModeration(model.id)
        ? `<span class="vmb-moderation-badge vmb-tooltip" data-tooltip="${TOOLTIPS.content_moderation}" tabindex="0" aria-label="Moderated: ${TOOLTIPS.content_moderation}">Moderated</span>`
        : '';
      ...
      const aspectRatioHtml = aspectRatios.length > 0
        ? `<span class="vmb-aspect-ratios">${aspectRatios.map(ar => {
            ...
            return `<span class="vmb-ar-pair"><span class="vmb-ar ${cls}" aria-hidden="true"></span><span>${escapeHtml(ar)}</span></span>`;
          }).join('')}</span>`
        : '';
      ...
      const leftParts = [
        `<span class="vmb-model-id"><span class="vmb-meta-label">${t('Model ID')}</span><span class="vmb-id-value"><span class="vmb-id-text">${modelIdDisplay}</span>${idCopyBtn}</span></span>`,
        model.type === 'video' && videoControlsHtml
          ? `<span class="vmb-meta-block"><span class="vmb-meta-label">${t('Configure price')}</span><span class="vmb-video-controls">${videoControlsHtml}</span></span>`
          : (priceStr ? `<span class="vmb-meta-block"><span class="vmb-meta-label">${t('Pricing')}</span><span class="vmb-pricing">${priceStr}</span></span>` : ''),
        videoMeta ? `<span class="vmb-video-static-meta">${videoMeta}</span>` : ''
      ].filter(Boolean);
      ...
      const contextMobile = contextStr ? `<span class="vmb-context vmb-context-mobile"><span class="vmb-meta-label">${t('Capacity')}</span>${contextStr}</span>` : '';
      const desktopPrice = model.type === 'video'
        ? `<span class="vmb-table-video">${videoControlsHtml}</span>`
        : (priceStr ? `<span class="vmb-pricing">${priceStr}</span>` : '<span class="vmb-table-empty">—</span>');
      const desktopCapacity = contextStr
        ? `<span class="vmb-context">${contextStr}</span>`
        : (videoMeta ? `<span class="vmb-video-static-meta">${videoMeta}</span>` : '<span class="vmb-table-empty">—</span>');
    ...
    modelsContainer.addEventListener('change', (e) => {
      ...
      if (isImgRes) {
        ...
        const scope = target.closest('.vmb-model-mobile, .vmb-table-capacity, .vmb-table-price') || target.closest('.vmb-model');
        const row = target.closest('.vmb-model');
        row?.querySelectorAll('.vmb-img-res').forEach(select => {
          if (select !== target) select.value = resolution;
        });
        const priceEls = row ? [...row.querySelectorAll('.vmb-img-price-val')] : [];
        if (price !== undefined) {
          priceEls.forEach(priceEl => { priceEl.textContent = formatPrice(price); });
        }
        return;
      }

      // Handle video pricing
      const scope = target.closest('.vmb-model-mobile, .vmb-table-price') || target.closest('.vmb-model');
      const resSelect = scope.querySelector('.vmb-res-select');
      const durSelect = scope.querySelector('.vmb-dur-select');
      const audioToggle = scope.querySelector('.vmb-audio-toggle');
      ...
      const row = target.closest('.vmb-model');
      row.querySelectorAll(`.${isResSelect ? 'vmb-res-select' : 'vmb-dur-select'}`).forEach(select => {
        if (select !== target) select.value = target.value;
      });

      updateVideoPrice(modelId, model, { resolution, duration, audio }, row);
    });
    ...
    modelsContainer.addEventListener('click', (e) => {
      ...
      const nextAudio = !isOn;
      const row = toggle.closest('.vmb-model');
      row.querySelectorAll('.vmb-audio-toggle').forEach(audioToggle => {
        audioToggle.dataset.audio = nextAudio ? 'true' : 'false';
        audioToggle.textContent = `♪ ${t(nextAudio ? 'Audio on' : 'Audio off')}`;
        audioToggle.classList.toggle('off', !nextAudio);
        audioToggle.setAttribute('aria-pressed', nextAudio ? 'true' : 'false');
      });
      ...
      const activeScope = toggle.closest('.vmb-model-mobile, .vmb-table-price') || row;
      const resSelect = activeScope.querySelector('.vmb-res-select');
      const durSelect = activeScope.querySelector('.vmb-dur-select');
