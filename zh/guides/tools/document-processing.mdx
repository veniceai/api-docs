---
title: "文档处理"
description: "使用 Venice Text Parser API（/augment/text-parser 端点）从 PDF、DOCX、PPTX、XLSX 及最大 25 MB 的纯文本文件中提取文本和 token 数量，无需运行模型推理，并在提示、嵌入和数据管道中复用输出。"
...
提取出的文本是一个起点，而不是最终结果。[从文档提取结构化数据](/guides/tools/document-extraction)从这里继续：将输出固定到一个 JSON schema，使每份文档都产出相同的字段；并在解析器报告某个文件没有可提取文本时切换到视觉模型。

## 相关资源

- [从文档提取结构化数据](/guides/tools/document-extraction)
