...
要解决该错误,可以在 [venice.ai/settings/api](https://venice.ai/settings/api) 中调高或移除该密钥的 Epoch Consumption Limits,或使用新的 `consumptionLimits` 调用 [更新 API 密钥](/api-reference/endpoint/api_keys/update)。账户级别的余额(USD、DIEM 和捆绑积分)是独立的支付来源,系统会在有可用余额时自动扣除;你无需在请求时选择使用哪一种。
