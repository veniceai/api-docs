---
title: "函数调用"
description: "通过与 OpenAI 兼容的函数调用（function calling）、tools 工具 schema 定义以及 /chat/completions API，让 Venice 聊天模型返回结构化的工具调用，由您的应用执行函数并回传结果，适用于实时数据查询与应用操作。"
...
上面的示例在一次往返之后就停止了。真实的智能体会持续运行，因为模型通常需要多次调用工具才能给出答案。[使用函数调用构建能使用工具的智能体](/guides/features/tool-using-agent)完整走通了整个循环，面对的是模型从未见过的数据库，其中包括并行工具调用、模型能从中恢复的错误返回，以及模型自己"拒绝"与守护逻辑强制"拦截"之间的区别。
...
- [使用函数调用构建能使用工具的智能体](/guides/features/tool-using-agent)
