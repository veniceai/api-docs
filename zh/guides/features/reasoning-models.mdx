---
title: "推理模型"
description: "调用 Venice 推理模型（reasoning models），通过 reasoning_content 字段读取思维链、使用 reasoning_effort 控制思考力度，并在 /chat/completions 聊天补全中呈现适合数学、代码与逻辑任务的逐步答案。"
...
完整的模型列表、定价和上下文限制请参阅[模型页面](/overview/models)。并非所有推理模型都支持 [`reasoning_effort`](#推理强度) 参数。详情请参阅[模型支持](#模型支持)。
...
下表列出了已知支持 `reasoning_effort` 的模型。其他模型仍可能生成推理 token（可在 `usage.completion_tokens_details.reasoning_tokens` 中查看），但不提供 effort 控制。要查看特定模型是否支持推理或 `reasoning_effort` 参数，请读取 [`/v1/models`](/api-reference/endpoint/models/list) 端点上的 `supportsReasoning` 和 `supportsReasoningEffort` 字段。
...
要获得更多可见输出，请提高上限、降低 `reasoning_effort`，或[禁用推理](#禁用推理)。
