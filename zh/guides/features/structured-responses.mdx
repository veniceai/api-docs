---
title: "结构化响应"
description: "使用 response_format 和 JSON Schema 强制 Venice 聊天模型返回符合您的模式的 JSON，以获得可靠、可解析的输出。"
'og:title': "Structured Responses | Venice API Docs"
'og:description': "Using structured responses within the Venice API"
---

Venice 使用 Chat Completions 生成文本，目前不提供 OpenAI 的 Responses API。结构化响应使用 `POST /chat/completions` 上的 `response_format`。
