---
title: "TEE 与 E2EE 模型"
description: "在 Venice 上通过 Trusted Execution Environment（TEE）与端到端加密（E2EE）模型运行私有推理，使用 e2ee-* 前缀在硬件安全 enclave 中处理 prompt，并通过远程 attestation 证明确保宿主与 Venice 均无法读取内容。"
...
<Info>
有关隐私模式的概述以及选择 TEE 或 E2EE 的帮助，请参阅[隐私](/overview/privacy)。本指南介绍协议和实现细节。
</Info>

<Note>
TEE 和 E2EE 目前仅适用于文本模型。
</Note>
