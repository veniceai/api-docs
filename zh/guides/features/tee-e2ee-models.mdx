---
title: "TEE 与 E2EE 模型"
description: "在 Venice 上通过 Trusted Execution Environment（TEE）与端到端加密（E2EE）模型运行私有推理，使用 tee-* 与 e2ee-* 前缀在硬件安全 enclave 中处理 prompt，并通过远程 attestation 证明确保宿主与 Venice 均无法读取内容。"
