---
title: "TEE 与 E2EE 模型"
description: "通过 Trusted Execution Environment (TEE) 模型和端到端加密在 Venice 上运行私有推理，让宿主无法读取您的 prompt。"
...
| 类型 | 前缀 | 含义 |
|------|--------|---------------|
| **TEE** | `e2ee-*` | 模型在硬件安全 enclave 中运行。Venice 无法访问运算过程。您可以通过证明（attestation）验证。 |
| **E2EE** | `e2ee-*` | 完整的端到端加密。您的 prompt 在发送前会在客户端加密。只有 TEE 能够解密。 |

<Info>
TEE 和 E2EE 模型的 ID 都使用 `e2ee-*` 前缀。原因是 Venice 上每一个经过 TEE 证明的模型同时也支持 E2EE——TEE 证明正是让端到端加密成为可能的机制——因此它们共用同一个模型系列和前缀。无论您使用普通的 TEE 推理还是完整的 E2EE 流程，模型 ID 都是相同的。E2EE 模型在 TEE 保护之上加入了客户端加密。TEE 模型提供 enclave 安全性，无需客户端加密。
</Info>
