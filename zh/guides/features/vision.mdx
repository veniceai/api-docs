...
`content` 是一个块数组，因此一条用户消息可以在文本之外包含多个 `image_url` 块。可以借此比较图片，或提出涉及多张图片的问题：
  ...
  "messages": [
    {
      "role": "user",
      "content": [
        {"type": "text", "text": "比较这两张图片并列出它们的差异。"},
        {
          "type": "image_url",
...
每张图片都会消耗上下文 token。如果包含多张图片的请求返回 `TOO_MANY_TOKENS`（HTTP 400），请减少图片数量、缩短指令，或改用上下文窗口更大的模型。
