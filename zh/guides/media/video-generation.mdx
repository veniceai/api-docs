...
与图像生成不同，视频队列端点不接受 `safe_mode` 参数。成人内容支持是模型特定的：请选择专为无审查输出设计的模型变体。例如，Wan 2.7 系列在默认变体之外还提供了无审查变体：

- `wan-2-7-uncensored-text-to-video`
- `wan-2-7-uncensored-image-to-video`

提交到非无审查变体的请求可能会因内容违规（`422`）而被拒绝。请参阅[视频模型](/models/video)和[定价](/overview/pricing)获取当前变体列表及其按秒计费。
