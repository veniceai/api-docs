...
<Info>
**报价参数仅用作定价输入。** 您发送到 `/video/quote` 的字段——包括 `aspect_ratio`、`audio` 和 `reference_video_total_duration`——仅用于计算价格。它们**不会**被转发到 `/video/queue`，也不会影响生成的视频。即使某些模型（例如 `seedance-2-0-image-to-video-basic`）在生成时会拒绝 `aspect_ratio`，向 quote 发送 `aspect_ratio` 也是可接受的，因为 image-to-video 模型会从输入图像推导出输出宽高比。
</Info>
