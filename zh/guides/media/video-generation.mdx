---
title: 视频生成
description: "在 Venice 异步队列 API 上从文本 prompt 或起始图像生成视频：使用 /video/quote 获取报价、/video/queue 提交任务、/video/retrieve 轮询状态并下载 MP4，最后用 /video/complete 从存储中删除视频。"
...
对于在排队时返回 `download_url` 的模型，retrieve 始终返回 JSON。使用 `GET download_url`（无 auth 头）获取视频。请参阅[私有下载链接](#私有下载链接)了解这些 URL 的工作方式、重试和可选的 `DELETE`。
...
| `prompt` | string | 是 | - | 要生成的内容。最大长度因模型而异，差别很大——从几百个字符到 20,000 个字符不等。未声明自身限制的模型默认为 2,500。请从 `GET /models?type=video` 读取 `model_spec.constraints.prompt_character_limit`，而不要假设单一限制 |
...
| `bitrate_mode` | string | 否 | `"standard"` | 仅限 Seedance 2.0 / 2.5。`"standard"` 或 `"high"`。`high` 请求更高质量、文件更大的编码。不改变价格。其他家族请省略 |
...
<Info>
**报价参数仅用作定价输入。** 您发送到 `/video/quote` 的字段——包括 `aspect_ratio`、`audio` 和 `reference_video_total_duration`——仅用于计算价格。它们**不会**被转发到 `/video/queue`，也不会影响生成的视频。即使某些模型（例如 `seedance-2-0-image-to-video`）在生成时会拒绝 `aspect_ratio`，向 quote 发送 `aspect_ratio` 也是可接受的，因为 image-to-video 模型会从输入图像推导出输出宽高比。
</Info>

### 参考视频到视频的定价

对于 R2V 模型（例如 Seedance 2.0 R2V），请传递 `reference_video_total_duration`——即您计划包含的所有参考剪辑的总时长（以秒为单位）——以便报价反映"带视频输入"的费率层级以及 `(输入 + 输出) × 像素` 的 token 公式。如果省略它，报价将改为返回无参考的基线值。

`reference_video_total_duration` 仅被 `/video/quote` 识别。它对 `/video/queue` 没有影响，省略它也不会导致生成错误。

### 显示稳定价格

报价是时点性的，价格可能会变化。如果您希望在应用中显示固定价格，请对您支持的每种参数组合调用一次 `/video/quote`，并将结果缓存到您自己的存储中——然后按计划（或在价格表更新时）重新报价以刷新缓存的值。不存在"锁定"或"静态"的报价端点。
...
4. 如果 `Content-Type` 为 `application/json` 且 `status` 为 `"COMPLETED"`，从队列响应中 `GET` `download_url` 以获取视频（请参阅[私有下载链接](#私有下载链接)）
...
与图像生成不同，视频队列端点不接受 `safe_mode` 参数。成人内容支持是模型特定的：请选择专为无审查输出设计的模型变体。例如，Wan 2.7 系列在默认变体之外还提供了 Enhanced 变体：

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

提交到默认变体的请求可能会因内容违规（`422`）而被拒绝。请参阅[视频模型](/models/video)和[定价](/overview/pricing)获取当前变体列表及其按秒计费。
