---
title: "视频放大"
description: "在 Venice API 上使用 topaz-video-upscale 模型增强视频分辨率和质量：通过 /video/queue 提交源片段、选择 1 倍、2 倍或 4 倍放大倍数、轮询 /video/retrieve 并调用 /video/complete 获取结果，按实际视频元数据计费。"
...
对于放大模型的报价，`video_url` 是**必填**的 —— 报价端点会获取文件并从字节中检测时长、FPS 和高度，因此无需另行提供这些信息。省略它会返回 `400` 及 *"video_url is required for quotes on this model"*。

<Warning>
报价端点和排队端点目前对放大倍数的命名不同：`/video/queue` 使用 `upscale_factor`（`1`、`2` 或 `4`），而 `/video/quote` 从 `resolution`（`"2x"` 或 `"4x"`）中读取倍数。向报价端点发送 `upscale_factor` 没有任何效果——它会回退到 `"2x"` 默认值，从而为可能是 4x 的作业返回 2x 的价格。为 4x 放大报价时，请传入 `resolution: "4x"`。
</Warning>
...
| `"Insufficient balance"` | 账户余额不足 | 在 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) 充值 |
