...
`omni_reference_task_type: "edit"` 会预先引导 BytePlus 的分类。`duration: "auto"`（或 `"-1"`）和 `aspect_ratio: "adaptive"`（或 `"auto"`）会让输出跟随源片段。参见[与源匹配的宽高比和时长](#source-matched-aspect-ratio-and-duration)与[`omni_reference_task_type`](#omni_reference_task_type-seedance-25-r2v)。
