...
description: "使用 Venice 上的 Seedance 2.0 与 2.5 生成、编辑、延长并拼接视频 - 文本/图像/参考工作流、faces-off API 行为，以及按系列区分的多模态限制。"
'og:title': "Seedance 2.0 与 2.5 指南 | Venice API Docs"
'og:description': "通过 Venice API 使用 Seedance 2.0 与 2.5 生成、编辑、延长并拼接视频。工作流、faces-off、多模态限制、定价与示例。"
---
...
<Warning>
**公共 Seedance API 不支持人脸。** Venice 不会为公共 Seedance 模型运行人脸资产开通或人脸同意证明（`consents.seedance` / `needs_consent`）。含人脸的输入可能在上游被以内容政策 / 提供商错误拒绝。如需 likeness / 人脸资产生成，请使用 Venice 应用或 Studio。
</Warning>
