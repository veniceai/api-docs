...
description: "在 Venice API 上使用 Seedance 2.0 与 2.5 系列多模态视频模型生成、编辑、扩展和拼接视频，涵盖文生视频、图生视频、参考生视频四种主要工作流，以及公共 API 媒体政策、家族特定的多模态限制、分辨率选项、定价明细与 curl 请求示例。"
...
本指南涵盖各变体、四种工作流、**公共 API 媒体政策**、**家族特定的多模态限制**、输出码率、定价以及 `curl` 示例。
...
| `seedance-2-5-text-to-video-basic` | T2V | 480p / 720p / 1080p | 输出最长 30 秒；原生音频 |
| `seedance-2-5-image-to-video-basic` | I2V | 480p / 720p / 1080p | 输出最长 30 秒；首帧（可选末帧）基础 |
| `seedance-2-5-reference-to-video-basic` | R2V | 480p / 720p / 1080p | 最多 30 张图像 + 10 段视频 + 10 段音频；相同的 Reference / Edit / Extend / Stitch 工作流 |
...
`resolution` 传入以下值之一：`480p`、`720p`、`1080p` 或 `4k`（小写）。Seedance **2.0**（非 Fast）接受全部四种；**2.5** 接受 `480p` / `720p` / `1080p`；**2.0 Fast** 仅接受 `480p` / `720p`。请使用 `GET /models?type=video` 查询实时的模型 ID 与能力 —— 不要在代码中硬编码可用性。

## 输出码率

你可以在所有公开的 Seedance **2.0**（含 Fast）和 **2.5** 模型上设置输出编码码率。请自行在 `/video/queue` 中传入 `bitrate_mode` —— Venice 不会自动选择。

| 值 | 行为 |
|---|---|
| `standard` | 默认码率。与省略该字段相同。 |
| `high` | 更高质量、文件更大的编码。 |

这是一个仅限队列的编码选项：它不会改变价格或 token 费用，`/video/quote` 也不接受 `bitrate_mode`。其他视频家族（Wan、Kling、LTX 等）不支持该字段。
...
BytePlus 会根据提示词把 Seedance 2.5 全模态参考任务分类为 **reference**、**edit** 或 **extend**。你可以在 `/video/queue` 上通过以下值可选地预先引导该分类：

| 值 | 含义 |
| --- | --- |
| `auto` | 让 BytePlus 根据提示词进行分类 |
| `reference` | 多模态参考生成（由素材源引导的新片段） |
| `edit` | 编辑已有片段（搭配 `duration: "auto"` / `"-1"`，通常还配 `aspect_ratio: "adaptive"`） |
| `extend` | 延伸已有片段（固定 `duration` 指定要生成的时长；通常配 `aspect_ratio: "adaptive"`） |

别名 `editing`→`edit` 与 `extension`→`extend` 也被接受。若省略该字段，当存在 `reference_video_urls` 时，Venice 会从提示词中推断其值。提示词仍必须与所选类型相匹配 —— 冲突仍可能异步失败。
...
用 AI 生成的转场把输入片段连接起来。请遵守[多模态输入限制](#多模态输入限制)中**家族特定**的合并时长与片段数上限（Seedance 2.0：≤3 段 / 合计 ≤15 秒；Seedance 2.5：视频上限更高）。
...
| 输出分辨率 | 480p / 720p / 1080p / **4k**（Fast：仅 480p / 720p） | 480p / 720p / 1080p |
| 输出码率（`bitrate_mode`） | `standard`（默认）或 `high` | `standard`（默认）或 `high` |
| R2V 参考图像 | 1–9 | 1–30 |
...
`omni_reference_task_type: "edit"` 会预先引导 BytePlus 的分类。`duration: "auto"`（或 `"-1"`）和 `aspect_ratio: "adaptive"`（或 `"auto"`）会让输出跟随源片段。参见[与源匹配的宽高比和时长](#与源匹配的宽高比和时长)与[`omni_reference_task_type`](#omni_reference_task_typeseedance-25-r2v)。
...
任务运行期间，响应是 JSON（`{ "status": "PROCESSING", ... }`）；任务完成后，响应体切换为 `video/mp4` 字节。`status` 为 `PROCESSING` 或 `COMPLETED` 之一 —— 不存在 `failed` 状态。失败的任务会改为返回携带所记录错误的非 200 响应。完整轮询模式请参阅[视频生成](/guides/media/video-generation)。
...
参考生视频提交必须至少包含 `reference_image_urls`、`reference_video_urls` 或 `image_references` 之一。仅文本的生成不是有效的 R2V 工作流 —— 请改用文生视频的模型 ID。仅有 `reference_audio_urls` 也不够（参见上面的音频章节）。请注意，Seedance **不**接受 `video_references` —— 它专属于支持按参考配音音频的模型，传入会返回 *"This model does not support per-reference video entities"*。
