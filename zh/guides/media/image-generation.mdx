---
title: "图像生成"
description: "使用 Venice API 从文本 prompt 生成图像，涵盖模型尺寸选项、风格预设、LoRA 强度、安全模式和二进制输出。"
...
<Warning>
`qwen-image`、`qwen-image-3` 和 `qwen-image-3-pro` 不再接受 `width` 或 `height`。发送任一参数都会返回 `400` 及 *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."* 在选择尺寸字段之前，请先通过 `GET /models?type=image` 检查 `model_spec.constraints`。
</Warning>
...
样式名称位于 `data` 内部，而不在顶层——请读取 `response.data`，而不是直接迭代响应对象。
...
| `width` | integer | 否 | `1024` | 基于像素的模型（如 `venice-sd35`）的输出宽度（像素）。会被 Qwen Image 系列拒绝——见下方说明 |
| `height` | integer | 否 | `1024` | 基于像素的模型（如 `venice-sd35`）的输出高度（像素）。会被 Qwen Image 系列拒绝——见下方说明 |
...
| `lora_strength` | integer | 否 | - | 模型内置 LoRA 的强度（`0`-`100`）；没有额外 LoRA 的模型会忽略该参数 |
...
某些图像模型内置了额外的 LoRA。`lora_strength` 参数（整数，`0` 到 `100`）控制这些内置 LoRA 的应用强度。它只影响使用额外 LoRA 的模型。其他模型会忽略它。

API 不支持上传或加载自定义 LoRA 文件。也没有可以列出模型所捆绑 LoRA 的端点。LoRA 的使用仅限于在包含 LoRA 的模型上调整 `lora_strength`。
...
### 在 `/images/generations` 上控制成人内容模糊

OpenAI 兼容端点不接受 `safe_mode`。发送该参数会返回 `400` 及 `Unrecognized key(s) in object: 'safe_mode'`。请改用 OpenAI 风格的 `moderation` 参数：

| 值 | 行为 |
|---|---|
| `auto`（默认） | 启用 Safe Venice 模式并模糊成人内容 |
| `low` | 禁用 Safe Venice 模式；返回未模糊输出 |
...
`safe_mode` 仅在原生 `POST /image/generate` 端点上有效。`moderation` 仅在 OpenAI 兼容的 `POST /images/generations` 端点上有效。
