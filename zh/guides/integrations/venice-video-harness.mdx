...
| 角色 | 默认模型 | 使用场景 |
|---|---|---|
| 角色镜头（1-2 个角色） | `seedance-2-0-reference-to-video-api` | 默认 R2V，采用扁平 `reference_image_urls`、`@Image` 标签，最长 15 秒，原生立体声音频 |
| 角色镜头（3+ 个角色） | `kling-o3-standard-reference-to-video` | 自动回退，采用结构化 `elements` 处理多角色身份 |
| 建立镜头/氛围/动作 | `seedance-2-0-image-to-video-api` | 无角色；史诗级电影质量，最长 15 秒 |
