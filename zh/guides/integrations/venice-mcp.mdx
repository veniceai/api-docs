...
description: "官方 Venice Model Context Protocol 服务器在任何 MCP 宿主中暴露 30 个 Venice 工具，用于聊天、图像、视频、音频、音乐和角色。"
...
[Venice MCP Server](https://github.com/veniceai/venice-mcp-server) 是 Venice 的官方 [Model Context Protocol](https://modelcontextprotocol.io/) 服务器。它将完整的 Venice API（聊天、图像、视频、音频、音乐、embeddings、web augment 和 character）作为 **30 个工具**暴露给任何兼容 MCP 的 agent。
...
<CardGroup cols={3}>
  <Card title="30 个工具" icon="toolbox">
    一个配置块覆盖每种 Venice 模态
  </Card>
...
**30 个工具**，覆盖每种 Venice 模态；**3 个资源**（`venice://models`、`venice://styles`、`venice://voices`）；**3 个 prompt 模板**。
