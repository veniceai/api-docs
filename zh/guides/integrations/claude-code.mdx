---
title: Claude Code
description: "使用开源 claude-code-router 本地代理配置 Claude Code CLI，将 Anthropic Claude Opus 与 Sonnet 4.5-4.8 及 Fable 5 请求通过 Venice 路由，获得私有、按 token 计费的编码 agent 推理与 prompt 缓存。"
...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 是 Anthropic 用于代理式编码的 CLI 工具。本指南将向您展示如何通过 Venice AI 运行它，以按 token 付费的方式访问 Claude Opus 4.5/4.6/4.7/4.8、Sonnet 4.5/4.6/4.7/4.8 和 Fable 5。
...
<CardGroup cols={3}>
  ...
  <Card title="Claude 模型" icon="microchip">
    通过 Venice 访问 Opus 4.5/4.6/4.7/4.8、Sonnet 4.5/4.6/4.7/4.8 和 Fable 5
  </Card>
...
<Steps>
  ...
  <Step title="获取您的 API 密钥">
    从 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) 生成密钥。您将在下一步直接在配置文件中粘贴它。
  </Step>
...
| Claude Opus 4.7 | `claude-opus-4-7` | 复杂推理、大型重构 |
| Claude Opus 4.7 Fast | `claude-opus-4-7-fast` | 低延迟的复杂推理 |
| Claude Opus 4.8 | `claude-opus-4-8` | 复杂推理、大型重构 |
| Claude Opus 4.8 Fast | `claude-opus-4-8-fast` | 低延迟的复杂推理 |
| Claude Fable 5 | `claude-fable-5` | 需要最高智能的复杂任务 |
