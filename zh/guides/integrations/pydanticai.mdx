...
<Note>
请使用 `OpenAIChatModel`（Chat Completions），而不是 `OpenAIResponsesModel` / `openai:` 简写。Venice 目前不提供 OpenAI 的 Responses API；文本生成路径是 `/chat/completions`。显式使用 Chat Completions 可以避免较新版本 PydanticAI 中 OpenAI 默认模型所采用的 Responses-only 行为。
</Note>
