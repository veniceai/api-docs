---
title: Venice CLI
description: "官方 Venice CLI：在终端中进行聊天、网页搜索、图像生成、文本转语音、语音转录与视频生成，并可选启用 E2EE。"
...
如果运行 `npm install -g veniceai-cli` 后终端提示无法识别 `venice`，说明 npm 的全局 bin 文件夹不在你的 `PATH` 中。这在 Windows 上很常见。

1. 运行 `npm config get prefix` 查找 npm 的全局安装文件夹。
2. 将全局 bin 文件夹添加到 `PATH`。在 Windows 上，它就是前缀文件夹本身（通常为 `%AppData%\npm`）。在 macOS 和 Linux 上，它是前缀下的 `bin` 子文件夹。
3. 打开新的终端窗口并运行 `venice chat "Hello"`。

如需跳过 `PATH` 设置，可改用 `npx` 运行 CLI，例如 `npx veniceai-cli chat "Hello"`。
