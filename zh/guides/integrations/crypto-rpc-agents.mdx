...
description: "将 Venice 同时用作自主 AI agent 的模型提供方和区块链 RPC 层：一份 API 凭证即可访问 230+ 模型和 11 条链（Ethereum、Base、Starknet 等）的 JSON-RPC 代理，支持 x402 钱包认证，无需 MetaMask。"
...
| 方法 | 最适合 | 工作原理 |
|---|---|---|
| **API 密钥** | 服务端 agent、固定部署 | `Authorization: Bearer <key>` 头。在 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) 获取密钥。 |
...
<Tip>
真正自主的 agent 可以通过在 Base 上质押 VVV 来铸造自己的 API 密钥。请参阅 [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent)。
</Tip>
...
上面的列表是一个快照。请调用 [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks)（一个无需身份验证的公共端点）获取实时的权威列表。
...
| **通过仪表板进行 USD 或加密货币充值** | 否（浏览器） | 使用相同的钱包登录 [venice.ai](https://venice.ai/?utm_source=venice-api-documentation)（Sign-In-With-Ethereum），然后在 Settings、API 中添加 credit。Stripe（信用卡）和 Coinbase（加密货币）都在该页面之后，并且需要浏览器。Credit 永不过期。 |
...
请参阅 [Autonomous Agent API Key Creation](/guides/integrations/generating-api-key-agent) 获取带代码和完整错误参考的完整演练。
...
<CardGroup cols={2}>
  ...
  <Card title="自主 Agent API 密钥" icon="robot" href="/guides/integrations/generating-api-key-agent">
    通过质押 VVV 铸造您自己的密钥
  </Card>
