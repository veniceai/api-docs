...
<Steps>
  ...
  <Step title="铸造 API 密钥">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` 返回一条绑定到该钱包、且在签发 15 分钟后过期的一次性 EIP-4361 挑战消息。使用质押钱包原样签署 `message`，然后将地址、签名和挑战消息 `POST` 回去。Venice 返回一个绑定到由该钱包派生的用户账户的 API 密钥。
  </Step>
</Steps>
