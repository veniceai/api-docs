...
description: "将 Venice 同时用作自主 AI agent 的模型提供方与区块链 RPC 层——一份 API 凭证即可访问 230+ 私有模型和 11 条链（Ethereum、Base、Arbitrum、Starknet 等），通过 x402 与质押 VVV 实现无 MetaMask、无人值守的推理与交易签名。"
