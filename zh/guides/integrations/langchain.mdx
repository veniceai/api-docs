...
Venice 仅嵌入文本。LangChain 的 `OpenAIEmbeddings` 默认使用 `check_embedding_ctx_length=True`，会通过 tiktoken 进行分词，并可能发送整数 token 数组。Venice 会用 HTTP 400 拒绝这类请求，因此请设置 `check_embedding_ctx_length=False`。
...
| 复杂推理 | `z-ai-glm-5-3-flash` | 快速的私有推理模型 |
| 函数调用 | `z-ai-glm-5-3-flash` | 可靠的工具调用 |
