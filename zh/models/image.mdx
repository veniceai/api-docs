---
title: "图像模型"
description: "用于生成、编辑、背景移除与放大的 Venice 图像模型，包括 Flux 2、GPT Image、Grok Imagine 与 Chroma。"
...
在 API 请求中，将 `id` 值用作 `model` 参数。当前有 66 个模型可用。
...
<Note>
请参阅 [Image Generate API](/zh/api-reference/endpoint/image/generate) 进行文生图，[Upscale API](/zh/api-reference/endpoint/image/upscale) 进行增强，以及 [Edit API](/zh/api-reference/endpoint/image/edit) 进行 inpainting。
</Note>
...
<Tip>
**质量等级。** `gpt-image-2`（生成）和 `gpt-image-2-edit`（编辑/多图像编辑）接受可选的 `quality` 参数 —— `low`、`medium` 或 `high`（默认）。较低的等级费用更低；完整的价格矩阵请参阅[定价概览](/zh/overview/pricing#image-generation)，或从 [Models 端点](/zh/api-reference/endpoint/models/list)获取 `model_spec.pricing.quality`。
</Tip>
