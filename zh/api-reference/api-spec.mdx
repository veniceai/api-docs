---
title: 简介
description: "Venice API 参考文档，涵盖身份验证、调试、OpenAI 兼容性、响应标头、错误处理以及所支持端点的完整列表。"
...
Venice API 使用 API 密钥进行身份验证。在您的 [API 设置](https://venice.ai/settings/api?utm_source=venice-api-documentation)中创建和管理您的 API 密钥。

大多数 API 请求都需要 HTTP Bearer 身份验证。少数发现类端点——包括 `GET /models`、`GET /models/traits`、`GET /models/compatibility_mapping` 和 `GET /image/styles`——无需密钥即可读取：
...
密钥分为 `INFERENCE` 和 `ADMIN` 两种。推理密钥可以调用推理端点——聊天、图像、视频、音频、嵌入向量以及增强类端点。账户管理端点需要 `ADMIN` 密钥：

| 端点 | 所需密钥 |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`、`GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | 两者均可 |
| `GET /billing/balance`、`/billing/usage-history`、`/billing/usage-analytics` | `ADMIN` |

使用推理密钥调用仅限管理员的端点会返回 `401` 及 `{"error":"Admin API key required"}`。请注意，`GET /api_keys/rate_limits` 是该组中唯一的例外——它可以使用任一密钥类型。
...
<Warning>
base URL 是 `https://api.venice.ai/api/v1`，而不是 `https://venice.ai`。发送到网站域名的请求会到达 Web 应用而非 API，并因 `405 Method Not Allowed` 之类的错误而失败。
</Warning>
...
| `enable_web_citations` | boolean | 启用网页搜索时，请求 LLM 使用插入符上标格式引用其来源——单个来源为 `^1^`，多个来源为 `^1,2^`。方括号形式（如 `[1]`、`(1)` 和 `<sup>1</sup>`）会被显式抑制 | `false` |
...
6. **Embeddings 输入**：`input` 必须是字符串或字符串数组。token ID 数组会被拒绝并返回 HTTP 400。LangChain `OpenAIEmbeddings` 必须设置 `check_embedding_ctx_length=False`
