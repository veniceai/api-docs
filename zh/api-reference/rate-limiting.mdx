...
视频、音乐和语音转换任务不受速率限制。它们均按每次生成从您的积分余额中计费，因此实际的约束是成本而非请求上限。请先使用 [`POST /video/quote`](/zh/api-reference/endpoint/video/quote)、[`POST /audio/quote`](/zh/api-reference/endpoint/audio/quote) 或 [`POST /audio/voice-changer/quote`](/zh/api-reference/endpoint/audio/voice-changer/quote) 对任务进行估价。
