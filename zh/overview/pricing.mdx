---
title: API 定价
description: "Venice API 针对文本、图像、视频、音频和音乐模型的定价，包含以美元、VVV 和 DIEM 计的按 token、按次生成和按秒费率。"
"og:title": "Pricing | Venice API Docs"
"og:description": "了解 Venice API 的定价。"
---

除非另有说明，价格按每 1M token 计。所有价格以美元计。1 Diem = $1/天 的计算量。
...
*每 1M token 的价格。[查看所有模型 →](/zh/models/text)*
...
> **多张输入图像的编辑：** **Per Edit** 价格已包含第一张输入图像。列出 **Extra Input Image** 价格的模型会为第一张之后的每张额外输入图像收取该费用。示例：在每次编辑 $0.11、额外图像费用为 $0.0035 的模型上使用 3 张输入图像编辑，费用为 $0.11 + 2 × $0.0035 = $0.117。
...
<Info>
如需在生成前获得准确定价，请使用 [Audio Quote API](/zh/api-reference/endpoint/audio/quote)。按时长分级的模型有固定的价格档位，而按秒计费的模型则根据输出长度收费。
</Info>
...
视频定价因分辨率和时长而异。请访问[视频模型页面](/zh/models/video)获取准确报价，或使用 [Video Quote API](/zh/api-reference/endpoint/video/quote)。
...
| MiniMax H3 Max Multi-Angle | `minimax-h3-max-multi-angle` | 文生视频 | Variable | Private |
...
<Info>
**Web Scraping** 自动检测每个请求最多 5 个 URL，抓取并将内容转换为结构化 markdown，并将提取的文本添加到模型上下文中。仅对成功抓取的 URL 计费。

**X Search** 为支持的 Grok 模型（如 `grok-4-20-beta`）启用 xAI 原生搜索。它搜索 web 和 X/Twitter 以获取实时信息。按模型返回的搜索结果计费（例如，如果模型返回 10 个搜索结果，您将按每个 $0.01 收取 10 个结果的费用 = $0.10）。

这些费用是在标准模型 token 定价之外的附加费用。
</Info>
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api?utm_source=venice-api-documentation">
    使用信用卡购买 Venice credit。Credit 永不过期。
  </Card>
  <Card title="加密货币" icon="bitcoin" href="https://venice.ai/settings/api?utm_source=venice-api-documentation">
    使用加密货币购买 Venice credit。费率与 USD 相同。
  </Card>
  <Card title="质押 DIEM" icon="coins" href="https://venice.ai/token?utm_source=venice-api-documentation">
    每个 Diem = $1/天 的 credit，每日刷新。需要一个已连接 Base 钱包的 Venice 账户。
  </Card>
</CardGroup>

### Pro 用户

Pro 是 Venice 网页和移动应用的订阅服务。它不包含免费或无限的 API 使用额度。所有 API 请求都按上述表格中的费率按 token、按图像或按次生成计费，与订阅等级无关。

Pro 订阅者在升级时确实会获得**一次性 $10 Venice credit**。该额度会作为打包额度用于 API 使用：API key 按 **DIEM → 打包额度 → USD** 的顺序从关联账户中扣费，因此只有在当前 epoch 已质押的 DIEM 额度耗尽后，才会动用这 $10。有关完整的扣费顺序，请参阅 [VVV 与 DIEM](/overview/vvv-diem)；要查看您的 key 下一次将以哪种货币计费，请参阅 [Billing Balance](/api-reference/endpoint/billing/balance)。

### 免费套餐

没有免费的 API 套餐，也没有免费试用 key。新生成的 key 可以通过身份验证，但付费端点（聊天补全、图像生成、音频、视频、嵌入）会返回 `402 INSUFFICIENT_BALANCE`，直到账户在 DIEM、打包额度或 USD 中拥有可支付的余额。请在 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) 使用 USD 或加密货币为账户充值，或质押 DIEM 以获取每日刷新的额度。
