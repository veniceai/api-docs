---
title: API 定价
description: "Venice API 文本、图像、视频、音频和向量嵌入模型按每 100 万 token 计费的价格，以及网络搜索、RPC 等服务的额度费用一览。"
...
Venice API 上没有零成本模型 —— 每个模型都有按 token、按图像、按秒或按请求的价格。Venice.ai Pro 订阅的每日图像和消息额度仅适用于网页应用和移动应用，不适用于 API 调用。

在没有按请求 USD 费用的情况下使用 API 的两种方式是：

- **质押 VVV 获取每日 DIEM credit。** VVV 质押者按其在质押池中的份额比例获得每日 DIEM 分配。每 1 DIEM = $1/天 的推理 credit，在 00:00 UTC 刷新。参见下文 [通过质押 VVV 获取每日 credit](#earn-daily-credits-by-staking-vvv) 部分。
- **Pro 订阅 credit。** Pro 订阅者升级时会获得一次性 $10 API credit。它用于测试和小型应用，而不是持续的免费使用。

`stt-xai-v1` 的语音转文本列价为每音频秒 $0.0000，但这是舍入的假象 —— 使用仍会从您的余额中计量，并由上游提供商定价。

### 通过质押 VVV 获取每日 credit

在 Base 上质押 VVV 会为您的钱包提供每日 DIEM 额度，该额度会在任何 USD 余额之前自动扣除。

1. **在 Base 上获取 VVV。** 在 Aerodrome 或 Uniswap 等 DEX 上购买或兑换成 VVV（合约地址 `0xacfE6019Ed1A7Dc6f7B508C02d1b04ec88cC21bf`）。您在 Base 上还需要少量 ETH 用于 gas。
2. **在 [venice.ai/token](https://venice.ai/token) 上进行质押。** 质押按钮位于该页面。在底层，它是 [Venice 质押合约](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff) 上的两笔链上交易：先在 VVV 代币上执行 `approve`，然后在质押合约上执行 `stake`。您的 VVV 余额减少，sVVV 余额增加相同的数额。
3. **每日获取 DIEM。** 您钱包的每日 DIEM 分配与其在质押池中的份额成正比，并在每个纪元于 00:00 UTC 刷新。您的账户需要至少 **累计 0.1 DIEM** 后，DIEM 才可用于消费。
4. **在 API 上消费。** 当您调用 API 时，铸造的密钥优先从 DIEM 中扣除，然后是打包的 credit，最后是 USD。当 DIEM 覆盖支出时，不收取任何按请求的费用。

对于从质押钱包铸造自己的 API 密钥且无人工干预的 agent，请参见 [自主 agent API 密钥创建](/guides/getting-started/generating-api-key-agent)。

### Pro 用户

Pro 订阅者升级到 Pro 时会获得一次性 $10 API credit。用它来测试和构建小型应用。Venice.ai 应用的每日消息和图像额度与此 credit 分开，不适用于 API 请求。

## 无审查和宽松模型

Venice 的内容政策比大多数上游提供商更为宽松，上述目录中的多个模型在发布时未附带上游提供商的安全层。您可以通过其 ID 或显示名称识别它们：

- **`*-uncensored*`** —— 由 Venice 微调或 Venice 精选的检查点，跳过了提供商的默认拒绝训练。例如：`venice-uncensored-1-2`、`gemma-4-uncensored`、`qwen-edit-uncensored`、`wan-2-7-uncensored-image-to-video`。
- **`*-heretic*`** —— 社区微调的检查点（例如 `olafangensan-glm-4.7-flash-heretic`），在提供给 Venice 之前经过了去审查微调。基础权重来自模型 ID 中命名的上游提供商；`heretic` 后缀标识微调版本，而非基础模型。
- **角色扮演变体** —— 例如 `venice-uncensored-role-play`，针对长篇角色和创意写作进行了调优。

Venice 上的非无审查模型对于上游提供商拒绝的提示，仍会返回 `CONTENT_POLICY_VIOLATION`（HTTP 422）。完整列表请参见 [错误代码](/api-reference/error-codes)。如果您想要最宽松的行为，请选择 `uncensored` 或 `heretic` 变体，并使用 `venice_parameters.include_venice_system_prompt: false` 禁用 Venice 的默认系统提示 —— 参数参考请参见 [API 规范](/api-reference/api-spec)。
