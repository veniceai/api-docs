...
> **使用多张输入图像进行编辑：** **Per Edit** 价格包含第一张输入图像。列出了 **额外输入图像** 价格的模型会针对第一张之后的每张额外输入图像收取该费用。示例：在每次编辑收费 $0.11、每张额外图像收费 $0.0035 的模型上使用 3 张输入图像进行编辑，总费用为 $0.11 + 2 × $0.0035 = $0.117。
