...
<Info>
**API credits 以美元计价。** $1 USD 即可购买 $1 的 API credits — 不存在单独的 token、乘数或兑换率。Credits 永不过期。

API 账单与 venice.ai 的消费者订阅（Pro、Plus、Max）**完全分开**。这些套餐覆盖 venice.ai 应用内的使用，**不**包含持续的 API 使用额度。关于升级到 Pro 时附赠的一次性 API credit，请参见下方的 [Pro 用户](#pro-用户) 部分。
</Info>
...
升级到 Pro 时，Pro 订阅者会获得**一次性 $10 API credit**。用它来测试和构建小型应用。

<Note>
venice.ai 的消费者订阅（Pro、Plus、Max）覆盖的是 **venice.ai 应用内**的使用——它们不包含按周期发放的 API credit 额度。无论你使用哪个消费者套餐，API 请求都会按上述每个模型的费率从你的 USD 或 DIEM 余额中扣费。升级到 Pro 时附赠的一次性 $10 credit 是订阅中唯一附带的 API 额度。
</Note>
