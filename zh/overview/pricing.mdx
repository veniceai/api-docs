---
title: Venice API 定价
description: "Venice API 完整定价参考：文本对话、图像生成、视频合成、语音音频转录和嵌入向量模型均按每 1M token 以美元计费，另附网络搜索查询、网页抓取、文档解析、Crypto RPC 调用以及 x402 钱包支付等服务的额度消耗费率与每日 Diem 折算说明。"
...
<Info>
**API 与消费者套餐。** Venice API 按**按量付费**方式以美元 credit 计费（或通过质押 DIEM）。它与 Venice 消费者 Web 应用的月度订阅套餐（Free、Pro、Max）是分开的，那些套餐的额度——例如图像次数或无限聊天——仅适用于 Web/桌面/移动应用，**不**适用于 API 使用。所有 API 调用都会按您的 API credit 余额计量，与您的消费者套餐无关。请参阅下方的 [Pro 用户](#pro-用户) 了解 Pro 订阅者获得的一次性 credit。
</Info>
