---
title: API 定价
description: "Venice 文本、图像、音频、视频与嵌入模型的按 token 和按请求计费费率，以及网页搜索定价与各种支付选项。"
...
*价格按每 1M token 计。[查看所有模型 →](/models/text)*
...
<Info>
要在生成前获取精确定价，请使用 [Audio Quote API](/api-reference/endpoint/audio/quote)。基于时长的模型有固定的价格层级，而按秒模型根据输出长度收费。
</Info>
...
视频定价因分辨率和时长而异。访问 [视频模型页面](/models/video) 获取精确报价，或使用 [Video Quote API](/api-reference/endpoint/video/quote)。
...
API 使用始终按需付费。API 没有免费层级，也没有无限量的 API 套餐：本页列出的每个模型都按 token、按图像或按请求计费。

Venice 应用订阅（即 [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) 聊天界面所使用的套餐）与 API 用量分开计费。其套餐层级、价格和应用内权益不在本 API 文档的涵盖范围内。请在您的 Venice 账户中管理它们。

订阅附带的 credit（例如一次性 $10 Pro credit）会以**打包额度**（bundled credits）的形式显示在您的账户中。API 按以下顺序消耗账户余额：先 DIEM，然后是打包额度，最后是 USD。请参阅 [VVV & DIEM](/overview/vvv-diem#how-funding-works) 了解各资金来源的运作方式以及如何查询余额。
