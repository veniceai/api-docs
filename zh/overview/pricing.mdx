---
title: Venice API 定价
description: "Venice API 定价详情页面：文本、图像、视频、音频与 embedding 等各类 AI 推理模型均按每 1M token 单位计费，同时列出 web 搜索、RPC 调用以及 DIEM 质押访问对应的额度消耗费率、账户充值的支付方式以及计费单位的详细说明。"
...
<Note>
[venice.ai](https://venice.ai) 消费者应用（Web、桌面和移动端聊天）上的 Free、Pro 和 Max 套餐与 API 是分开的。消费者订阅的限额——例如图像次数、字符配额或无限聊天——仅适用于这些应用，**不**适用于 API 使用。无论您使用哪种消费者套餐，API 调用始终按您的 Venice credit 余额计量。请参阅下方的 [Pro 用户](#pro-用户) 了解 Pro 订阅者获得的一次性 credit。
</Note>
...
<Note>
[venice.ai](https://venice.ai) 上的每日免费提示额度是消费者 Web 应用的功能，而非 API 的功能。API 请求始终从你已充值的余额中扣除。
</Note>

### 将 Venice 密钥用于其他提供商

Venice API 密钥只能用于 `https://api.venice.ai/api/v1`。它们不能与 OpenRouter、OpenAI、Anthropic 或其他提供商的密钥互换，来自这些提供商的密钥也无法在 Venice 上通过身份验证。要使用 Venice 模型，请在 [venice.ai/settings/api](https://venice.ai/settings/api) 生成密钥，并将你的客户端指向 Venice 的基础 URL。
