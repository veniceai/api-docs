...
Venice API 采用**按量付费**模式：每个请求都会被计量并从您账户的 API credit 余额中扣费（以美元充值，或通过质押 DIEM）。API 没有免费额度、没有每月配额，也没有基于登录的订阅套餐——访问通过 API key 进行身份验证，并按请求计费。

<Note>
[venice.ai](https://venice.ai) 消费者应用（Web、桌面和移动端聊天）上的 Free、Pro 和 Max 套餐与 API 是分开的。消费者订阅的限额——例如图像次数、字符配额或无限聊天——仅适用于这些应用，**不**适用于 API 使用。无论您使用哪种消费者套餐，API 调用始终按您的 API credit 余额计量。请参阅下方的 [Pro 用户](#pro-用户) 了解 Pro 订阅者获得的一次性 credit。
</Note>
...
升级到 Pro 时，Pro 订阅者会获得一次性 $10 API credit。用它来测试和构建小型应用。
