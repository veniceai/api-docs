---
title: Venice API 定价
description: "Venice API 按每 1M token 计费的文本、图像、视频、音频与 embedding 模型价格，以及 web 搜索、RPC 和 DIEM 费用。"
...
Venice API 采用**按量付费**模式：每个请求都会被计量并从您账户的 Venice credit 余额中扣费（以美元充值，或通过质押 DIEM）。API 没有免费额度、没有每月配额，也没有基于登录的订阅套餐——访问通过 API key 进行身份验证，并按请求计费。

<Note>
[venice.ai](https://venice.ai/?utm_source=venice-api-documentation) 消费者应用（Web、桌面和移动端聊天）上的 Free、Pro 和 Max 套餐与 API 是分开的。消费者订阅的限额——例如图像次数、字符配额或无限聊天——仅适用于这些应用，**不**适用于 API 使用。无论您使用哪种消费者套餐，API 调用始终按您的 Venice credit 余额计量。请参阅下方的 [Pro 用户](#pro-用户) 了解 Pro 订阅者获得的一次性 credit。
</Note>

要使用 API，您需要一个 [API key](/guides/getting-started/generating-api-key) 以及可用余额，余额可通过下方的[支付选项](#支付选项)之一充值。本页列出的每个模型都可供任何具有足够余额的 API key 使用——API 上不存在单独的"基础"与"Pro"模型区分。
...
除此 credit 外，API 使用仍按上文列出的相同按量付费费率计费——Pro 订阅本身不包含额外的 API 额度，例如包含的图像、字符或 token。某些模型仅限 Pro 账户使用（`PRO_ONLY_MODEL`）；从没有有效 Pro 订阅的账户调用它们会返回 `AUTHENTICATION_FAILED_INACTIVE_KEY`。请参阅[错误代码](/api-reference/error-codes)。

### 免费层级

Venice API 没有免费层级。每一次聊天、图像、音频、视频和嵌入调用都通过 credit、DIEM 或 [x402/USDC](/guides/integrations/x402-venice-api) 付费。成本最低的文本模型起价为每 100 万输入 token $0.05，嵌入模型起价为每 100 万 token $0.01。

<Note>
[venice.ai](https://venice.ai/?utm_source=venice-api-documentation) 上的每日免费提示额度是消费者 Web 应用的功能，而非 API 的功能。API 请求始终从你已充值的余额中扣除。
</Note>

### 将 Venice 密钥用于其他提供商

Venice API 密钥只能用于 `https://api.venice.ai/api/v1`。它们不能与 OpenRouter、OpenAI、Anthropic 或其他提供商的密钥互换，来自这些提供商的密钥也无法在 Venice 上通过身份验证。要使用 Venice 模型，请在 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) 生成密钥，并将你的客户端指向 Venice 的基础 URL。
