---
title: API 定价
description: "Venice API 全部模型定价：聊天补全按 token 计价，图像、视频与音频按生成计价，支持 USD、VVV 与 DIEM 支付。"
...
*价格按每 1M token 计。[查看所有模型 →](/zh/models/text)*
...
> **使用多张输入图像进行编辑：** **Per Edit** 价格包含第一张输入图像。列出了 **Extra Input Image** 价格的模型会针对第一张之后的每张额外输入图像收取该费用。示例：在每次编辑收费 $0.11、每张额外图像收费 $0.0035 的模型上使用 3 张输入图像进行编辑，总费用为 $0.11 + 2 × $0.0035 = $0.117.
...
<Info>
要在生成前获取精确定价，请使用 [Audio Quote API](/zh/api-reference/endpoint/audio/quote)。基于时长的模型有固定的价格层级，而按秒模型根据输出长度收费。
</Info>
...
视频定价因分辨率和时长而异。访问 [视频模型页面](/zh/models/video) 获取精确报价，或使用 [Video Quote API](/zh/api-reference/endpoint/video/quote)。
...
### Venice.ai 应用订阅与 API 计费

Venice.ai 的消费者订阅计划（例如 [venice.ai](https://venice.ai) 上的 Pro 和 Max）适用于 [venice.ai](https://venice.ai) 上的 Venice 聊天、图像和视频应用。它们与 API 计费是分开的。

* API 使用始终按上文所列的每 token、每张图像、每秒或每次生成的价格逐请求计费。不存在任何无限或不计量的 API 层级。
* 应用订阅权益 — 无限聊天提示、应用内视频、音乐和 frontier 生成的每月 credit 额度，以及 credit banking — 仅在 Venice 应用内有效。除上文提到的 Pro 订阅者一次性 $10 API credit 外，这些权益不会授予 API 访问权限，也不会转换为 API credit。
* 要为 API 请求充值，可在 [API 设置](https://venice.ai/settings/api?utm_source=venice-api-documentation)页面用 USD 或加密货币购买 Venice credit，或质押 DIEM 获取每日额度（参见 [VVV 与 DIEM](/overview/vvv-diem)）。
