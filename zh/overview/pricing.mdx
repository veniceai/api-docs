...
<CardGroup cols={3}>
  ...
  <Card title="质押 DIEM" icon="coins" href="/overview/vvv-diem">
...
打包额度是通过 Venice 订阅计划附加到你的 Venice 账户上的 API credit（例如，新 Pro 订阅者获得的一次性额度）。它们与 DIEM 和 USD 额度一同被 API 消耗，并在计费响应中以 `BUNDLED_CREDITS` 币种显示。

- **覆盖范围** —— 打包额度可用于任何付费的 Venice API 端点（chat、images、audio、video、embeddings）。它们按本页相同的 USD 价目表计价。
- **扣费顺序** —— API key 按以下顺序从你的账户中扣费：**DIEM → 打包额度 → USD**。你无需为每次请求指定币种。请参阅[资金如何运作](/overview/vvv-diem#how-funding-works)。
- **在何处查看余额** —— 在 [venice.ai/settings/api](https://venice.ai/settings/api) 查看当前余额，或调用 [Billing Balance](/api-reference/endpoint/billing/balance) 端点并读取 `BUNDLED_CREDITS` 条目。

<Note>
  [Venice 消费者计划](https://venice.ai) 页面上显示的任何每月额度（例如，用于 venice.ai 聊天应用或媒体生成器的额度）与 API 打包额度是分开的，可能无法通过本 API 使用。Venice API 仅消耗由 [Billing Balance](/api-reference/endpoint/billing/balance) 端点报告的 DIEM、打包和 USD 余额。
</Note>
