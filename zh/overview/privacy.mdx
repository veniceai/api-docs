---
title: 隐私
description: "详解 Venice 如何处理 prompt、响应与元数据：请求经由加密代理转发、默认不留日志，并通过 Anonymous、Private、Trusted Execution Environment 与端到端加密四种隐私模式，分层保护您的推理请求免受提供商、宿主与 Venice 自身访问。"
...
| 模式 | 模型 ID 前缀 | 保护方式 |
| --- | --- | --- |
| **TEE** | `e2ee-*` | 支持远程证明的硬件隔离推理。 |
| **E2EE** | `e2ee-*` | 从客户端加密到已验证的 TEE。 |

两种模式使用相同的模型 ID。请求流程决定模型是仅以 TEE 模式运行，还是使用客户端 E2EE。
