...
description: "在 Venice 之上使用 Axum、Postgres 和 SQLx 构建兼容 OpenAI 的 Rust LLM 网关：暴露 /v1/chat/completions 端点、接受自有 bearer token 认证、支持流式响应与限流，并发出 OpenTelemetry span 和指标用于可观测性。"
