...
Cuando se produce un error, la API de Venice devuelve un código de estado HTTP y un mensaje descriptivo. Algunos errores también incluyen campos adicionales, como el estado del reembolso, modelos sugeridos para reintento, detalles de validación o IDs de solicitud.

## Formato de respuesta de error

La mayoría de los errores devuelven un campo `error` con un mensaje legible:
...
Algunos endpoints incluyen campos adicionales específicos del endpoint. Por ejemplo, los errores de moderación de video pueden incluir si se reembolsaron créditos:
...
Los errores de validación pueden incluir los campos `details` e `issues` con información de validación a nivel de campo:
...
Algunos endpoints compatibles con OpenAI pueden devolver objetos de error con el estilo de OpenAI para casos específicos, como errores de longitud de contexto:
