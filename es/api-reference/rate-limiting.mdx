...
Los trabajos de vídeo, música y cambiador de voz no tienen límites de velocidad. Se facturan por generación contra tu saldo de créditos, por lo que la restricción práctica es el coste y no un tope de solicitudes. Calcula primero el precio de un trabajo con [`POST /video/quote`](/es/api-reference/endpoint/video/quote), [`POST /audio/quote`](/es/api-reference/endpoint/audio/quote) o [`POST /audio/voice-changer/quote`](/es/api-reference/endpoint/audio/voice-changer/quote).
