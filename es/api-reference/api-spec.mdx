---
title: Introducción
description: "Referencia de la API de Venice: autenticación, depuración, compatibilidad con OpenAI, cabeceras de respuesta, errores y endpoints admitidos."
...
La API de Venice usa API keys para la autenticación. Crea y gestiona tus API keys en tu [configuración de API](https://venice.ai/settings/api?utm_source=venice-api-documentation).


La mayoría de las solicitudes a la API requieren autenticación HTTP Bearer. Algunos endpoints de descubrimiento — incluidos `GET /models`, `GET /models/traits`, `GET /models/compatibility_mapping` y `GET /image/styles` — se pueden leer sin una key:
...
Las keys son de tipo `INFERENCE` o `ADMIN`. Las keys de inferencia pueden llamar a los endpoints de inferencia: chat, imagen, vídeo, audio, embeddings y los endpoints de augmentación. Los endpoints de gestión de la cuenta requieren una key `ADMIN`:

| Endpoint | Key requerida |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`, `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | Cualquiera |
| `GET /billing/balance`, `/billing/usage-history`, `/billing/usage-analytics` | `ADMIN` |

Llamar a un endpoint exclusivo de administrador con una key de inferencia devuelve `401` con `{"error":"Admin API key required"}`. Ten en cuenta que `GET /api_keys/rate_limits` es la única excepción de ese grupo: funciona con cualquiera de los dos tipos de key.
...
<Warning>
La URL base es `https://api.venice.ai/api/v1`, no `https://venice.ai`. Las solicitudes enviadas al dominio del sitio web llegan a la aplicación web en lugar de a la API y fallan con errores como `405 Method Not Allowed`.
</Warning>
...
| `enable_web_citations` | boolean | Cuando la búsqueda web está activada, solicita al LLM que cite sus fuentes utilizando superíndices con caret: `^1^` para una sola fuente y `^1,2^` para varias. Las formas con corchetes como `[1]`, `(1)` y `<sup>1</sup>` se suprimen explícitamente | `false` |
...
6. **Entrada de embeddings**: `input` debe ser una cadena o un arreglo de cadenas. Los arreglos de IDs de tokens se rechazan con HTTP 400. LangChain `OpenAIEmbeddings` debe establecer `check_embedding_ctx_length=False`
