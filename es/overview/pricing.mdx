...
description: "Precios de Venice API por 1M tokens para modelos de texto, imagen, video, audio y embeddings, más créditos para búsqueda web, RPC y otros servicios."
...
| Hunyuan Image 3.0 (Beta) | `hunyuan-image-v3` | Por imagen: $0.09 | Private |
| Krea v2 Large (Beta) | `krea-v2-large` | Por imagen: $0.07 | Anonymized |
| Krea v2 Medium (Beta) | `krea-v2-medium` | Por imagen: $0.04 | Anonymized |
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api">
    Compra créditos de la API con tarjeta de crédito. Los créditos no caducan.
  </Card>
  <Card title="Cripto" icon="bitcoin" href="https://venice.ai/settings/api">
    Compra créditos de la API con criptomonedas. Mismas tarifas que en USD.
  </Card>
...
### ¿Hay un nivel gratuito de la API?

En la API de Venice no existe ningún modelo con coste cero — todos los modelos tienen un precio por token, por imagen, por segundo o por solicitud. Las asignaciones diarias de imágenes y mensajes de la suscripción Venice.ai Pro se aplican únicamente a la aplicación web y a la app móvil, no a las llamadas a la API.

Las dos formas de usar la API sin un cargo en USD por solicitud son:

- **Hacer staking de VVV para obtener créditos DIEM diarios.** Los stakers de VVV reciben una asignación diaria de DIEM proporcional a su parte del pool de staking. Cada 1 DIEM = $1/día en créditos de inferencia que se renuevan a las 00:00 UTC. Consulta la sección [Gana créditos diarios haciendo staking de VVV](#gana-creditos-diarios-haciendo-staking-de-vvv) a continuación.
- **Crédito de la suscripción Pro.** Los suscriptores Pro reciben un crédito único de $10 para la API al actualizarse. Está pensado para pruebas y aplicaciones pequeñas, no para un uso gratuito sostenido.

Voz a texto con `stt-xai-v1` aparece a $0.0000 por segundo de audio, pero se trata de un artefacto de redondeo — el uso se sigue midiendo contra tu saldo y lo tarifica el proveedor original.

### Gana créditos diarios haciendo staking de VVV

Hacer staking de VVV en Base otorga a tu wallet una asignación diaria de DIEM que se gasta automáticamente antes que cualquier saldo en USD.

1. **Consigue VVV en Base.** Compra o intercambia por VVV (contrato `0xacfE6019Ed1A7Dc6f7B508C02d1b04ec88cC21bf`) en un DEX como Aerodrome o Uniswap. También necesitas una pequeña cantidad de ETH en Base para el gas.
2. **Haz staking en [venice.ai/token](https://venice.ai/token).** El botón de staking está en esa página. Internamente son dos transacciones on-chain sobre el [contrato de staking de Venice](https://basescan.org/address/0x321b7ff75154472B18EDb199033fF4D116F340Ff): `approve` sobre el token VVV y luego `stake` sobre el contrato de staking. Tu saldo de VVV disminuye y tu saldo de sVVV aumenta en la misma cantidad.
3. **Gana DIEM a diario.** La asignación diaria de DIEM de tu wallet es proporcional a su parte del pool de staking y se renueva cada epoch a las 00:00 UTC. Tu cuenta necesita al menos **0,1 DIEM acumulados** antes de que los DIEM sean gastables.
4. **Gástalos en la API.** Cuando llamas a la API, la clave emitida gasta primero de DIEM, luego de los créditos incluidos y por último de USD. No se aplica ningún cargo por solicitud mientras los DIEM cubran el gasto.

Para agentes que emiten su propia clave de API desde una wallet con staking sin intervención humana, consulta [Creación autónoma de claves de API para agentes](/guides/getting-started/generating-api-key-agent).

### Usuarios Pro

Los suscriptores Pro reciben un crédito único de $10 para la API al actualizarse a Pro. Úsalo para probar y construir aplicaciones pequeñas. Las asignaciones diarias de mensajes e imágenes de la app de Venice.ai son independientes de este crédito y no se aplican a las solicitudes de la API.

## Modelos sin censura y permisivos

La política de contenido de Venice es más permisiva que la de la mayoría de los proveedores originales, y varios modelos del catálogo anterior se ofrecen sin la capa de seguridad del proveedor original. Puedes identificarlos por su ID o nombre de visualización:

- **`*-uncensored*`** — checkpoints ajustados o seleccionados por Venice que omiten el entrenamiento de rechazo por defecto del proveedor. Ejemplos: `venice-uncensored-1-2`, `gemma-4-uncensored`, `qwen-edit-uncensored`, `wan-2-7-uncensored-image-to-video`.
- **`*-heretic*`** — checkpoints ajustados por la comunidad (p. ej. `olafangensan-glm-4.7-flash-heretic`) que pasan por un fine-tune de descensura antes de ofrecerse en Venice. Los pesos base provienen del proveedor original nombrado en el ID del modelo; el sufijo `heretic` identifica el fine-tune, no el modelo base.
- **Variantes de role-play** — p. ej. `venice-uncensored-role-play`, ajustadas para escritura creativa y de personajes de formato largo.

Los modelos no uncensored en Venice todavía devuelven `CONTENT_POLICY_VIOLATION` (HTTP 422) para los prompts que el proveedor original rechaza. Consulta [Códigos de error](/api-reference/error-codes) para ver la lista completa. Si quieres el comportamiento más permisivo, elige una variante `uncensored` o `heretic` y deshabilita el prompt de sistema por defecto de Venice con `venice_parameters.include_venice_system_prompt: false` — consulta la [Especificación de la API](/api-reference/api-spec) como referencia del parámetro.
