...
description: "Precios de la API de Venice por 1M de tokens para modelos de texto, imagen, video, audio y embeddings, más búsqueda web, RPC y DIEM."
...
La API de Venice es **de pago por uso**: cada solicitud se mide y se cobra contra el saldo de créditos de Venice de tu cuenta (financiado en USD o mediante DIEM en staking). No hay nivel gratuito, no hay cuota mensual de API ni plan de suscripción basado en inicio de sesión para la API: el acceso se autentica mediante una clave de API y se factura por solicitud.

<Note>
Los planes Free, Pro y Max de la app de consumidor [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) (chat web, de escritorio y móvil) son independientes de la API. Los límites de suscripción de consumidor —como recuentos de imágenes, cuotas de caracteres o chat ilimitado— se aplican únicamente a esas apps, **no** al uso de la API. Las llamadas a la API siempre se contabilizan contra el saldo de créditos de Venice con independencia de tu plan de consumidor. Consulta [Usuarios Pro](#usuarios-pro) más abajo para el crédito único que reciben los suscriptores Pro.
</Note>
...
La API de Venice no tiene un nivel gratuito. Cada llamada de chat, imagen, audio, vídeo y embeddings se paga mediante créditos, DIEM o [x402/USDC](/guides/integrations/x402-venice-api). Los modelos de texto de menor costo comienzan en $0.05 por 1M de tokens de entrada, y los modelos de embeddings comienzan en $0.01 por 1M de tokens.

<Note>
La asignación diaria gratuita de prompts en [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) es una función de la aplicación web para consumidores, no de la API. Las solicitudes a la API siempre se descuentan de tu saldo cargado.
</Note>

### Uso de claves de Venice con otros proveedores

Las claves de la API de Venice solo funcionan contra `https://api.venice.ai/api/v1`. No son intercambiables con claves de OpenRouter, OpenAI, Anthropic u otros proveedores, y las claves de esos proveedores no autenticarán contra Venice. Para usar los modelos de Venice, genera una clave en [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) y apunta tu cliente a la URL base de Venice.
