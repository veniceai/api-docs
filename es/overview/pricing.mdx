...
<CardGroup cols={3}>
  ...
  <Card title="Staking de DIEM" icon="coins" href="/overview/vvv-diem">
...
Los créditos agrupados son créditos de la API que se añaden a tu cuenta de Venice mediante un plan de suscripción de Venice (por ejemplo, el crédito único que se otorga a los nuevos suscriptores Pro). La API los gasta junto con los créditos DIEM y USD, y aparecen en las respuestas de facturación como la moneda `BUNDLED_CREDITS`.

- **Qué cubren** — los créditos agrupados pueden gastarse en cualquier endpoint de pago de la API de Venice (chat, imágenes, audio, video, embeddings). Se cotizan con la misma hoja de tarifas en USD de esta página.
- **Orden de gasto** — las claves de API descuentan de tu cuenta en este orden: **DIEM → créditos agrupados → USD**. No necesitas elegir una moneda por solicitud. Consulta [Cómo funciona la financiación](/overview/vvv-diem#how-funding-works).
- **Dónde consultar el saldo** — revisa los saldos actuales en [venice.ai/settings/api](https://venice.ai/settings/api), o llama al endpoint [Billing Balance](/api-reference/endpoint/billing/balance) y lee la entrada `BUNDLED_CREDITS`.

<Note>
  Cualquier asignación mensual de créditos que se muestre en la página de [planes de consumidor de Venice](https://venice.ai) (por ejemplo, créditos para la aplicación de chat de venice.ai o los generadores multimedia) es independiente de los créditos agrupados de la API y puede no ser gastable a través de esta API. La API de Venice solo gasta los saldos de DIEM, agrupados y USD que reporta el endpoint [Billing Balance](/api-reference/endpoint/billing/balance).
</Note>
