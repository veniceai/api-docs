...
<Info>
**Los créditos de la API están denominados en USD.** $1 USD compra $1 en créditos de API — no hay token aparte, multiplicador ni tasa de conversión. Los créditos nunca caducan.

La facturación de la API es **independiente** de las suscripciones de consumidor de venice.ai (Pro, Plus, Max). Esos planes cubren el uso en la app de venice.ai y **no** incluyen el uso continuo de la API. Consulta [Usuarios Pro](#usuarios-pro) más abajo para conocer el crédito único de API incluido al pasar a Pro.
</Info>
...
<Note>
Las suscripciones de consumidor de venice.ai (Pro, Plus, Max) cubren el uso **dentro de la app de venice.ai** — no incluyen una asignación recurrente de créditos de API. Las solicitudes a la API se cobran de tu saldo en USD o DIEM a las tarifas por modelo indicadas arriba, sin importar el plan de consumidor que tengas. El crédito único de $10 al actualizarse a Pro es la única asignación de API incluida en una suscripción.
</Note>
