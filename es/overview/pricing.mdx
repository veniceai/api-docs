...
<Note>
La asignación diaria gratuita de prompts en [venice.ai](https://venice.ai) es una función de la aplicación web para consumidores, no de la API. Las solicitudes a la API siempre se descuentan de tu saldo cargado.
</Note>

### Uso de claves de Venice con otros proveedores

Las claves de la API de Venice solo funcionan contra `https://api.venice.ai/api/v1`. No son intercambiables con claves de OpenRouter, OpenAI, Anthropic u otros proveedores, y las claves de esos proveedores no autenticarán contra Venice. Para usar los modelos de Venice, genera una clave en [venice.ai/settings/api](https://venice.ai/settings/api) y apunta tu cliente a la URL base de Venice.
