...
description: "Construye sistemas multi-agente de IA con Venice AI y CrewAI"
"og:title": "Integración con CrewAI | Venice API Docs"
"og:description": "Crea crews autónomos de agentes de IA impulsados por los modelos privados y sin censura de Venice"
---

[CrewAI](https://www.crewai.com/) te permite construir sistemas multi-agente autónomos donde agentes de IA especializados colaboran en tareas complejas. Venice AI funciona como proveedor de LLM directo gracias a la compatibilidad con OpenAI.
...
Configura Venice como proveedor de LLM de CrewAI usando la interfaz compatible con OpenAI:
...
O configura por agente con el objeto LLM:
...
Crea un crew sencillo de investigación con dos agentes:
...
## Crew multi-agente de análisis de producto
...
# Modelos distintos para distintas capacidades de los agentes
...
# Red Team - se beneficia de un pensamiento sin censura
red_team = Agent(
    ...
    llm=uncensored_llm,  # Sin censura para una crítica honesta
    verbose=True,
)
...
Potencia los agentes con búsqueda web y otras herramientas:

<Note>
`SerperDevTool` requiere una variable de entorno `SERPER_API_KEY` desde [serper.dev](https://serper.dev). Como alternativa, puedes usar la búsqueda web integrada de Venice pasando `venice_parameters: {"enable_web_search": "auto"}` vía `model_kwargs` — sin API key adicional. Consulta la sección [Integración con búsqueda web](/es/guides/integrations/langchain#web-search-integration) de la guía de LangChain para un ejemplo.
</Note>
...
# Herramienta de búsqueda web (requiere variable de entorno SERPER_API_KEY)
...
## Guía de selección de modelo para CrewAI

Elige el modelo de Venice adecuado para cada rol de agente:

| Rol del agente | Modelo recomendado | Por qué |
|-----------|-------------------|-----|
| Razonamiento complejo / Estrategia | `zai-org-glm-5-1` | Mejor modelo privado de razonamiento |
| Análisis sin censura / Red team | `venice-uncensored-1-2` | Sin filtro de contenido |
| Alto volumen / tareas rápidas | `qwen3-5-9b` | El más barato: $0,10/1M de entrada y $0,15/1M de salida |
| Agentes de generación de código | `qwen3-coder-480b-a35b-instruct` | Optimizado para código |
| Tareas multimodales/visión | `qwen3-vl-235b-a22b` | Comprensión visual avanzada |
...
1. **Usa modelos más baratos para agentes más simples**: no todos los agentes necesitan un modelo flagship. Usa `qwen3-4b` para formatear, resumir o extraer datos sencillos.
...
4. **Limita las iteraciones máximas**: establece `max_iter` en los agentes para evitar consumos descontrolados de tokens:
   ```python
...
Las garantías de privacidad de Venice lo hacen ideal para casos de uso de CrewAI que involucren:
...
- **Ejercicios de red team** — Los modelos sin censura dan feedback honesto sin filtros de contenido
