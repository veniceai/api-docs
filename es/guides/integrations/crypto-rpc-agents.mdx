...
description: "Usa Venice como proveedor de modelos y como capa RPC de blockchain para agentes de IA autónomos — una credencial, más de 230 modelos, 11 cadenas, sin MetaMask."
...
|---|---|---|
| **API key** | Agentes del lado del servidor, despliegues fijos | Cabecera `Authorization: Bearer <key>`. Obtén una clave en [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation). |
...
<Tip>
Los agentes verdaderamente autónomos pueden acuñar su propia API key haciendo staking de VVV en Base. Consulta [Creación autónoma de API Key por agente](/guides/integrations/generating-api-key-agent).
</Tip>
...
La lista anterior es una instantánea. Llama a [`GET /crypto/rpc/networks`](/api-reference/endpoint/crypto/networks) — un endpoint público que no requiere autenticación — para la lista autoritativa en vivo.
...
| **Recarga en USD o cripto vía el dashboard** | No (navegador) | Inicia sesión en [venice.ai](https://venice.ai/?utm_source=venice-api-documentation) con el mismo monedero (Sign-In-With-Ethereum) y añade créditos en Settings, API. Tanto Stripe (tarjeta) como Coinbase (cripto) están detrás de esa página y requieren un navegador. Los créditos no caducan. |
...
Consulta [Creación autónoma de API Key por agente](/guides/integrations/generating-api-key-agent) para el recorrido completo con código y la referencia completa de errores.
...
<CardGroup cols={2}>
  ...
  <Card title="API Key de agente autónomo" icon="robot" href="/guides/integrations/generating-api-key-agent">
