...
<Steps>
  ...
  <Step title="Acuña una API key">
    `GET /api/v1/api_keys/generate_web3_key?address=<wallet>` devuelve un desafío EIP-4361 de un solo uso, vinculado a ese monedero, que expira 15 minutos después de su emisión. Firma el `message` tal cual con el monedero de staking y luego `POST` la dirección, la firma y el mensaje. Venice devuelve una API key vinculada a la cuenta de usuario derivada de ese monedero.
  </Step>
</Steps>
