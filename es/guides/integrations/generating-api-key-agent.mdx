...
<Steps>
  ...
  <Step title="Acuña la API key">
    ...
    - `consumptionLimit` — el límite de gasto de esta clave, denominado en `usd`, `vcu` o `diem`
    - `limitPeriod` — la ventana en la que se mide el límite: `EPOCH` (se restablece cada día UTC), `MONTH` (se restablece el día 1 de cada mes UTC) o `LIFETIME` (nunca se restablece, por lo que el límite es permanente)
    - `modelPrivacy` — qué modelos puede llamar la clave, según el nivel de privacidad: `ALL`, `PRIVATE_TEXT` o `PRIVATE_ONLY`

    <Warning>
    `limitPeriod` toma el valor predeterminado `EPOCH` cuando se omite, lo que significa que el límite **se restablece cada día** en lugar de ser un total de por vida. Una clave creada con `consumptionLimit: {"usd": 50}` y sin `limitPeriod` puede gastar $50 al día. Para la clave de un agente desatendido, establece `limitPeriod: "LIFETIME"` de forma explícita para que el límite sea un techo permanente.
    </Warning>
...
const mintResponse = await fetch("https://api.venice.ai/api/v1/api_keys/generate_web3_key", {
  ...
  body: JSON.stringify({
    ...
    // Un techo permanente de $50. Sin limitPeriod, se restablecería a diario.
