---
title: "Rig"
description: "Crea agentes tipados en Rust con el proveedor Venice nativo de Rig: chat completions, streaming, herramientas, salida estructurada y embeddings."
...
Si prefieres un proxy Axum en crudo en lugar de un framework de agentes, consulta [Cómo crear un LLM Gateway en Rust](/learn/rust-llm-gateway).
...
<Note>
Utiliza `venice::Client` en lugar de apuntar el cliente de OpenAI de Rig a Venice. El cliente `openai::Client` predeterminado apunta a la API Responses de OpenAI. El proveedor de Venice habla `/chat/completions` y expone [`VeniceParameters`](#parámetros-específicos-de-venice).
</Note>
...
El mismo `venice::Client` también cubre transcripción, generación de imágenes y voz. Trae al scope el trait de cliente correspondiente (`TranscriptionClient`, `ImageGenerationClient` o `AudioGenerationClient`) antes de llamar al constructor:

- **Transcripción** — `client.transcription_model(venice::WHISPER_LARGE_V3)`
- **Generación de imágenes** — `client.image_generation_model(venice::Z_IMAGE_TURBO)` (habilita la feature `image` de Rig)
