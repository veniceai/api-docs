...
<Note>
Usa `OpenAIChatModel` (Chat Completions), no `OpenAIResponsesModel` ni el atajo `openai:`. Venice no expone actualmente la API Responses de OpenAI; su ruta de generación de texto es `/chat/completions`. Fijar Chat Completions evita el comportamiento exclusivo de Responses que los modelos por defecto de OpenAI utilizan en versiones más recientes de PydanticAI.
</Note>
