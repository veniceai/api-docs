...
description: "Genera vídeos de IA consistentes con elementos de personaje y referencias de escena usando Kling O3 y Grok Imagine R2V en el Venice Video Studio."
...
Esta función está disponible en los modelos **Kling O3** y **Grok Imagine R2V** en [Venice Video Studio](https://venice.ai/video?utm_source=venice-api-documentation). Cada familia de modelos usa un enfoque diferente para las imágenes de referencia — consulta las secciones específicas del modelo a continuación.
...
Puedes añadir hasta **4 elementos** por generación. Refiérete a ellos en tu prompt usando `@Element1`, `@Element2`, etc.
...
Cada tipo de entrada tiene un tope independiente aplicado por la API:

| Límite | Valor |
|---|---|
| **Mínimo requerido** | Al menos 1 entrada visual (frame inicial, elemento o imagen de escena) |
| `elements` | **4 máximo** |
| `scene_image_urls` | 4 máximo |
| `reference_image_urls` por elemento | 1–3 |

<Note>
La API aplica estos topes de forma independiente: no hay un presupuesto combinado de imágenes entre `elements`, `scene_image_urls` y los frames inicial/final, y añadir un frame inicial o final no reduce cuántos elementos puedes enviar. Superar un tope individual devuelve un `400` que nombra ese campo, por ejemplo *"elements must have at most 4 items"*.
</Note>
...
Ve a [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). En el Model Browser de la izquierda, selecciona uno de los modelos **Kling O3 Reference to Video**:
...
Repite para personajes u objetos adicionales (hasta 4 elementos en total).

<Warning>
`elements` y `scene_image_urls` tienen cada uno un tope de **4**, y los topes se aplican de forma independiente. Consulta [Limitaciones](#limitaciones) para detalles.
</Warning>
...
| Error `"elements must have at most 4 items"` | Se enviaron más de 4 elementos | Envía como máximo 4 elementos; el tope no cambia cuando hay un frame inicial o final |
| Error `"scene_image_urls must have at most 4 images"` | Se enviaron más de 4 imágenes de escena | Envía como máximo 4 imágenes de escena. Este tope es independiente del tope de elementos |
...
| Duración | Cualquier segundo entero de 1s a 10s | 8s |
...
Ve a [venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation). En el Model Browser, selecciona **Grok Imagine R2V**.
...
| `reference_image_urls` | string[] | **Sí** | 1–7 URLs de imagen o data URLs |
| `duration` | string | **Sí** | Cualquier segundo entero de `"1s"` a `"10s"`. El sufijo `s` es obligatorio |
| `aspect_ratio` | string | **Sí** | p. ej., `"16:9"`, `"9:16"`, `"1:1"` |
| `resolution` | string | No | `"480p"` (predeterminado) o `"720p"` |

<Note>
Grok Imagine R2V no usa los campos `elements`, `scene_image_urls` ni `image_url`. Todas las imágenes de referencia se pasan vía `reference_image_urls`.
</Note>
...
| Error "At least one reference image is required" | `reference_image_urls` está vacío o ausente | Proporciona al menos una URL de imagen en `reference_image_urls` |
...
| El vídeo es demasiado corto | La duración predeterminada es 8s | Establece duración en `"10s"` para vídeos más largos |
| Error `"Invalid enum value ... received '8'"` | Se envió `duration` sin el sufijo `s` | Envía `"8s"`, no `"8"`. La API exige exactamente la forma mostrada |
