...
<Info>
**Los parámetros de cotización son solo entradas de precio.** Los campos que envías a `/video/quote` —incluyendo `aspect_ratio`, `audio` y `reference_video_total_duration`— se usan únicamente para calcular el precio. **No** se reenvían a `/video/queue` y no afectan al vídeo generado. Enviar `aspect_ratio` a quote se acepta incluso para modelos (como `seedance-2-0-image-to-video-basic`) que rechazan `aspect_ratio` en el momento de la generación, ya que los modelos image-to-video derivan la relación de aspecto de salida a partir de la imagen de entrada.
</Info>
