...
description: "Genera vídeos desde texto o imágenes con la cola asíncrona de Venice: cotiza el precio, envía a /video/queue y consulta /video/retrieve para el MP4."
...
Para los modelos que devolvieron un `download_url` en la cola, retrieve siempre devuelve JSON. Obtén el vídeo con `GET download_url` (sin cabecera de auth). Consulta [Enlaces de descarga privados](#enlaces-de-descarga-privados) para saber cómo funcionan estas URLs, los reintentos y el `DELETE` opcional.
...
| `prompt` | string | Sí | - | Qué generar. La longitud máxima es específica de cada modelo y varía mucho: desde unos cientos de caracteres hasta 20,000. Los modelos que no declaran su propio límite usan 2,500 por defecto. Lee `model_spec.constraints.prompt_character_limit` en `GET /models?type=video` en lugar de asumir un único límite |
...
| `bitrate_mode` | string | No | `"standard"` | Solo Seedance 2.0 / 2.5. `"standard"` o `"high"`. `high` solicita una codificación de mayor calidad y archivos más grandes. No cambia el precio. Omítelo para otras familias |
...
4. Si `Content-Type` es `application/json` y `status` es `"COMPLETED"`, haz `GET` del `download_url` de la respuesta de la cola para obtener el vídeo (consulta [Enlaces de descarga privados](#enlaces-de-descarga-privados))
...
A diferencia de la generación de imágenes, el endpoint de la cola de vídeo no acepta un parámetro `safe_mode`. El soporte para contenido adulto es específico del modelo: elige una variante de modelo que esté diseñada para salida sin censura. La familia Wan 2.7, por ejemplo, expone variantes Enhanced junto a las variantes predeterminadas:

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

Las solicitudes enviadas a las variantes predeterminadas pueden ser rechazadas como violaciones de contenido (`422`). Consulta [Modelos de vídeo](/models/video) y [Precios](/overview/pricing) para ver la lista actual de variantes y sus precios por segundo.
