...
description: "Mejora la resolución y calidad de vídeos con el modelo topaz-video-upscale en la cola asíncrona de vídeo de Venice, con factores de escalado 1x, 2x o 4x."
...
`video_url` es **obligatorio** para las cotizaciones en modelos de upscaling: el endpoint de cotización descarga el archivo y detecta la duración, los FPS y la altura a partir de los bytes, por lo que no hace falta proporcionarlos. Omitirlo devuelve `400` con *"video_url is required for quotes on this model"*.

<Warning>
Los endpoints de cotización y de cola nombran actualmente el factor de upscaling de forma distinta: `/video/queue` recibe `upscale_factor` (`1`, `2` o `4`), mientras que `/video/quote` lee el factor de `resolution` (`"2x"` o `"4x"`). Enviar `upscale_factor` al endpoint de cotización no tiene efecto: recurre al valor predeterminado `"2x"` y devuelve un precio de 2x para lo que puede ser un trabajo de 4x. Al cotizar un upscaling de 4x, pasa `resolution: "4x"`.
</Warning>
...
| `"Insufficient balance"` | Créditos de cuenta demasiado bajos | Añade créditos en [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation) |
