...
description: "Genera imágenes desde prompts de texto con la API de Venice, cubriendo opciones de tamaño por modelo, presets de estilo, intensidad de LoRA, modo seguro y salida binaria."
...
<Warning>
`qwen-image`, `qwen-image-3` y `qwen-image-3-pro` ya no aceptan `width` ni `height`. Enviar cualquiera de los dos devuelve `400` con *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."* Comprueba `model_spec.constraints` en `GET /models?type=image` antes de elegir los campos de tamaño.
</Warning>
...
Los nombres de los estilos están dentro de `data`, no en el nivel superior: lee `response.data` en lugar de iterar directamente el objeto de la respuesta.
...
| `width` | integer | No | `1024` | Ancho de salida en píxeles para modelos basados en píxeles como `venice-sd35`. Rechazado por la familia Qwen Image — consulta la nota más abajo |
| `height` | integer | No | `1024` | Alto de salida en píxeles para modelos basados en píxeles como `venice-sd35`. Rechazado por la familia Qwen Image — consulta la nota más abajo |
...
| `lora_strength` | integer | No | - | Intensidad (`0`-`100`) de los LoRAs integrados en el modelo; los modelos sin LoRAs adicionales lo ignoran |
...
Algunos modelos de imagen incluyen LoRAs adicionales integrados. El parámetro `lora_strength` (integer, `0` a `100`) controla con qué intensidad se aplican esos LoRAs integrados. Solo afecta a los modelos que usan LoRAs adicionales. Los demás modelos lo ignoran.
...
Algunos modelos de imagen están diseñados específicamente para generación adulta / sin censura y se combinan bien con `safe_mode: false`. Ejemplos en la página de precios incluyen `lustify-sdxl`, `lustify-v7` y `lustify-v8`. Consulta [Modelos de imagen](/models/image) y [Precios](/overview/pricing) para ver la lista actual y los costes por modelo.

Para editar imágenes existentes, el modelo de edición predeterminado `qwen-edit` bloquea la imaginería sexual explícita. Si necesitas edición sin censura, usa `qwen-edit-uncensored` a través de [Edición de imágenes](/guides/media/image-editing).
...
|---|---|
| `auto` (predeterminado) | Habilita el modo Safe Venice y difumina el contenido adulto |
| `low` | Deshabilita el modo Safe Venice; devuelve la salida sin difuminar |
