...
<Note>
Seedance I2V models (`seedance-2-0-image-to-video-api`, its Fast variant, and `seedance-2-5-image-to-video-api`) **do not accept `aspect_ratio`** — the output aspect ratio is auto-derived from the input image's dimensions. Passing the field returns a 400 with *"This model does not support aspect_ratio"*. Use the T2V or R2V variants if you need explicit aspect-ratio control.
</Note>
