...
description: "Genera, edita, amplía y une vídeos con Seedance 2.0 y 2.5 en Venice - flujos texto/imagen/referencia, comportamiento faces-off de la API y límites multimodales por familia."
'og:title': "Guía Seedance 2.0 y 2.5 | Venice API Docs"
'og:description': "Genera, edita, amplía y une vídeos con Seedance 2.0 y 2.5 vía la API de Venice. Patrones de flujo, faces-off, límites multimodales, precios y ejemplos."
---
...
<Warning>
**Los rostros no están soportados en la API pública de Seedance.** Venice no ejecuta aprovisionamiento de face assets ni atestación de consentimiento facial (`consents.seedance` / `needs_consent`) para los modelos Seedance públicos. Las entradas con rostros pueden rechazarse upstream como error de política de contenido / proveedor. Para generación con likeness / face assets, usa la app o Studio de Venice.
</Warning>
...
|---|---|---|---|
| `seedance-2-0-text-to-video` | T2V | 480p / 720p / 1080p / **4k** | Text prompt only |
| `seedance-2-0-image-to-video` | I2V | 480p / 720p / 1080p / **4k** | First-frame (and optionally last-frame) image grounding |
| `seedance-2-0-reference-to-video` | R2V | 480p / 720p / 1080p / **4k** | Up to 9 reference images + 3 reference videos + 3 reference audio donors. Powers Reference / Edit / Extend / Stitch |
| `seedance-2-0-fast-text-to-video` | Fast T2V | 480p / 720p | Faster, lower-fidelity tier (no 1080p / 4k) |
| `seedance-2-0-fast-image-to-video` | Fast I2V | 480p / 720p | Faster, lower-fidelity tier (no 1080p / 4k) |
| `seedance-2-0-fast-reference-to-video` | Fast R2V | 480p / 720p | Faster, lower-fidelity tier (no 1080p / 4k); same workflow set |
| `seedance-2-5-text-to-video` | T2V | 480p / 720p | Up to 30s output; native audio |
| `seedance-2-5-image-to-video` | I2V | 480p / 720p | Up to 30s output; first-frame (and optionally last-frame) grounding |
| `seedance-2-5-reference-to-video` | R2V | 480p / 720p | Up to 30 images + 10 videos + 10 audio donors; same Reference / Edit / Extend / Stitch workflows |
...
The reference-to-video variants (`seedance-2-0-reference-to-video`, its Fast sibling, and `seedance-2-5-reference-to-video`) use the same prompt-routing pattern. **The model infers the task from the prompt prefix and the shape of your inputs.** There is no `task` or `workflow` field — the prompt syntax is the routing.
...
<Note>
Seedance I2V models (`seedance-2-0-image-to-video`, its Fast variant, and `seedance-2-5-image-to-video`) **do not accept `aspect_ratio`** — the output aspect ratio is auto-derived from the input image's dimensions. Passing the field returns a 400 with *"This model does not support aspect_ratio"*. Use the T2V or R2V variants if you need explicit aspect-ratio control.
</Note>
