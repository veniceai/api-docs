...
description: "Ejecuta inferencia privada en Venice con modelos TEE y cifrado de extremo a extremo que mantienen tus prompts ilegibles para el host."
...
|------|--------|---------------|
| **TEE** | `e2ee-*` | El modelo se ejecuta en un enclave protegido por hardware. Venice no puede acceder al cómputo. Puedes verificarlo mediante atestación. |
...
<Info>
Tanto los ID de los modelos TEE como los E2EE usan el prefijo `e2ee-*`. Esto se debe a que todo modelo con atestación TEE en Venice también es compatible con E2EE — la atestación TEE es el mecanismo que hace posible el cifrado de extremo a extremo —, por lo que comparten una única familia de modelos y un único prefijo. Ya sea que uses inferencia TEE simple o el flujo E2EE completo, el ID del modelo es el mismo. Los modelos E2EE incluyen protección TEE más cifrado del lado del cliente. Los modelos TEE proporcionan seguridad de enclave sin requerir cifrado del lado del cliente.
</Info>
...
Consulta la [página de Modelos](/overview/models) para ver la lista completa con precios y límites de contexto.
