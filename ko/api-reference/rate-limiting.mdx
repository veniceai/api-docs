...
비디오, 음악, 보이스 체인저 작업에는 속도 제한이 적용되지 않습니다. 모두 크레딧 잔액에서 생성 건당 청구되므로, 요청 상한이 아니라 비용이 실질적인 제약입니다. 먼저 [`POST /video/quote`](/ko/api-reference/endpoint/video/quote), [`POST /audio/quote`](/ko/api-reference/endpoint/audio/quote) 또는 [`POST /audio/voice-changer/quote`](/ko/api-reference/endpoint/audio/voice-changer/quote)로 작업 비용을 산정하세요.
