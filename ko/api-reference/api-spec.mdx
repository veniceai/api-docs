...
Venice API는 인증을 위해 API 키를 사용합니다. [API 설정](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 API 키를 생성하고 관리하세요.


대부분의 API 요청에는 HTTP Bearer 인증이 필요합니다. `GET /models`, `GET /models/traits`, `GET /models/compatibility_mapping`, `GET /image/styles`를 포함한 일부 디스커버리 엔드포인트는 키 없이 읽을 수 있습니다:
...
키는 `INFERENCE` 또는 `ADMIN` 중 하나입니다. Inference 키는 추론 엔드포인트 — chat, image, video, audio, embeddings 및 augmentation 엔드포인트 — 를 호출할 수 있습니다. 계정 관리 엔드포인트에는 `ADMIN` 키가 필요합니다:

| 엔드포인트 | 필요한 키 |
|---|---|
| `GET/POST/PATCH/DELETE /api_keys`, `GET /api_keys/{id}` | `ADMIN` |
| `GET /api_keys/rate_limits/log` | `ADMIN` |
| `GET /api_keys/rate_limits` | 둘 다 가능 |
| `GET /billing/balance`, `/billing/usage-history`, `/billing/usage-analytics` | `ADMIN` |

관리자 전용 엔드포인트를 inference 키로 호출하면 `401`과 함께 `{"error":"Admin API key required"}`가 반환됩니다. `GET /api_keys/rate_limits`는 이 그룹에서 유일한 예외로, 어느 키 유형으로도 작동합니다.
...
<Warning>
base URL은 `https://api.venice.ai/api/v1`이며, `https://venice.ai`가 아닙니다. 웹사이트 도메인으로 전송된 요청은 API가 아닌 웹 앱에 도달하여 `405 Method Not Allowed`와 같은 오류로 실패합니다.
</Warning>
...
| `enable_web_citations` | boolean | 웹 검색이 활성화된 경우, LLM이 캐럿 위 첨자를 사용하여 출처를 인용하도록 요청 — 단일 출처는 `^1^`, 여러 출처는 `^1,2^`. `[1]`, `(1)`, `<sup>1</sup>` 같은 괄호 형식은 명시적으로 억제됩니다 | `false` |
...
6. **임베딩 입력**: `input`은 문자열 또는 문자열 배열이어야 합니다. 토큰 ID 배열은 HTTP 400으로 거부됩니다. LangChain `OpenAIEmbeddings`는 반드시 `check_embedding_ctx_length=False`로 설정해야 합니다
