...
오류가 발생하면 Venice API는 HTTP 상태 코드와 설명 메시지를 반환합니다. 일부 오류는 환불 상태, 권장 재시도 모델, 검증 세부 정보 또는 요청 ID와 같은 추가 필드도 포함합니다.

## 오류 응답 형식

대부분의 오류는 사람이 읽을 수 있는 메시지가 있는 `error` 필드를 반환합니다.
...
일부 엔드포인트는 엔드포인트에 특정한 추가 필드를 포함합니다. 예를 들어, 비디오 모더레이션 오류에는 크레딧이 환불되었는지 여부가 포함될 수 있습니다.
...
검증 오류에는 필드 수준 검증 정보가 있는 `details` 및 `issues` 필드가 포함될 수 있습니다.
