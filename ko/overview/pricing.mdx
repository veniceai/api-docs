---
title: API 가격
description: "모든 모델의 Venice API 가격: 채팅 토큰당 요금, 이미지·비디오·오디오 생성별 가격을 USD, VVV, DIEM으로 안내."
...
### Venice.ai 앱 구독 vs. API 요금 청구

Venice.ai의 소비자용 구독 플랜(예: [venice.ai](https://venice.ai)의 Pro 및 Max)은 [venice.ai](https://venice.ai)의 Venice 채팅, 이미지, 비디오 앱에 적용됩니다. API 요금 청구와는 별개입니다.

* API 사용량은 항상 위에 나열된 토큰당, 이미지당, 초당 또는 생성당 요율에 따라 요청 단위로 측정됩니다. 무제한 또는 미측정 API 등급은 없습니다.
* 앱 구독 혜택(무제한 채팅 프롬프트, 앱 내 비디오·음악·frontier 생성용 월간 크레딧 할당량, 크레딧 뱅킹)은 Venice 앱 내부에서만 적용됩니다. 위에서 언급한 Pro 구독자 일회성 $10 API 크레딧을 제외하면 API 액세스 권한을 부여하거나 API 크레딧으로 전환되지 않습니다.
* API 요청 비용을 충전하려면 [API 설정](https://venice.ai/settings/api?utm_source=venice-api-documentation) 페이지에서 USD 또는 암호화폐로 Venice 크레딧을 구매하거나, DIEM을 스테이킹하여 일일 할당량을 받으세요([VVV & DIEM](/overview/vvv-diem) 참조).
