---
title: Venice API 가격
description: "텍스트, 이미지, 비디오, 오디오, 임베딩(embeddings) 모델별 100만 토큰당 Venice API 요금과 웹 검색, RPC 호출, DIEM 기반 액세스에 대한 크레딧 비용 및 지원되는 결제 옵션을 표 형식으로 상세히 안내합니다."
...
<Note>
[venice.ai](https://venice.ai) 소비자용 앱(웹, 데스크톱, 모바일 채팅)의 Free, Pro, Max 요금제는 API와 별개입니다. 소비자 구독 한도 — 이미지 수량, 캐릭터 할당량, 무제한 채팅 등 — 는 해당 앱에만 적용되며 API 사용에는 **적용되지 않습니다**. API 호출은 소비자 요금제와 관계없이 항상 Venice 크레딧 잔액에서 계량 차감됩니다. Pro 구독자가 받는 일회성 크레딧에 대해서는 아래 [Pro 사용자](#pro-사용자)를 참고하세요.
</Note>
...
<Note>
[venice.ai](https://venice.ai)의 무료 일일 프롬프트 할당량은 소비자용 웹 앱의 기능이며, API의 기능이 아닙니다. API 요청은 항상 충전된 잔액에서 차감됩니다.
</Note>

### 다른 제공업체와 함께 Venice 키 사용하기

Venice API 키는 `https://api.venice.ai/api/v1`에서만 작동합니다. OpenRouter, OpenAI, Anthropic 또는 기타 제공업체의 키와 호환되지 않으며, 해당 제공업체의 키로는 Venice에 인증할 수 없습니다. Venice 모델을 사용하려면 [venice.ai/settings/api](https://venice.ai/settings/api)에서 키를 생성하고 클라이언트를 Venice 베이스 URL로 지정하세요.
