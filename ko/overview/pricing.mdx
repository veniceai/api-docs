---
title: API 가격
"og:title": "가격 | Venice API Docs"
"og:description": "Venice API의 가격에 대해 알아보세요."
---

명시되지 않은 한 100만 토큰당 가격입니다. 모든 가격은 USD입니다. 1 Diem = 하루 $1 상당의 컴퓨팅.
...
| 모델 | ID | 입력 가격 | 출력 가격 | 캐시 읽기 | 캐시 쓰기 | Context | 프라이버시 |
|---|---|---|---|---|---|---|---|
...
*100만 토큰당 가격입니다. [모든 모델 보기 →](/ko/models/text)*

### Embeddings

<div id="pricing-embedding-placeholder">

| 모델 | ID | 입력 (100만 토큰당) | 출력 (100만 토큰당) | 프라이버시 |
|---|---|---|---|---|
...
<Info>
생성 전에 정확한 가격을 알아보려면 [Audio Quote API](/ko/api-reference/endpoint/audio/quote)를 사용하세요. 재생 시간 기반 모델은 고정 가격 등급을 가지며, 초당 모델은 출력 길이를 기준으로 청구됩니다.
</Info>
...
비디오 가격은 해상도와 재생 시간에 따라 다릅니다. 정확한 견적은 [비디오 모델 페이지](/ko/models/video)를 방문하거나, [Video Quote API](/ko/api-reference/endpoint/video/quote)를 사용하세요.
...
| 기능 | 설정 | 가격 |
|---|---|---|
| 웹 검색 | `enable_web_search: true` | 1K 요청당 $10.00 |
| 웹 스크레이핑 | `enable_web_scraping: true` | 1K URL당 $10.00 |
| X 검색 (xAI) | `enable_x_search: true` | 1K 결과당 $10.00 |

</div>

<Info>
**웹 스크레이핑**은 요청당 최대 5개의 URL을 자동으로 감지하고, 콘텐츠를 스크레이프하여 구조화된 마크다운으로 변환하고, 추출된 텍스트를 모델 컨텍스트에 추가합니다. 성공적으로 스크레이프된 URL만 청구됩니다.

**X 검색**은 지원되는 Grok 모델에 대해 xAI의 네이티브 검색을 활성화합니다 (e.g., `grok-4-20-beta`). This searches both the web and X/Twitter for real-time information. Billed per search result returned by the model (e.g., if the model returns 10 search results, you are charged for 10 results at $0.01 each = $0.10).

이 요금은 표준 모델 토큰 가격에 더해 부과됩니다.
</Info>
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api">
    신용카드로 API 크레딧을 구매합니다. 크레딧은 만료되지 않습니다.
  </Card>
  <Card title="Crypto" icon="bitcoin" href="https://venice.ai/settings/api">
    crypto로 API 크레딧을 구매합니다. USD와 동일한 요금.
  </Card>
  <Card title="DIEM 스테이킹" icon="coins" href="https://venice.ai/token">
    Diem 1개 = 매일 새로고침되는 하루 $1 상당의 크레딧.
  </Card>
</CardGroup>

### Pro 사용자

Pro 구독자는 Pro로 업그레이드할 때 일회성 $10 API 크레딧을 받습니다. 테스트 및 소규모 앱 빌드에 사용하세요.
