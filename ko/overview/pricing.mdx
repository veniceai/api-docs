...
번들 크레딧은 Venice 구독 플랜에 의해 Venice 계정에 연결되는 API 크레딧입니다(예: 신규 Pro 구독자에게 부여되는 일회성 크레딧). API가 DIEM 및 USD 크레딧과 함께 소진하며, 결제 응답에서 `BUNDLED_CREDITS` 통화로 표시됩니다.

- **적용 범위** — 번들 크레딧은 모든 유료 Venice API 엔드포인트(chat, images, audio, video, embeddings)에서 사용할 수 있습니다. 이 페이지에 있는 동일한 USD 요율표를 기준으로 가격이 책정됩니다.
- **차감 순서** — API 키는 계정에서 다음 순서로 차감됩니다: **DIEM → 번들 크레딧 → USD**. 요청마다 통화를 선택할 필요가 없습니다. [자금 조달 방식](/overview/vvv-diem#how-funding-works)을 참고하세요.
- **잔액 확인 위치** — 현재 잔액은 [venice.ai/settings/api](https://venice.ai/settings/api)에서 확인하거나, [Billing Balance](/api-reference/endpoint/billing/balance) 엔드포인트를 호출해 `BUNDLED_CREDITS` 항목을 확인하세요.

<Note>
  [Venice 소비자 플랜](https://venice.ai) 페이지에 표시된 월간 크레딧 할당량(예: venice.ai 채팅 앱이나 미디어 생성기용 크레딧)은 API 번들 크레딧과는 별개이며 이 API를 통해 사용하지 못할 수 있습니다. Venice API는 [Billing Balance](/api-reference/endpoint/billing/balance) 엔드포인트가 보고하는 DIEM, 번들, USD 잔액만 소진합니다.
</Note>
