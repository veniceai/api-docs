---
title: API 가격
description: "Venice 텍스트, 이미지, 오디오, 비디오, 임베딩 모델의 토큰당·요청당 요금과 웹 검색 가격 및 결제 옵션을 안내합니다."
...
| ↳ >272K Context | | $2.50 | $11.25 | $0.25 | $3.13 | | |
...
| ↳ >272K Context | | $12.50 | $56.25 | $1.25 | $15.63 | | |
...
| ↳ >272K Context | | $6.25 | $28.13 | $0.63 | $7.81 | | |
...
API 사용은 항상 사용한 만큼 지불하는 방식입니다. 무료 API 티어나 무제한 API 플랜은 없습니다: 이 페이지의 모든 모델은 토큰당, 이미지당 또는 요청당 과금됩니다.

Venice 앱 구독([venice.ai](https://venice.ai/?utm_source=venice-api-documentation)의 채팅 인터페이스에서 사용되는 플랜)은 API 사용량과 별도로 청구됩니다. 해당 플랜의 티어, 가격, 앱 내 혜택은 이 API 문서에서 다루지 않습니다. Venice 계정에서 관리하세요.

일회성 $10 Pro 크레딧처럼 구독에 포함된 크레딧은 계정에 **번들 크레딧**으로 표시됩니다. API는 계정 잔액을 다음 순서로 사용합니다: DIEM 먼저, 그다음 번들 크레딧, 그다음 USD. 각 자금 조달 소스의 작동 방식과 잔액 확인 방법은 [VVV & DIEM](/overview/vvv-diem#how-funding-works)을 참조하세요.
