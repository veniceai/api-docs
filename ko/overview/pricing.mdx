---
title: API 가격
description: "텍스트, 이미지, 비디오, 오디오 및 음악 모델을 위한 Venice API 가격으로, USD, VVV, DIEM 기준의 토큰당, 생성당, 초당 요금을 포함합니다."
...
*100만 토큰당 가격. [모든 모델 보기 →](/ko/models/text)*
...
| Hunyuan Image 3.0 | `hunyuan-image-v3` | Per Image: $0.09 | Private |
...
| Krea v2 Large | `krea-v2-large` | Per Image: $0.07 | Anonymized |
...
| Krea v2 Medium | `krea-v2-medium` | Per Image: $0.04 | Anonymized |
...
| Muse Image | `muse-image` | Per Image: $0.02 | Anonymized |
...
> **여러 입력 이미지로 편집하기:** **Per Edit** 가격에는 첫 번째 입력 이미지가 포함됩니다. **추가 입력 이미지** 가격이 표시된 모델은 첫 번째 이후의 각 추가 입력 이미지에 대해 해당 요금을 부과합니다. 예: 편집당 $0.11에 추가 이미지 요금이 $0.0035인 모델에서 입력 이미지 3장을 사용해 편집하면 비용은 $0.11 + 2 × $0.0035 = $0.117.
...
<Info>
생성 전에 정확한 가격을 확인하려면 [Audio Quote API](/ko/api-reference/endpoint/audio/quote)를 사용하세요. 길이 기반 모델은 고정 가격 구간이 있고, 초당 가격 모델은 출력 길이에 따라 과금됩니다.
</Info>
...
비디오 가격은 해상도와 길이에 따라 다릅니다. 정확한 견적은 [Video 모델 페이지](/ko/models/video)를 방문하거나 [Video Quote API](/ko/api-reference/endpoint/video/quote)를 사용하세요.
...
<CardGroup cols={3}>
  <Card title="USD" icon="credit-card" href="https://venice.ai/settings/api?utm_source=venice-api-documentation">
    신용카드로 Venice 크레딧을 구매하세요. 크레딧은 만료되지 않습니다.
  </Card>
  <Card title="암호화폐(Crypto)" icon="bitcoin" href="https://venice.ai/settings/api?utm_source=venice-api-documentation">
    암호화폐로 Venice 크레딧을 구매하세요. USD와 동일한 요율입니다.
  </Card>
  <Card title="Stake DIEM" icon="coins" href="https://venice.ai/token?utm_source=venice-api-documentation">
    Diem 1개당 매일 갱신되는 하루 $1 상당의 크레딧입니다. Base 지갑이 연결된 Venice 계정이 필요합니다.
  </Card>
</CardGroup>

### Pro 사용자

Pro는 Venice 웹 및 모바일 앱을 위한 구독입니다. 무료 또는 무제한 API 사용을 포함하지 않습니다. 모든 API 요청은 구독 등급과 관계없이 위 표의 요율에 따라 토큰당, 이미지당, 또는 생성당 과금됩니다.

Pro 구독자는 업그레이드 시 **일회성 $10 Venice 크레딧**을 받습니다. 이 크레딧은 번들 크레딧으로 API 사용에 적용됩니다. API 키는 연결된 계정에서 **DIEM → 번들 크레딧 → USD** 순서로 차감되므로, $10은 현재 에포크의 스테이킹된 DIEM 할당량이 모두 소진된 후에만 사용됩니다. 전체 차감 순서는 [VVV & DIEM](/overview/vvv-diem)을, 키가 다음에 어떤 통화로 과금될지 확인하려면 [Billing Balance](/api-reference/endpoint/billing/balance)를 참조하세요.

### 무료 등급

무료 API 등급이나 무료 체험 키는 없습니다. 새로 생성된 키는 인증은 되지만, 계정에 DIEM, 번들 크레딧, 또는 USD로 사용 가능한 잔액이 있을 때까지 유료 엔드포인트(채팅 완성, 이미지 생성, 오디오, 비디오, 임베딩)는 `402 INSUFFICIENT_BALANCE`를 반환합니다. [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 USD 또는 암호화폐로 계정을 충전하거나, DIEM을 스테이킹하여 매일 갱신되는 할당량을 받으세요.
