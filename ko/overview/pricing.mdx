...
<Info>
**API 크레딧은 USD로 표시됩니다.** $1 USD로 $1 상당의 API 크레딧을 구매할 수 있습니다 — 별도의 토큰, 배수, 환산 비율은 없습니다. 크레딧은 만료되지 않습니다.

API 과금은 venice.ai 컨슈머 구독(Pro, Plus, Max)과 **별도**입니다. 해당 플랜은 venice.ai 앱 내 사용을 위한 것이며 지속적인 API 사용을 포함하지 **않습니다**. Pro 업그레이드 시 포함되는 일회성 API 크레딧에 대해서는 아래 [Pro 사용자](#pro-사용자) 섹션을 참고하세요.
</Info>
...
Pro 구독자는 Pro로 업그레이드 시 **일회성 $10 API 크레딧**을 받습니다. 작은 앱을 테스트하고 빌드하는 데 사용하세요.

<Note>
venice.ai 컨슈머 구독(Pro, Plus, Max)은 **venice.ai 앱 내** 사용을 위한 것이며, 정기적으로 지급되는 API 크레딧 할당량을 포함하지 않습니다. 어떤 컨슈머 플랜을 사용하든 API 요청은 위의 모델별 요율에 따라 USD 또는 DIEM 잔액에서 차감됩니다. Pro 업그레이드 시 제공되는 일회성 $10 크레딧이 구독에 포함된 유일한 API 할당량입니다.
</Note>
