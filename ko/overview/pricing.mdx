---
title: Venice API 가격
description: "텍스트, 이미지, 비디오, 오디오, 임베딩 모델의 100만 토큰당 Venice API 요금과 웹 검색, RPC, DIEM 요금 안내."
...
Venice API는 **사용량 기반(pay-as-you-go)** 방식입니다. 모든 요청은 계량되어 계정의 Venice 크레딧 잔액(USD로 충전하거나 스테이킹된 DIEM을 통해 충전)에서 차감됩니다. 무료 티어, 월간 API 할당량, 로그인 기반 구독 요금제는 없으며 — 접근은 API 키로 인증되고 요청 단위로 과금됩니다.

<Note>
[venice.ai](https://venice.ai/?utm_source=venice-api-documentation) 소비자용 앱(웹, 데스크톱, 모바일 채팅)의 Free, Pro, Max 요금제는 API와 별개입니다. 소비자 구독 한도 — 이미지 수량, 캐릭터 할당량, 무제한 채팅 등 — 는 해당 앱에만 적용되며 API 사용에는 **적용되지 않습니다**. API 호출은 소비자 요금제와 관계없이 항상 Venice 크레딧 잔액에서 계량 차감됩니다. Pro 구독자가 받는 일회성 크레딧에 대해서는 아래 [Pro 사용자](#pro-사용자)를 참고하세요.
</Note>
...
이 크레딧을 넘어선 API 사용은 위에 나열된 것과 동일한 사용량 기반 요율로 과금됩니다. Pro 구독 자체에는 포함된 이미지, 문자, 토큰과 같은 추가 API 사용 한도가 포함되어 있지 않습니다. 일부 모델은 Pro 계정으로 제한됩니다(`PRO_ONLY_MODEL`). 활성 Pro 구독이 없는 계정에서 해당 모델을 호출하면 `AUTHENTICATION_FAILED_INACTIVE_KEY`가 반환됩니다. [Error Codes](/api-reference/error-codes)를 참고하세요.

### 무료 등급

Venice API에는 무료 등급이 없습니다. 모든 채팅, 이미지, 오디오, 비디오, 임베딩 호출은 크레딧, DIEM 또는 [x402/USDC](/guides/integrations/x402-venice-api)를 통해 결제됩니다. 가장 저렴한 텍스트 모델은 입력 토큰 100만 개당 $0.05부터 시작하며, 임베딩 모델은 토큰 100만 개당 $0.01부터 시작합니다.

<Note>
[venice.ai](https://venice.ai/?utm_source=venice-api-documentation)의 무료 일일 프롬프트 할당량은 소비자용 웹 앱의 기능이며, API의 기능이 아닙니다. API 요청은 항상 충전된 잔액에서 차감됩니다.
</Note>

### 다른 제공업체와 함께 Venice 키 사용하기

Venice API 키는 `https://api.venice.ai/api/v1`에서만 작동합니다. OpenRouter, OpenAI, Anthropic 또는 기타 제공업체의 키와 호환되지 않으며, 해당 제공업체의 키로는 Venice에 인증할 수 없습니다. Venice 모델을 사용하려면 [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 키를 생성하고 클라이언트를 Venice 베이스 URL로 지정하세요.
