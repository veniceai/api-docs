---
title: 개인정보 보호
description: "Venice가 프롬프트·응답·메타데이터를 처리하는 방법과 Anonymous, Private, TEE, E2EE 네 가지 프라이버시 모드의 차이, 프록시 아키텍처, attestation, 클라이언트 측 종단간 암호화 옵션을 상세히 설명합니다."
...
| 모드 | 모델 ID 접두어 | 보호 방식 |
| --- | --- | --- |
| **TEE** | `e2ee-*` | 원격 attestation을 지원하는 하드웨어 격리 추론. |
| **E2EE** | `e2ee-*` | 검증된 TEE를 대상으로 하는 클라이언트 측 암호화. |

두 모드 모두 동일한 모델 ID를 사용합니다. 요청 흐름에 따라 모델이 TEE 전용 모드로 실행되는지 또는 클라이언트 측 E2EE를 사용하는지가 결정됩니다.
