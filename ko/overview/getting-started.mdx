...
"og:title": "빠른 시작 | Venice API Docs"
---

몇 분 만에 Venice API를 시작하고 실행하세요. API 키를 생성하고, 첫 요청을 보내고, 빌드를 시작하세요.
...
<Steps>
  <Step title="API 키 받기">
    ...
    For a detailed walkthrough with screenshots, check out the [API Key guide](/ko/guides/getting-started/generating-api-key).
  </Step>

  <Step title="API 키 설정">
    API 키를 환경에 추가하세요. 셸에서 export할 수 있습니다.
  ...
  <Step title="SDK 설치">
    Venice는 OpenAI 호환이므로 OpenAI SDK를 사용할 수 있습니다. cURL이나 raw HTTP 요청을 선호하면 이 단계를 건너뛸 수 있습니다.
  ...
  <Step title="Switch models by changing the model ID">
    ...
    <Card title="View All Models" icon="database" href="/ko/overview/models">
  ...
  <Step title="Customize response behavior (optional)">
    ...
    지원되는 모든 파라미터에 대한 자세한 내용은 [Chat Completions 문서](/ko/api-reference/endpoint/chat/completions)를 참고하세요.
  </Step>
</Steps>
...
diffusion 모델을 사용해 텍스트 프롬프트에서 이미지를 생성합니다.
...
<Card title="View All Image Models" icon="image" href="/ko/overview/models#image-models">
  See all available image models with pricing and capabilities
</Card>

For more advanced parameter options like `cfg_scale`, `negative_prompt`, `style_preset`, `seed`, `variants`, and more, check out the [Images API Reference](/ko/api-reference/endpoint/image/generate).
...
See the [Image Edit API](/ko/api-reference/endpoint/image/edit) for all parameters.
...
See the [Image Upscale API](/ko/api-reference/endpoint/image/upscale) for all parameters.
...
See the [TTS API](/ko/api-reference/endpoint/audio/speech) for all voice options.
...
See the [Transcriptions API](/ko/api-reference/endpoint/audio/transcriptions) for all options.
...
See the [Embeddings API](/ko/api-reference/endpoint/embeddings/generate) for batch processing and advanced options.
...
<CardGroup cols={2}>
  <Card title="Browse Models" icon="database" href="/ko/overview/models">
    Compare all available models with their capabilities, pricing, and context limits
  </Card>
  <Card title="API Reference" icon="code" href="/ko/api-reference/api-spec">
    Explore detailed API documentation with all endpoints and parameters
  </Card>
  <Card title="Structured Responses" icon="brackets-curly" href="/ko/guides/features/structured-responses">
    Learn how to get JSON responses with guaranteed schemas
  </Card>
  <Card title="AI Agents Guide" icon="robot" href="/ko/guides/integrations/ai-agents">
...
<CardGroup cols={2}>
  <Card title="Rate Limiting" icon="gauge" href="/ko/api-reference/rate-limiting">
    Understand rate limits and best practices for production usage
  </Card>
  <Card title="Error Codes" icon="triangle-exclamation" href="/ko/api-reference/error-codes">
    Reference for handling API errors and troubleshooting issues
  </Card>
  <Card title="Postman Collection" icon="bolt" href="/ko/guides/getting-started/postman">
    Import our complete Postman collection for easy testing
  </Card>
  <Card title="Privacy & Security" icon="shield" href="/ko/overview/privacy">
...
- **Documentation**: Browse our [complete API reference](/ko/api-reference/api-spec)
