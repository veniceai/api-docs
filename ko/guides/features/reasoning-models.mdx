...
See the full list of models, pricing and context limits on the [Models page](/ko/overview/models). Not all reasoning models support the [`reasoning_effort`](#reasoning-effort) parameter. See [model support](#model-support) for details.
...
Each model's upper bound is available as `maxCompletionTokens` on the [`/v1/models`](/ko/api-reference/endpoint/models/list) endpoint.
...
Check what a model supports via the [`/v1/models`](/ko/api-reference/endpoint/models/list) endpoint:
