...
description: "사고 과정 토큰을 노출하고, 사고 노력을 제어하며, chat completion에서 단계별 답변을 표시하는 Venice 추론 모델을 호출하세요."
...
모델, 가격, context 한도의 전체 목록은 [Models 페이지](/overview/models)에서 확인하세요. 모든 추론 모델이 [`reasoning_effort`](#reasoning-effort) 파라미터를 지원하는 것은 아닙니다. 자세한 내용은 [모델 지원](#model-support)을 참고하세요.
