---
title: "함수 호출"
description: "OpenAI 호환 함수 호출, tools 배열 도구 스키마, /chat/completions API를 사용해 Venice 채팅 모델이 실시간 데이터 조회 등 애플리케이션의 도구와 함수를 호출하고 tool_calls 결과를 대화로 되돌려 보내도록 하세요."
...
위 예시는 한 번의 왕복에서 멈춥니다. 실제 에이전트는 계속 진행합니다. 모델이 답을 내놓기 전에 대개 여러 도구가 필요하기 때문입니다. [함수 호출로 도구를 활용하는 에이전트 만들기](/guides/features/tool-using-agent)에서는 모델이 한 번도 본 적 없는 데이터베이스를 상대로 전체 루프를 실행해 봅니다. 병렬 도구 호출, 모델이 스스로 복구하는 오류 반환, 그리고 모델이 거절하는 경우와 가드가 거부하는 경우의 차이를 다룹니다.
...
- [함수 호출로 도구를 활용하는 에이전트 만들기](/guides/features/tool-using-agent)
