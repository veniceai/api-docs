---
title: Venice CLI
description: "터미널에서 채팅, 웹 검색, 이미지 생성, TTS, 전사, 비디오까지 지원하는 공식 Venice CLI, 선택적 E2EE 제공."
...
<Steps>
  ...
  <Step title="Venice API 키 발급">
    [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 발급받으세요. 단계별 안내는 [API 키 가이드](/guides/getting-started/generating-api-key)를 참고하세요.
  </Step>
...
### 문제 해결: `venice` 명령을 찾을 수 없음

`npm install -g veniceai-cli` 실행 후 터미널에서 `venice`를 인식할 수 없다고 표시되면 npm의 전역 bin 폴더가 `PATH`에 없는 것입니다. 이 문제는 Windows에서 자주 발생합니다.

1. `npm config get prefix`를 실행해 npm의 전역 설치 폴더를 확인합니다.
2. 전역 bin 폴더를 `PATH`에 추가합니다. Windows에서는 prefix 폴더 자체입니다(보통 `%AppData%\npm`). macOS와 Linux에서는 prefix의 `bin` 하위 폴더입니다.
3. 새 터미널 창을 열고 `venice chat "Hello"`를 실행합니다.

`PATH` 설정을 건너뛰려면 `npx`로 CLI를 실행하세요. 예: `npx veniceai-cli chat "Hello"`.
