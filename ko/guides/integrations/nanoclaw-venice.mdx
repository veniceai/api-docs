---
title: NanoClaw
description: "WhatsApp과 Telegram용 경량 셀프 호스팅 개인 AI 비서인 NanoClaw를 Venice API에 배포해 Anthropic 구독 없이 Docker 컨테이너 격리, 데이터 보존 제로의 비공개·무검열 채팅과 미디어 생성을 토큰 단위 결제로 활용하세요."
...
[Venice](https://venice.ai/?utm_source=venice-api-documentation)는 프라이버시 우선 AI 플랫폼입니다. Venice는 [서버에 어떤 prompt나 응답도 저장하거나 로그하지 않습니다](https://venice.ai/privacy?utm_source=venice-api-documentation) — 대화는 사용자의 기기에만 존재합니다. 요청은 Venice의 프록시를 통해 탈중앙화 GPU 공급자로 종단간 암호화되어 전송되며, 데이터 보존은 제로입니다. 즉, AI 비서 대화가 Venice조차 알 수 없도록 프라이빗하게 유지됩니다.
...
| **가격** | 토큰 단위 결제, 구독 없음. 또는 [DIEM](https://venice.ai/lp/diem?utm_source=venice-api-documentation)을 스테이킹해 매일 갱신되는 크레딧 사용 | 월 $20–200 구독 또는 토큰 단위 API |
...
<CardGroup cols={2}>
  ...
  <Card title="Venice API 키" icon="key" href="https://venice.ai/settings/api?utm_source=venice-api-documentation">
    Venice 계정에서 생성
  </Card>
</CardGroup>
