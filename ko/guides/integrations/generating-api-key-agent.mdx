...
description: "Base에서 VVV를 스테이킹하고 Venice가 발급한 단기 검증 토큰을 에이전트 지갑으로 서명해 온체인 AI 에이전트가 사람의 개입 없이 자율적으로 Venice API 키를 발급받는 전체 흐름과 추론 비용 자금 조달 옵션을 다룹니다."
...
* 스테이킹할 VVV의 0이 아닌 양. 발행 엔드포인트는 지갑에 0이 아닌 sVVV 잔액만 있으면 되므로, 1 VVV로도 키를 발행하기에 충분합니다. 실제로 유료 엔드포인트를 호출하는 데 필요한 것은 [추론 비용 지불](#추론-비용-지불)을 참조하세요.
...
<Steps>
  ...
  <Step title="API 키 발행">
    ...
    - `consumptionLimit` — 이 키의 지출 상한으로, `usd`, `vcu` 또는 `diem` 단위로 표시
    - `limitPeriod` — 상한이 측정되는 기간: `EPOCH`(매 UTC 일마다 재설정), `MONTH`(매 UTC 월 1일에 재설정), 또는 `LIFETIME`(재설정되지 않아 상한이 영구적)
    - `modelPrivacy` — 키가 호출할 수 있는 모델을 개인정보 보호 등급으로 지정: `ALL`, `PRIVATE_TEXT`, 또는 `PRIVATE_ONLY`

    <Warning>
    `limitPeriod`를 생략하면 기본값이 `EPOCH`가 되며, 이는 상한이 평생 총액이 아니라 **매일 재설정**된다는 뜻입니다. `consumptionLimit: {"usd": 50}`으로 생성하고 `limitPeriod`를 지정하지 않은 키는 하루에 $50를 지출할 수 있습니다. 무인 에이전트 키의 경우 제한이 영구 상한이 되도록 `limitPeriod: "LIFETIME"`을 명시적으로 설정하세요.
    </Warning>
