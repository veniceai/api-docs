...
Venice는 텍스트만 임베딩합니다. LangChain의 `OpenAIEmbeddings`는 기본적으로 `check_embedding_ctx_length=True`로 설정되어 있어 tiktoken으로 토큰화하고 정수 토큰 배열을 전송할 수 있습니다. Venice는 이를 HTTP 400으로 거부하므로 반드시 `check_embedding_ctx_length=False`로 설정하세요.
...
| 복잡한 추론 | `z-ai-glm-5-3-flash` | 빠른 프라이빗 추론 모델 |
| 함수 호출 | `z-ai-glm-5-3-flash` | 신뢰성 있는 도구 사용 |
