---
title: Venice MCP 서버
description: "npm의 @veniceai/mcp-server로 게시된 공식 Venice Model Context Protocol 서버는 Claude Desktop, Cursor 등 모든 MCP 호스트에서 채팅, 이미지, 비디오, 오디오, 음악, 캐릭터를 위한 31개의 Venice 도구를 노출합니다."
...
<Steps>
  <Step title="Venice API 키 발급">
    [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 키를 발급받으세요. 단계별 안내는 [API 키 가이드](/guides/getting-started/generating-api-key)를 참고하세요.
  </Step>
...
| `VENICE_SIWX_TOKEN` | _(없음)_ | x402 지갑 모드 인증 토큰. [아래 x402](#x402-지갑-모드) 참고. |
