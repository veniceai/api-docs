---
title: Venice MCP 서버
description: "공식 Venice Model Context Protocol 서버는 모든 MCP 호스트에서 채팅, 이미지, 비디오, 오디오, 음악, 캐릭터를 위한 30개의 Venice 도구를 노출합니다."
...
[Venice MCP 서버](https://github.com/veniceai/venice-mcp-server)는 Venice를 위한 공식 [Model Context Protocol](https://modelcontextprotocol.io/) 서버입니다. Venice API 전체(chat, image, video, audio, music, embeddings, web augment, characters)를 모든 MCP 호환 에이전트가 호출할 수 있는 **30개 도구**로 노출합니다.
...
<CardGroup cols={3}>
  <Card title="30개 도구" icon="toolbox">
...
Venice의 모든 모달리티를 아우르는 **30개 도구**, **3개의 리소스**(`venice://models`, `venice://styles`, `venice://voices`), **3개의 prompt 템플릿**.
