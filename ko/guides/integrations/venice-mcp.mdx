...
<Steps>
  <Step title="Get a Venice API key">
    Generate one from [venice.ai/settings/api](https://venice.ai/settings/api). See the [API key guide](/ko/guides/getting-started/generating-api-key) for step-by-step instructions.
  </Step>
...
Only relevant if you authenticate with a wallet via [x402](/ko/guides/integrations/x402-venice-api) instead of an API key.
...
Venice supports authenticating with a [Sign-In-With-X wallet token](/ko/guides/integrations/x402-venice-api) backed by prepaid USDC credit on Base or Solana, in addition to the normal API key flow. No email, phone, or KYC required: your wallet is the only identity.
...
<CardGroup cols={2}>
  ...
  <Card title="Venice Skills" icon="book" href="/ko/guides/integrations/venice-skills">
  ...
  <Card title="MCP Spec" icon="arrow-up-right-from-square" href="https://modelcontextprotocol.io/">
    자세히 알아보기: the Model Context Protocol
  </Card>
</CardGroup>
