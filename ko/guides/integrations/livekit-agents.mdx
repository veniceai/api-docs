...
description: "LiveKit Agents와 Venice로 실시간 음성 에이전트를 구축하고, `livekit-plugins-openai` 플러그인을 Venice base URL로 지정해 프라이빗 STT, LLM, TTS 모델을 STT-LLM-TTS 파이프라인으로 연결하며 각 단계를 완전히 제어하세요."
