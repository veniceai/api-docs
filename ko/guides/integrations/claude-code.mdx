---
title: Claude Code
description: "Anthropic이 배포한 Claude Code CLI를 구성해 Claude Opus 및 Sonnet 요청을 Venice API로 라우팅함으로써, 비공개·사용량 기반의 프라이빗 코딩 에이전트 추론과 도구 호출을 실행하는 방법을 단계별로 안내합니다."
"og:title": "Claude Code with Venice | Venice API Docs"
"og:description": "Venice AI를 통해 Claude Opus 4.5/4.6/4.7/4.8, Sonnet 4.5/4.6/4.7/4.8 및 Fable 5를 사용하도록 Claude Code를 설정하는 방법"
---

[Claude Code](https://docs.anthropic.com/en/docs/claude-code)는 Anthropic의 에이전트 코딩용 CLI 도구입니다. 이 가이드는 Venice AI를 통해 토큰 단위 결제로 Claude Opus 4.5/4.6/4.7/4.8, Sonnet 4.5/4.6/4.7/4.8 및 Fable 5를 사용하는 방법을 보여줍니다.
...
<CardGroup cols={3}>
  ...
  <Card title="Claude 모델" icon="microchip">
    Venice를 통해 Opus 4.5/4.6/4.7/4.8, Sonnet 4.5/4.6/4.7/4.8 및 Fable 5에 접근
  </Card>
...
<Steps>
  ...
  <Step title="API 키 발급">
    [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 키를 생성하세요. 다음 단계에서 설정 파일에 직접 붙여넣게 됩니다.
  </Step>
...
| Claude Opus 4.7 | `claude-opus-4-7` | 복잡한 추론, 대규모 리팩토링 |
| Claude Opus 4.7 Fast | `claude-opus-4-7-fast` | 더 낮은 지연 시간으로 복잡한 추론 |
| Claude Opus 4.8 | `claude-opus-4-8` | 복잡한 추론, 대규모 리팩토링 |
| Claude Opus 4.8 Fast | `claude-opus-4-8-fast` | 더 낮은 지연 시간으로 복잡한 추론 |
| Claude Fable 5 | `claude-fable-5` | 최고 수준의 지능이 필요한 복잡한 작업 |
