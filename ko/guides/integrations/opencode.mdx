---
title: OpenCode
description: "opencode.json의 커스텀 제공자로 코딩 에이전트 OpenCode를 Venice에 연결해 프라이빗 모델에서 실행하세요."
...
위와 같이 `"model": "venice/z-ai-glm-5-3-flash"`를 설정했다면 OpenCode는 기본적으로 해당 Venice 모델을 사용해야 합니다. `z-ai-glm-5-3-flash`를 [텍스트 모델 카탈로그](/models/text)의 모델 ID로 바꾸고 `models` 맵에 추가하여 기본값을 변경할 수 있습니다.
