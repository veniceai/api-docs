---
title: Jan AI
description: Use Jan AI with Venice through an OpenAI-compatible remote model provider
...
- A Venice text model ID from the [model catalog](/ko/models/text)
...
<Steps>
  ...
  <Step title="Add Venice models">
    ...
    You can add any supported text model from the [model catalog](/ko/models/text).
  </Step>
...
- Model IDs must match Venice exactly. If a model does not appear in Jan's selector, add it manually using the model ID from the [model catalog](/ko/models/text).
