...
description: "Venice로 여러분의 자료를 인용이 포함된 답변과 두 명의 진행자가 이끄는 오디오 요약으로 바꿔보세요."
...
여기서의 검색은 의도적으로 단순합니다. Python 리스트에 담긴 벡터, 반복문으로 계산하는 코사인 유사도. 몇 십 개의 자료라면 이 정도의 기계 장치가 적당하며, 움직이는 부품이 눈에 보이는 상태를 유지할 수 있습니다. 이를 넘어서는 규모라면 [프라이빗 RAG 봇 만들기](/guides/projects/private-rag-bot)에서 같은 파이프라인을 실제 벡터 데이터베이스와 재순위화 단계와 함께 다룹니다.
...
<CardGroup cols={2}>
  <Card title="프라이빗 RAG 봇" icon="database" href="/guides/projects/private-rag-bot">
