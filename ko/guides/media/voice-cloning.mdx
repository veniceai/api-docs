---
title: "음성 클로닝"
description: "Chatterbox HD로 짧은 참조 오디오 샘플에서 음성을 클로닝하고, 반환된 voice handle을 저장한 뒤 Venice Audio API로 음성을 생성하세요."
...
<Note>
Voice handle은 모델별로 구분됩니다. `tts-chatterbox-hd`로 생성된 handle은 반드시 `tts-chatterbox-hd`와 함께 사용해야 합니다. 보이스 클로닝은 참조 샘플을 기반으로 한 text-to-speech입니다. 기존 녹음을 다른 음성으로 다시 녹음하려면 대신 [보이스 체인저](/ko/guides/media/voice-changer) API를 사용하세요.
</Note>
...
- Venice API 키
- MP3, WAV, FLAC 또는 MP4 포맷의 깨끗한 참조 샘플
...
`curl -F`를 사용할 때는 `Content-Type`을 수동으로 설정하지 마세요. `curl`이 `multipart/form-data` 헤더와 필수 boundary를 자동으로 추가합니다.
...
응답 본문은 JSON이 아니라 모델의 기본 형식으로 된 바이너리 오디오입니다. `tts-chatterbox-hd`는 현재 WAV를 기본값으로 사용합니다.

---

## 전체 예제

이 예제는 참조 샘플을 업로드하고, `jq`로 voice handle을 추출한 다음, 생성된 오디오를 `chatterbox-clone.wav`에 기록합니다:
...
| `file` | file | 예 | 참조 오디오 샘플. 지원 포맷은 MP3, WAV, FLAC 및 MP4. |
...
| `response_format` | string | 아니오 | 모델별 | 선택적 출력 형식 재정의. 설정하기 전에 모델의 지원 형식과 기본 형식을 확인하세요. |
...
성공한 speech 응답은 바이너리 오디오이며, `Content-Type`이 반환된 형식을 나타냅니다. `response_format`을 생략하면 모델 기본값이 사용됩니다. 재정의하기 전에 `GET /models?type=tts`에서 `model_spec.supported_formats`와 `model_spec.default_format`을 조회하세요. 지원되지 않는 형식을 요청하면 HTTP `400`이 반환됩니다.

## 일반적인 오류

| 상태 | 원인 | 해결 방법 |
|---|---|---|
| `400` | 지원되지 않는 오디오 컨테이너 또는 호환되지 않는 voice handle | MP3, WAV, FLAC 또는 MP4를 사용하고 handle을 생성에 사용한 동일 모델과 함께 사용하세요. |
