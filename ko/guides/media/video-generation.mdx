---
title: 비디오 생성
description: "Venice API의 비동기 큐에서 텍스트 프롬프트나 시작 이미지로 비디오를 생성하세요 — /video/queue에 작업을 제출하고 queue_id로 /video/retrieve를 폴링해 완성된 MP4 비디오를 다운로드하는 전체 과정을 안내합니다."
...
큐 시점에 `download_url`을 반환한 모델은 retrieve가 항상 JSON을 반환합니다. `GET download_url`(인증 헤더 없음)로 비디오를 가져오세요. 이 URL의 동작 방식, 재시도, 선택적 `DELETE`는 [Private 다운로드 링크](#private-다운로드-링크)를 참고하세요.
...
| `prompt` | string | 예 | - | 생성할 내용. 최대 길이는 모델별로 크게 다르며 — 수백 자에서 최대 20,000자까지 다양합니다. 자체 제한을 선언하지 않는 모델은 기본값이 2,500자입니다. 단일 제한을 가정하지 말고 `GET /models?type=video`의 `model_spec.constraints.prompt_character_limit`를 읽으세요 |
...
| `bitrate_mode` | string | 아니오 | `"standard"` | Seedance 2.0 / 2.5 전용. `"standard"` 또는 `"high"`. `high`는 더 높은 품질, 더 큰 파일의 인코딩을 요청합니다. 가격은 변경되지 않습니다. 다른 패밀리에는 생략 |
...
<Info>
**견적 파라미터는 가격 산정 input 전용입니다.** `/video/quote`에 전송하는 필드(`aspect_ratio`, `audio`, `reference_video_total_duration` 포함)는 순전히 가격을 계산하는 데만 사용됩니다. 이 필드들은 `/video/queue`로 전달되지 **않으며** 생성되는 비디오에 영향을 주지 않습니다. quote에 `aspect_ratio`를 보내는 것은 생성 시점에 `aspect_ratio`를 거부하는 모델(예: `seedance-2-0-image-to-video`)에도 허용되는데, 이는 image-to-video 모델이 입력 이미지에서 출력 종횡비를 도출하기 때문입니다.
</Info>

### Reference-to-video 가격

R2V 모델(예: Seedance 2.0 R2V)의 경우 `reference_video_total_duration`(포함하려는 모든 레퍼런스 클립의 총 길이(초))을 전달하세요. 그러면 견적이 "input with video" 요율 구간과 `(input + output) × pixels` 토큰 공식을 반영합니다. 이를 생략하면 견적은 no-reference 기준선을 반환합니다.

`reference_video_total_duration`은 `/video/quote`에서만 인식됩니다. `/video/queue`에는 영향을 주지 않으며 생략해도 생성 오류가 발생하지 않습니다.

### 안정적인 가격 표시

견적은 특정 시점 기준이며 가격은 변경될 수 있습니다. 앱에서 고정 가격을 표시하려면 지원하는 각 파라미터 조합에 대해 `/video/quote`를 한 번 호출하고 결과를 자체 저장소에 캐시하세요. 그런 다음 캐시된 값을 갱신하기 위해 일정에 따라(또는 가격표 업데이트 시) 다시 견적을 요청하세요. "잠긴(locked)" 또는 "정적(static)" 견적 endpoint는 없습니다.
...
4. `Content-Type`이 `application/json`이고 `status`가 `"COMPLETED"`이면 큐 응답의 `download_url`에 `GET`해서 비디오 가져오기([Private 다운로드 링크](#private-다운로드-링크) 참고)
...
이미지 생성과 달리, 비디오 큐 endpoint는 `safe_mode` 파라미터를 받지 않습니다. 성인 콘텐츠 지원은 모델별입니다: 무검열 output을 위해 설계된 모델 변형을 선택하세요. 예를 들어 Wan 2.7 계열은 기본 변형과 함께 Enhanced 변형을 제공합니다:

- `wan-2-7-enhanced-text-to-video`
- `wan-2-7-enhanced-image-to-video`

기본 변형에 제출된 요청은 콘텐츠 위반(`422`)으로 거부될 수 있습니다. 현재 변형 목록과 초당 가격은 [비디오 모델](/models/video)과 [가격](/overview/pricing)을 참고하세요.
