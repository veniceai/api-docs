...
이미지 생성과 달리, 비디오 큐 endpoint는 `safe_mode` 파라미터를 받지 않습니다. 성인 콘텐츠 지원은 모델별입니다: 무검열 output을 위해 설계된 모델 변형을 선택하세요. 예를 들어 Wan 2.7 계열은 기본 변형과 함께 무검열 변형을 제공합니다:

- `wan-2-7-uncensored-text-to-video`
- `wan-2-7-uncensored-image-to-video`

무검열이 아닌 변형에 제출된 요청은 콘텐츠 위반(`422`)으로 거부될 수 있습니다. 현재 변형 목록과 초당 가격은 [비디오 모델](/models/video)과 [가격](/overview/pricing)을 참고하세요.
