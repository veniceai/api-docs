...
<Info>
**견적 파라미터는 가격 산정 input 전용입니다.** `/video/quote`에 전송하는 필드(`aspect_ratio`, `audio`, `reference_video_total_duration` 포함)는 순전히 가격을 계산하는 데만 사용됩니다. 이 필드들은 `/video/queue`로 전달되지 **않으며** 생성되는 비디오에 영향을 주지 않습니다. quote에 `aspect_ratio`를 보내는 것은 생성 시점에 `aspect_ratio`를 거부하는 모델(예: `seedance-2-0-image-to-video-api`)에도 허용되는데, 이는 image-to-video 모델이 입력 이미지에서 출력 종횡비를 도출하기 때문입니다.
</Info>
