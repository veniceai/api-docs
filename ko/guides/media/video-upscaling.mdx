---
title: "비디오 업스케일링"
description: "Venice API에서 Topaz Video Upscale 모델로 비디오 해상도와 품질을 향상시키세요 — /video/queue에 원본 클립을 제출하고 upscale_factor로 1x·2x·4x 스케일을 선택해 결과물을 받는 비동기 워크플로 가이드입니다."
...
업스케일 모델의 견적에는 `video_url`이 **필수**입니다 — quote endpoint가 파일을 가져와 바이트에서 duration, FPS, 높이를 감지하므로 직접 제공할 필요가 없습니다. 생략하면 `400`과 함께 *"video_url is required for quotes on this model"*이 반환됩니다.

<Warning>
현재 quote와 queue endpoint는 업스케일 배수의 이름을 다르게 사용합니다: `/video/queue`는 `upscale_factor`(`1`, `2`, `4`)를 받고, `/video/quote`는 `resolution`(`"2x"` 또는 `"4x"`)에서 배수를 읽습니다. quote endpoint에 `upscale_factor`를 보내면 아무 효과가 없습니다 — 기본값 `"2x"`로 대체되어 4x 작업일 수도 있는 것에 대해 2x 가격을 반환합니다. 4x 업스케일의 견적을 낼 때는 `resolution: "4x"`를 전달하세요.
</Warning>
...
| `"Insufficient balance"` | 계정 크레딧 부족 | [venice.ai/settings/api](https://venice.ai/settings/api?utm_source=venice-api-documentation)에서 크레딧 추가 |
