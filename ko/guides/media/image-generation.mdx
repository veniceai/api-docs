---
title: "이미지 생성"
description: "Venice API로 텍스트 프롬프트에서 이미지를 생성하는 방법을 모델 크기 옵션, 스타일 프리셋, LoRA 강도, safe mode, 바이너리 출력과 함께 안내합니다."
...
<Warning>
`qwen-image`, `qwen-image-3`, `qwen-image-3-pro`는 더 이상 `width`나 `height`를 받지 않습니다. 둘 중 하나를 보내면 `400`과 함께 *"`width` and `height` are no longer supported for this model. Use `aspect_ratio` instead."*가 반환됩니다. 크기 필드를 선택하기 전에 `GET /models?type=image`의 `model_spec.constraints`를 확인하세요.
</Warning>
...
스타일 이름은 최상위 레벨이 아니라 `data` 안에 있습니다 — 응답 객체를 직접 순회하지 말고 `response.data`를 읽으세요.
...
| `width` | integer | 아니오 | `1024` | `venice-sd35` 같은 픽셀 기반 모델의 출력 너비(픽셀). Qwen Image 계열에서는 거부됨 — 아래 참고 사항 참조 |
| `height` | integer | 아니오 | `1024` | `venice-sd35` 같은 픽셀 기반 모델의 출력 높이(픽셀). Qwen Image 계열에서는 거부됨 — 아래 참고 사항 참조 |
...
| `lora_strength` | integer | 아니오 | - | 모델에 내장된 LoRA의 강도(`0`-`100`). 추가 LoRA가 없는 모델에서는 무시됨 |
...
일부 이미지 모델에는 추가 LoRA가 내장되어 있습니다. `lora_strength` 파라미터(정수, `0`에서 `100`)는 내장 LoRA가 얼마나 강하게 적용되는지 제어합니다. 추가 LoRA를 사용하는 모델에만 영향을 주며, 다른 모델은 이를 무시합니다.

API는 커스텀 LoRA 파일의 업로드나 로드를 지원하지 않습니다. 모델에 번들된 LoRA를 나열하는 endpoint도 없습니다. LoRA 사용은 LoRA가 포함된 모델에서 `lora_strength`를 조정하는 것으로 제한됩니다.
