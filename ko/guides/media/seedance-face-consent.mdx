...
description: "When a Seedance 2.0 request contains a human face, the Venice API requires a one-time consent attestation before it will process the media. 이 가이드는 covers the two-call flow, the consent object, and the responses."
...
이 가이드는 covers exactly what you send, what you get back, and how returning requests are handled.
...
Then poll `POST /api/v1/video/retrieve` with the `queue_id` as usual (see [Video Generation](/ko/guides/media/video-generation)).
...
- Video queue endpoint: [`POST /api/v1/video/queue`](/ko/api-reference/endpoint/video/queue)
- [Seedance 2.0 Guide](/ko/guides/media/seedance-2-0) — variants, workflows, prompt syntax, limits
- [Video Generation](/ko/guides/media/video-generation) — queue / polling overview
