...
이 가이드는 walks through the variants, the four workflows with their canonical prompts, the multimodal input limits, pricing, and complete `curl` examples.
...
All variants are async. Submit via `POST /api/v1/video/queue`, then poll `POST /api/v1/video/retrieve` until the response body is `video/mp4`. See [Video Generation](/ko/guides/media/video-generation) for the general queue flow.
...
The response is JSON (`{ "status": "queued" | "running" | "failed", ... }`) until the job completes, at which point the response body switches to `video/mp4` bytes. See [Video Generation](/ko/guides/media/video-generation) for the full polling pattern.
...
- Venice video queue endpoint: [`POST /api/v1/video/queue`](/ko/api-reference/endpoint/video/queue)
- Venice quote endpoint: [`POST /api/v1/video/quote`](/ko/api-reference/endpoint/video/quote)
- Companion guide: [Reference to Video](/ko/guides/media/reference-to-video) (covers Kling O3 + Grok Imagine R2V)
- Companion guide: [Video Generation](/ko/guides/media/video-generation) (queue / polling overview)
