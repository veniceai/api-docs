...
description: "Venice에서 Seedance 2.0 및 2.5로 영상을 생성·편집·확장·스티치 - 텍스트/이미지/레퍼런스 워크플로, faces-off API, 패밀리별 멀티모달 한도."
'og:title': "Seedance 2.0 & 2.5 가이드 | Venice API Docs"
'og:description': "Venice API로 Seedance 2.0 및 2.5 영상을 생성·편집·확장·스티치. 워크플로, faces-off, 멀티모달 한도, 가격, 예제."
---
...
<Warning>
**공개 Seedance API에서는 얼굴을 지원하지 않습니다.** Venice는 공개 Seedance 모델에 대해 페이스 에셋 프로비저닝이나 얼굴 동의 증명(`consents.seedance` / `needs_consent`)을 실행하지 않습니다. 얼굴이 포함된 입력은 업스트림에서 콘텐츠 정책 / 제공자 오류로 거부될 수 있습니다. likeness / 페이스 에셋 생성은 Venice 앱 또는 Studio를 사용하세요.
</Warning>
