...
이 가이드에서는 변형, 네 가지 워크플로우, **공개 API 미디어 정책**, **패밀리별 멀티모달 제한**, 출력 비트레이트, 요금, `curl` 예제를 다룹니다.
...
| `seedance-2-5-text-to-video-basic` | T2V | 480p / 720p / 1080p | 최대 30초 출력, 네이티브 오디오 |
| `seedance-2-5-image-to-video-basic` | I2V | 480p / 720p / 1080p | 최대 30초 출력, 첫 프레임(선택적으로 마지막 프레임) 기반 |
| `seedance-2-5-reference-to-video-basic` | R2V | 480p / 720p / 1080p | 최대 이미지 30개 + 비디오 10개 + 오디오 도너 10개. 동일한 Reference / Edit / Extend / Stitch 워크플로우 |
...
`resolution`은 다음 중 하나로 전달하세요: `480p`, `720p`, `1080p` 또는 `4k`(소문자). Seedance **2.0**(non-Fast)은 네 가지 모두 허용합니다. **2.5**는 `480p` / `720p` / `1080p`를, **2.0 Fast**는 `480p` / `720p`만 허용합니다. 실시간 모델 ID와 기능은 `GET /models?type=video`로 확인하며, 사용 가능 여부를 하드코딩하지 마세요.

## 출력 비트레이트

모든 공개 Seedance **2.0**(Fast 포함) 및 **2.5** 모델에서 출력 인코딩 비트레이트를 설정할 수 있습니다. `/video/queue`에서 `bitrate_mode`를 직접 전달하세요. Venice가 이를 자동으로 선택하지 않습니다.

| 값 | 동작 |
|---|---|
| `standard` | 기본 비트레이트. 필드를 생략한 것과 동일합니다. |
| `high` | 더 높은 품질, 더 큰 파일의 인코딩. |

이는 큐 전용 인코딩 옵션입니다. 가격이나 토큰 비용을 변경하지 않으며, `/video/quote`는 `bitrate_mode`를 받지 않습니다. 다른 비디오 패밀리(Wan, Kling, LTX 등)는 이 필드를 지원하지 않습니다.
...
AI가 생성한 전환으로 입력 클립을 연결합니다. **패밀리별** 결합 길이 및 클립 수 제한을 [멀티모달 입력 제한](#멀티모달-입력-제한)에서 확인하세요 (Seedance 2.0: 클립 ≤3개 / 결합 ≤15초; Seedance 2.5: 더 높은 비디오 제한).
...
| 출력 해상도 | 480p / 720p / 1080p / **4k** (Fast: 480p / 720p 전용) | 480p / 720p / 1080p |
| 출력 비트레이트 (`bitrate_mode`) | `standard`(기본값) 또는 `high` | `standard`(기본값) 또는 `high` |
...
`omni_reference_task_type: "edit"`은 BytePlus 분류를 미리 안내합니다. `duration: "auto"`(또는 `"-1"`)와 `aspect_ratio: "adaptive"`(또는 `"auto"`)를 사용하면 출력이 소스 클립을 따릅니다. [소스와 일치하는 종횡비 및 길이](#소스와-일치하는-종횡비-및-길이) 및 [`omni_reference_task_type`](#omni_reference_task_type-seedance-25-r2v)를 참고하세요.
...
작업이 실행되는 동안 응답은 JSON(`{ "status": "PROCESSING", ... }`)이며, 완료되면 본문이 `video/mp4` 바이트로 전환됩니다. `status`는 `PROCESSING` 또는 `COMPLETED` 중 하나입니다 — `failed` 상태는 없습니다. 실패한 작업은 대신 기록된 오류를 담은 200이 아닌 응답을 반환합니다. 전체 폴링 패턴은 [Video Generation](/guides/media/video-generation)을 참고하세요.
...
reference-to-video 제출은 `reference_image_urls`, `reference_video_urls` 또는 `image_references` 중 최소 하나를 포함해야 합니다. 순수 텍스트 전용 생성은 유효한 R2V 워크플로우가 아니므로 text-to-video 모델 ID를 대신 사용하세요. `reference_audio_urls`만으로는 충분하지 않습니다(위 Audio 섹션 참고). `video_references`는 Seedance에서 허용되지 **않습니다** — 이는 레퍼런스별 음성 오디오를 지원하는 모델 전용이며, 전달하면 *"This model does not support per-reference video entities"*가 반환됩니다.
