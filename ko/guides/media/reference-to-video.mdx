...
description: "Venice API에서 Kling O3 및 Grok Imagine R2V 모델을 사용해 캐릭터 요소, 장면 레퍼런스 이미지, 멀티 샷 제어로 캐릭터·제품·환경의 외형을 여러 생성에 걸쳐 고정한 시각적 일관성 있는 AI 비디오를 생성하세요."
...
이 기능은 [Venice Video Studio](https://venice.ai/video?utm_source=venice-api-documentation)의 **Kling O3** 및 **Grok Imagine R2V** 모델에서 사용 가능합니다. 각 모델 패밀리는 레퍼런스 이미지에 서로 다른 접근 방식을 사용합니다 — 아래 모델별 섹션을 참고하세요.
...
생성당 최대 **4개의 element**를 추가할 수 있습니다. prompt에서 `@Element1`, `@Element2` 등으로 참조하세요.
...
각 input 유형은 API에 의해 독립적으로 상한이 적용됩니다:

| Limit | Value |
|---|---|
| **최소 요구 사항** | 최소 1개의 시각 input(start frame, element, scene 이미지) |
| `elements` | 최대 **4** |
| `scene_image_urls` | 최대 4 |
| Element당 `reference_image_urls` | 1–3 |

<Note>
API는 이러한 상한을 독립적으로 적용합니다 — `elements`, `scene_image_urls`, start/end frame 전체에 걸친 결합 이미지 예산은 없으며, start 또는 end frame을 추가해도 보낼 수 있는 element 수가 줄어들지 않습니다. 하나의 상한을 초과하면 해당 필드를 명시한 `400`이 반환됩니다(예: *"elements must have at most 4 items"*).
</Note>
...
[venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation)로 이동하세요. 왼쪽의 Model Browser에서 **Kling O3 Reference to Video** 모델 중 하나를 선택하세요:
...
추가 캐릭터나 객체에 대해 반복하세요(총 최대 4개 elements).

<Warning>
`elements`와 `scene_image_urls`는 각각 최대 **4**개로 제한되며, 이 상한은 독립적으로 적용됩니다. 자세한 내용은 [제한 사항](#제한-사항)을 참고하세요.
</Warning>
...
| `"elements must have at most 4 items"` 에러 | 4개를 초과하는 elements 전송 | 최대 4개의 elements 전송. start 또는 end frame이 있어도 상한은 변하지 않음 |
| `"scene_image_urls must have at most 4 images"` 에러 | 4개를 초과하는 scene 이미지 전송 | 최대 4개의 scene 이미지 전송. 이 상한은 element 상한과 독립적임 |
...
| Duration | 1s부터 10s까지의 모든 정수 초 | 8s |
...
[venice.ai/video](https://venice.ai/video?utm_source=venice-api-documentation)로 이동하세요. Model Browser에서 **Grok Imagine R2V**를 선택하세요.
...
| `reference_image_urls` | string[] | **예** | 1–7개의 이미지 URL 또는 data URL |
| `duration` | string | **예** | `"1s"`부터 `"10s"`까지의 모든 정수 초. `s` 접미사가 필수 |
| `aspect_ratio` | string | **예** | 예: `"16:9"`, `"9:16"`, `"1:1"` |
| `resolution` | string | 아니오 | `"480p"`(기본) 또는 `"720p"` |

<Note>
Grok Imagine R2V는 `elements`, `scene_image_urls`, `image_url` 필드를 사용하지 않습니다. 모든 레퍼런스 이미지는 `reference_image_urls`를 통해 전달됩니다.
</Note>
...
| "At least one reference image is required" 에러 | `reference_image_urls`가 비어있거나 누락 | `reference_image_urls`에 최소 하나의 이미지 URL 제공 |
...
| 비디오가 너무 짧음 | 기본 길이가 8초 | 더 긴 비디오를 위해 길이를 `"10s"`로 설정 |
| `"Invalid enum value ... received '8'"` 에러 | `duration`이 `s` 접미사 없이 전송됨 | `"8"`이 아니라 `"8s"`를 보내세요. API는 표시 형식과 정확히 일치해야 합니다 |
