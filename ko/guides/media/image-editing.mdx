---
title: "이미지 편집"
description: "Venice API의 /image/edit, multi-edit 레이어, 마스크, safe mode 제어를 사용해 이미지를 편집·인페인팅하고 배경을 제거하세요."
...
Venice의 이미지 편집은 동기 방식입니다. 원본 이미지를 `/image/edit` 또는 `/image/multi-edit`로 보내면 편집된 결과가 같은 응답에 raw 이미지 파일로 돌아옵니다. 누끼 작업에는 `/image/background-remove`가 투명한 PNG를 반환합니다.
...
| `POST /image/multi-edit` | 여러 장의 참조 이미지로 편집(3장, 상위 등급 모델에서는 최대 6장) | 합성 및 다중 참조 편집 |
| `POST /image/background-remove` | 이미지에서 배경 제거 | 제품, 인물, 자산을 위한 투명 컷아웃 |

<Note>
Venice는 OpenAI 호환 `/images/edits` endpoint를 제공하지 않습니다. 유일한 OpenAI 호환 이미지 경로는 생성을 위한 `POST /images/generations`입니다. 편집에는 네이티브 `POST /image/edit` endpoint를 사용하세요.
</Note>
...
**응답(200):**
응답 본문은 raw 이미지 바이너리 데이터입니다. 그대로 파일에 저장하세요. `output_format`이 파일 타입을 제어하는 방법은 [응답 포맷](#응답-포맷)을 참조하세요.
...
`/image/multi-edit`는 여러 장의 이미지를 받으며, 상한은 모델별로 다릅니다: 대부분의 편집 모델은 3장을 허용하고, `gpt-image-2-edit`, `nano-banana-pro-edit`, `seedream-v5-pro-edit`, `flux-2-max-edit`, `wan-2-7-pro-edit` 등 상위 등급 모델은 최대 6장까지 허용합니다. `GET /models?type=inpaint`의 `model_spec.constraints.maxInputImages`를 읽어 특정 모델의 상한을 확인하세요. 이 필드가 없으면 상한은 3입니다.

첫 번째 이미지가 base 이미지입니다. 나머지 이미지는 편집을 조건화하는 추가 참조입니다.

<Note>
공개 엔드포인트에는 마스크 채널이 없습니다. 추가 이미지는 영역 맵이 아니라 이미지 전체에 걸친 참조로 작동합니다 — `mask` 파라미터는 존재하지 않으며, 전송하면 `400`이 반환됩니다. 편집이 적용될 위치를 제한하려면 prompt를 정밀하게 작성하세요.
</Note>
...
`/image/edit`와 마찬가지로 응답 본문은 raw 이미지 데이터입니다.
...
`/image/edit`와 `/image/multi-edit`는 편집된 이미지를 항상 raw 바이너리 데이터(`image/png`, `image/jpeg`, 또는 `image/webp`)로 반환합니다. 편집 endpoint는 `response_format` 파라미터를 받지 않습니다. `/images/generations`처럼 `b64_json`이나 `url` 옵션도 없습니다. 결과가 JSON 안에 필요하다면 바이너리 응답을 직접 코드에서 base64로 인코딩하세요.

`output_format`으로 파일 타입을 제어하세요:

- `output_format`은 `jpeg`, `jpg`, `png`, 또는 `webp`를 받습니다.
- 생략하면 `resolution`에서 포맷이 추론됩니다: 1K 편집은 PNG, 2K와 4K 편집은 JPEG입니다.
...
| `resolution` | string | 아니오 | `1K` | 출력 이미지의 해상도 등급(`1K`, `2K`, `4K`). 지원 값은 모델별로 다름 |
| `output_format` | string | 아니오 | `resolution`에서 추론 | 출력 파일 타입: `jpeg`, `jpg`, `png`, 또는 `webp` |
| `enhance_prompt` | boolean | 아니오 | `false` | 입력 이미지를 분석하고 추론 전에 편집 지시를 다시 작성 |
| `safe_mode` | boolean | 아니오 | `true` | 편집된 결과에서 성인 콘텐츠를 흐리게 처리합니다. `false`로 설정하면 흐림이 비활성화됩니다 |
...
| Parameter | Type | Required | Default | Description |
|---|---|---|---|---|
| `images` | 파일, base64 문자열, 또는 URL 배열 | 예 | - | 첫 이미지는 base 이미지, 나머지는 추가 참조. 최대 개수는 모델별로 다름 — 기본 3장, 상위 등급 모델에서는 최대 6장 |
...
<Note>
`/image/background-remove`는 고정된 내부 모델을 사용합니다. `model` 필드를 받지 않습니다. 필드를 함께 보내면(예: `"model": "bria-bg-remover"`) `400 invalid model id`가 반환됩니다.
</Note>

### 성인 콘텐츠 및 safe mode

`/image/edit`와 `/image/multi-edit` 모두 `safe_mode`를 받습니다(기본값: `true`). 활성화되면 편집된 output의 성인 콘텐츠가 블러 처리됩니다. 블러 처리되지 않은 output을 받으려면 `safe_mode: false`로 설정하세요:
...
기본 `qwen-edit` 모델은 `safe_mode`와 무관하게 여전히 노골적 성적 이미지와 실제 폭력을 차단합니다. 무검열 편집을 원한다면 `qwen-edit-uncensored`를 사용하세요.

Endpoint 간 모더레이션 이름 차이는 사람들을 헷갈리게 만드는 지점입니다:

| Endpoint | Field to disable blur |
|---|---|
| `POST /image/edit`, `POST /image/multi-edit` | `safe_mode: false` |
| `POST /image/generate` (네이티브 생성) | `safe_mode: false` |
| `POST /images/generations` (OpenAI 호환 생성) | `moderation: "low"` |

`/images/generations`에 `safe_mode`를 전달하면 `Unrecognized key(s) in object: 'safe_mode'`와 함께 `400`이 반환됩니다.
...
|---|---|---|---|
| `/image/edit` | Base64 문자열 또는 URL | 파일 업로드 | `output_format`에 따라 `image/png`, `image/jpeg`, 또는 `image/webp` |
| `/image/multi-edit` | Base64 문자열 또는 URL | 파일 업로드 | `output_format`에 따라 `image/png`, `image/jpeg`, 또는 `image/webp` |
