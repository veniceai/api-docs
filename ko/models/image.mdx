...
API 요청에서 `id` 값을 `model` 매개변수로 사용하세요. 현재 62개의 모델을 사용할 수 있습니다.
...
<Tip>
**품질 등급.** `gpt-image-2`(생성)와 `gpt-image-2-edit`(편집/다중 편집)는 선택적 `quality` 매개변수(`low`, `medium` 또는 `high`(기본값))를 허용합니다. 더 낮은 등급은 비용이 적게 듭니다. 전체 매트릭스는 [가격 개요](/ko/overview/pricing#image-generation)를 참조하거나 [모델 엔드포인트](/ko/api-reference/endpoint/models/list)에서 `model_spec.pricing.quality`를 가져오세요.
</Tip>
